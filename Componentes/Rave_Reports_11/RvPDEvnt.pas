  ...
  if Assigned(Instance) then begin
    ...
    try
      ...
      { Don't display if property is not visible }
        If not PropertyVisible(Instance.ClassType,string(PropInfo.Name)) then Continue;
      ...
      { Don't show non-event properties }
        ...
        if GetOrdProp(Instance,PropInfo) = 0 then begin
          AvailableCB.Items.Add(string(PropInfo.Name));
        end else begin
          DefinedLB.Items.Add(string(PropInfo.Name));
        end; { else }
      end; { for }
    finally
      FreeMem(PropList,Count * SizeOf(pointer));
...
begin
  ...
  Event := TRaveEvent(GetOrdProp(Instance,GetPropInfo(Instance.ClassInfo,EventName)));
    ...
    for I2 := 0 to (CurrPage.ComponentCount - 1) do begin
      ...
      try
        ...
        { Don't show non-event properties }
          ...
          if GetOrdProp(Instance,PropInfo) <> 0 then begin
            DefinedLB.Items.Add(Instance.Name + '.' + string(PropInfo.Name));
          end; { if }
        end; { for }
      finally
        FreeMem(PropList,Count * SizeOf(pointer));
...
begin
  StatusLabel.Transparent := not ADoError;
...
begin
  inherited;
  FEndingCRLF := (Length(Text) > 0) and CharInSet(Text[Length(Text)],[#10,#13]);
