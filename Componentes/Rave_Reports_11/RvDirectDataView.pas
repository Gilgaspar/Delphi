...
begin { OpenFileMap }
  ...
  if CurrentFileMap > 0 then begin
    ...
    FilePtr := PChar(integer(AltFileBuf) + (integer(FilePtr) - integer(FileBuf)));
    FileBuf := AltFileBuf;
  end; { if }
end;  { OpenFileMap }
...
begin { IsUnique }
  ...
  if I1 >= 0 then begin
    Result := ((I1 + 1) = RTConnectList.Count) or
      (RaveCompareText(RTConnectList[I1],RTConnectList[I1 + 1]) <> 0);
  end else begin
    I1 := DTConnectList.IndexOf(Name);
    Result := (I1 < 0) or ((I1 + 1) = DTConnectList.Count) or
      (RaveCompareText(DTConnectList[I1],DTConnectList[I1 + 1]) <> 0);
  end; { else }
end;  { IsUnique }
  ...
  while StringType > 0 do begin
    ...
    {$IFDEF UNICODE}
      if HaveUnicode then begin
        FilePtr := PChar(integer(FilePtr) + Len);
      end else begin
        SetLength(S1,Len);
        if Len > 0 then begin
    ...
    {$IFDEF UNICODE}
      ...
      HaveUnicode := true;
    {$ELSE}
      FilePtr := PChar(integer(FilePtr) + Len); // skip data
    {$ENDIF}
...
begin { WriteInt }
  ...
  FilePtr := PChar(integer(FilePtr) + SizeOf(longint));
end;  { WriteInt }

procedure TRaveDataSystem.WriteInt64(Value: int64);
begin { WriteInt64 }
  int64(pointer(FilePtr)^) := Value;
  FilePtr := PChar(integer(FilePtr) + SizeOf(int64));
end;  { WriteInt64 }
...
procedure TRaveDataSystem.WriteBuf(var Buffer; Len: integer);
begin { WriteBuf }
  if Len = 0 then Exit;
...
begin { ClearBuffer }
  ...
  FilePtr := PChar(integer(FileBuf) + 2 * SizeOf(longint)); { save room for bufsize & mapidx }
end;  { ClearBuffer }
          ...
          try
            ...
            for I1 := 0 to (I2 - 1) do begin { Read in all defined columns }
              ...
              while S1 <> '' do begin
                case S1[1] of
                  ...
                  'T': begin { Type }
                    DataType := TRPDataType(GetEnumValue(TypeInfo(TRPDataType),ReadStr));
                  end;
            ...
            { Add a field definition - Find TRaveDataField component }
              ...
              for I3 := 0 to ChildCount - 1 do begin
                if RaveCompareText(TRaveDataField(Child[I3]).FieldName,FieldName) = 0 then begin
