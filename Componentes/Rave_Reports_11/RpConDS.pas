        ...
        with Fields[I1] do begin
          if not CurrentFieldDef.Included or IsNull then begin
            ...
            case TRPDataSetType[DataType] of
                ...
                try
                  if not Fields[I1].IsBLOB then begin
             	  ...
             	  {$IFDEF LevelXE3}{$WARN SYMBOL_DEPRECATED DEFAULT}{$ENDIF}
                    ...
                    if Fields[I1] is TMemoField then begin
                      S1 := (Fields[I1] as TMemoField).AsString;
                      Stream.SetSize(Length(S1) * 2);
                      Move(S1[1],Stream.Memory^,Stream.Size);
                    end else begin
                  ...
                  P1 := Stream.Memory;
                  I2 := Stream.Size;
                  if ((TRPDataSetType[DataType] = dtGraphic) or (TRPDataSetType[DataType] = dtBlob)) and (I2 >= 8) then begin
                    Move(P1^,Header,SizeOf(Header));
                    if (Header.Count = 1) and (Header.HType = $0100) and
                     (Header.Size = (I2 - SizeOf(Header))) then begin
                      P1 := pointer(integer(P1) + 8);
                      I2 := I2 - 8;
                    end; { if }
                  end; { if }
                  WriteBlobData(P1^,I2);
              ...
              dtInt64: begin
                WriteInt64Data(DisplayText,{$IFDEF Level7}(Fields[I1] as TLargeIntField).{$ENDIF}AsLargeInt);
              end;
            end; { case }
          end; { else }
        end; { with }
