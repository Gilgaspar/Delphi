...
type
  ...
  protected
    ...
    property AlternateNdrLoaded: boolean read FAlternateNdrLoaded write FAlternateNdrLoaded;
  public
...
begin
  with RvRenderPreview do begin
    ...
    if AlternateNdrLoaded then begin
      NDRStream.Free;
    end; { else }
    NdrStream := nil;
    AlternateNdrLoaded := true;
...
{ TODO: Allow user to select a single page or page range to save/export }
  if soAllowSaveFromPreview in ReportSystem.SystemOptions then begin
...
begin
  actnFile_Print.Enabled := soAllowPrintFromPreview in ReportSystem.SystemOptions;
end;

procedure TRavePreviewForm.actnFile_SaveUpdate(Sender: TObject);
begin
  actnFile_Save.Enabled := soAllowSaveFromPreview in ReportSystem.SystemOptions;
end;
...
begin
  if InputFileName <> '' then begin
    ...
    AlternateNdrLoaded := true; // set stream to be deleted if ndr loaded from preview
  end else begin
