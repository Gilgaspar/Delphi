...
{$IFDEF ConditionalExpressions}
  ...
  {$IF CompilerVersion = 16.0} { Delphi 8.0 }
    CompVer = '8';
  {$IFEND}
  {$IF CompilerVersion = 17.0} { Delphi 9.0 }
    CompVer = '9';
  {$IFEND}
  {$IF CompilerVersion = 18.0} { Delphi 10.0 }
    CompVer = '10';
  {$IFEND}
  {$IF CompilerVersion = 18.5} { C++Builder 10.0 }
    CompVer = '10';
  {$IFEND}
  {$IF CompilerVersion = 19.0} { Delphi 11.0 }
    CompVer = '11';
  {$IFEND}
  {$IF CompilerVersion = 20.0} { Delphi 12.0 }
    CompVer = '12';
  {$IFEND}
...
{$ENDIF}
  ...
  RPVersion = RvMajorVersion + '.' + RvMinorVersion + '.' + RvSubVersion + RvSpecialVersion + ' (' + LibType + CompVer + RunMode + ')'; { Current revision number }
...
{ All user created commands should start here }
 ...
 {$EXTERNALSYM FontString}
  ...
  RTFIgnoreDest: array[1..32] of string = (
...
type
  ...
  TSystemOption = (soUseFiler,soWaitForOK,soShowStatus,soAllowPrintFromPreview,
   soAllowSaveFromPreview,soPreviewModal,soNoGenerate);
  ...
  TCharArray = array[0..100000000] of char;
  ...
  public
    procedure Assign(AValue: TSaveFont);
  end; { TSaveFont }
  ...
  TFormatState = class
    ...
    Para: TFormatPara;
    Prev: TFormatState;
    procedure Assign(Value: TFormatState);
  end; { TFormatState }
...
const
  TRPDataSetType: array[low(TFieldType)..high(TFieldType)] of TRPDataType =
   (
    ...
    {ftFMTBcd}dtBCD{$IFDEF LEVEL2006},
    {ftFixedWideChar}dtBlob,
    {ftWideMemo}dtBlob,
    {ftOraTimeStamp}dtBlob,
    {ftOraInterval}dtBlob{$IFDEF LEVEL2009},
    {ftLongWord}dtInt64,
    {ftShortint}dtInteger,
    {ftByte}dtInteger,
    {ftExtended}dtFloat,
    {ftConnection}dtBlob,
    {ftParams}dtBlob,
    {ftStream}dtBlob{$IFDEF LEVEL2010},
    {ftTimeStampOffset}dtDateTime,
    {ftObject}dtBlob,
    {ftSingle}dtFloat
...
type
  ...
  function InitFileBuf(FileMap: THandle): pointer;
{$IFDEF DBG}
const
  DebugMapSize = 500000;
...
function RaveUpperCase(const S: string): string;
function RaveCompareText(const S1, S2: string): Integer;
function RavePos(const Substr, S: string): Integer;
procedure StreamWriteBuffer(Stream: TStream; Value: string);
function ExtractTotalPages(NDRStream: TStream): Integer;
{$IFNDEF LEVEL2009}
type
  TCharSet = set of char;

function CharInSet(Char: char; CharSet: TCharSet): boolean;{$ENDIF}

implementation

  procedure TFormatState.Assign(Value: TFormatState);
  begin
    Name := Value.Name;
    Size := Value.Size;
    Style := Value.Style;
...
function AnsiSameText(const S1, S2: string): Boolean;
begin
  Result := RaveCompareText(S1,S2) = 0;
end;
{$ENDIF}
...
begin { InitFileMap }
  ...
  if Result = 0 then begin
    Result := CreateFileMapping(THandle(-1),nil,PAGE_READWRITE,0,MapSize,@MapName[1]);
    if Result = 0 then begin
      RaiseLastOSError;
    end;
  end; { if }
...
begin { InitFileBuf }
  Result := MapViewOfFile(FileMap,FILE_MAP_READ OR FILE_MAP_WRITE,0,0,0);
end;  { InitFileBuf }
...
begin { WriteDebug }
  ...
  try
    ...
    Offset := integer(DebugBuf^); { Get old offset }
...
begin { InitDebugMap }
  ...
  if Result = 0 then begin
    Result := CreateFileMapping(THandle(-1),nil,PAGE_READWRITE,0,DebugMapSize,
     DebugMapName);
  end; { if }
end;  { InitDebugMap }
{$ENDIF}
...
procedure TSaveFont.Assign(AValue: TSaveFont);
begin
    Name := AValue.Name;
    Color := AValue.Color;
    Pitch := AValue.Pitch;
    ...
    Descent := AValue.Descent;
    InternalLeading := AValue.InternalLeading;
    Bold := AValue.Bold;
    Underline := AValue.Underline;
    Italic := AValue.Italic;
    Strikeout := AValue.Strikeout;
    Subscript := AValue.Subscript;
    Superscript := AValue.Superscript;
    Rotation := AValue.Rotation;
    Align := AValue.Align;
    Index := AValue.Index;
    Handle := AValue.Handle;
    CharSet := AValue.CharSet;
end;
...
procedure StreamWriteBuffer(Stream: TStream; Value: string);
begin
  Stream.WriteBuffer(Value[1],{$IFDEF Unicode}2 * {$ENDIF}Length(Value));
end;

procedure TTab.Assign(Value: TTab);
begin
  Pos := Value.Pos;
...
var
  ...
  begin { ReadByte }
    NdrStream.ReadBuffer(Result, SizeOf(Result));
  end;  { ReadByte }

  function ReadSmallInteger: smallint;
  begin { ReadSmallInteger }
    NdrStream.ReadBuffer(Result, SizeOf(Result));
  end;  { ReadSmallInteger }

begin
  Result := 0;
  FileVer := 0;
  if not Assigned(NdrStream) then Exit;

  SavePos := NDRStream.Position;
  ...
  if FileVer >= 2010 then begin
    ...
    NdrStream.Position := NdrStream.Position + wLen;
  end; { if }
...
{ Pages }
    ...
    Result := ReadSmallInteger;
	end; { else }
  NdrStream.Position := SavePos;
end;

{$IFNDEF LEVEL2009}
function CharInSet(Char: char; CharSet: TCharSet): boolean;
begin
  Result := Char in CharSet;
end;
{$ENDIF}
