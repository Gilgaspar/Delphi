...
begin { WriteStr }
  ...
  if UseFinal then begin
    StreamWriteBuffer(OutputStream,Value);
  end else begin
    StreamWriteBuffer(WorkFile,Value);
  end; { else }
end;  { WriteStr }
...
begin { WriteText }
  if ContainsWideChars(Text) then begin
    ...
    while I1 < Length(Text) do begin
      ...
      // Method 2 - preferred
        if AnsiChar(Text[I1]) in ['\','{','}'] then begin
          S1 := S1 + '\';
        end; { if }
        S1 := S1 + Text[I1];
...
var
  ...
  FontHeight: double;
begin { LeftText }
  FontHeight := Converter.FontData.Size / 72;
...
begin
  ...
  WriteStr('{\dptxbxtext {\f' + IntToStr(GetFontIndex) + '\fs' +
   IntToStr(Round(Converter.FontData.Size * 2)) + ' ');
  WriteFont;
  WriteColor;
  WriteText(AText);
  WriteStr('\par}}');
...
begin
  ...
  { code for later version of MicroSoft Word
  if (Metafile.MMWidth <= 525) and (Metafile.MMHeight <= 516) then begin
    WriteStr('\picscalex1');
    WriteStr('\picscaley1');
  end;
  }
...
procedure TRvRenderRTF.PrintBitmapRect(const X1, Y1, X2, Y2: double; AGraphic: TBitmap);
begin
  WriteImageHeader;
  WritePictType;
