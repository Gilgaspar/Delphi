...
type
  ...
  protected
    ...
    procedure PushData(var Data;
                           Len: integer);
    procedure PushInteger(Data: integer);
...
{ class TRCPURunner }
  ...
  procedure TRCPURunner.PushInteger(Data: integer);
  begin { PushInteger }
    Push(4);
    integer(pointer(longint(SS) + SP)^) := Data;
  end;  { PushInteger }
...
// !!! Read in Unicode and ansi versions of strings

            Len := GetCodeIndex;
            P1 := pointer(GetCodeAddress);
            Initialize({$IFDEF CPUX64}string(P1^){$ELSE}string(longint(P1^)){$ENDIF});
            {$IFDEF CPUX64}string(P1^){$ELSE}string(longint(P1^)){$ENDIF} := PChar(longint(CS) + CO);
...
// !!! Read in Unicode and ansi versions of strings
        ...
        { Call }
          ...
          RAVECOPYPARAMSTRDEF: begin
            ...
            if pointer(Addr) = nil then begin { Copy default data }
              {$IFDEF CPUX64}string(P2^){$ELSE}string(longint(P2^)){$ENDIF} := PChar(longint(CS) + Offset);
            end else begin
              {$IFDEF CPUX64}string(P2^){$ELSE}string(longint(P2^)){$ENDIF} := {$IFDEF CPUX64}string(P1^){$ELSE}string(longint(P1^)){$ENDIF};
            end; { else }
          end;
          RAVEFREESTR: begin
            P1 := pointer(GetCodeAddress);
            Finalize({$IFDEF CPUX64}string(P1^){$ELSE}string(longint(P1^)){$ENDIF});
          end;
          RAVEFREEREC: begin
            P1 := pointer(GetCodeAddress);
        ...
        { Compare }
          RAVEEQBOL..RAVEGESTR: begin
            ...
            {$IFDEF CPUX64}
              RAVEEQSTR: boolean(P3^) := string(P1^) = string(P2^);
              RAVENESTR: boolean(P3^) := string(P1^) <> string(P2^);
              RAVELTSTR: boolean(P3^) := string(P1^) < string(P2^);
              RAVEGTSTR: boolean(P3^) := string(P1^) > string(P2^);
              RAVELESTR: boolean(P3^) := string(P1^) <= string(P2^);
              RAVEGESTR: boolean(P3^) := string(P1^) >= string(P2^);
            {$ELSE}
        ...
        { Conv }
          ...
          RAVECHR2STR: begin
            P1 := pointer(GetCodeAddress);
            P2 := pointer(GetCodeAddress);
            Initialize({$IFDEF CPUX64}string(P2^){$ELSE}string(longint(P2^)){$ENDIF});
            SetString({$IFDEF CPUX64}string(P2^){$ELSE}string(longint(P2^)){$ENDIF},PChar(P1^),1);
          end;
        ...
        { Assign }
          RAVEASSIGN..RAVERIGHTASSIGNSTR: begin
            ...
            if RaveCommand <= RAVERIGHTASSIGN then begin { Data assign }
              ...
              {$IFDEF CPUX64}string(P1^){$ELSE}string(longint(P1^)){$ENDIF} :=
               {$IFDEF CPUX64}string(P2^){$ELSE}string(longint(P2^)){$ENDIF};
            end; { else }
        ...
        { Misc }
          ...
          RAVEADDSTR: begin
            ...
            if (P3 <> P1) and (P3 <> P2) then begin { Init string }
              Initialize({$IFDEF CPUX64}string(P3^){$ELSE}string(longint(P3^)){$ENDIF});
            end; { if }
          {$IFDEF CPUX64}
            string(P3^) := string(P1^) + string(P2^);
          {$ELSE}
