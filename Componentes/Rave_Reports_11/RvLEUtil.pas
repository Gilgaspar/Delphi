...
uses
  Windows, VCL.ComCtrls, VCL.Dialogs, CommCtrl,
  Classes, SysUtils, RvLEDefine, RvClass, RvUtil, RpDefine;

type
  TTKStr = string; //!!!UNI!!! [255];
...
asm  { FuncCaller }
{$IFDEF CPUX64}
// rax=Proc, rcx=StackItems, rdx=StackData
  test rcx,rcx
  jz   @2
@1:
  mov  rbx,[rdx]
...
asm  { RecCaller }
{$IFDEF CPUX64}
// rax=Proc, rcx=StackItems, rdx=StackData, [bp-8]=RecordData
  test rcx,rcx
  jz   @2
@1: { Push parameters }
  mov  rbx,[rdx]
...
begin { SetText }
  ...
  if Value = '' then begin
    ...
    Move(Value[1],Buf^,(Length(Value) + 1){$IFDEF UNICODE} * 2{$ENDIF});
  end; { else }
end;  { SetText }
  ...
  { Skip whitespace }
    ...
    case BufPtr^ of
      ...
      'A'..'Z','a'..'z','_': begin { ID }
        repeat
          Next;
        until not CharInSet(BufPtr^,['0'..'9','A'..'Z','a'..'z','_']);
        TokenType := ttID;
      end;
      '\','''': begin { String }
        TokenEnd := BufPtr;
        ...
        while true do begin
          case BufPtr^ of
            '\': begin
              Next;
              I1 := 0;
              while CharInSet(BufPtr^,['0'..'9']) do begin
      ...
      '#': begin { Hex }
        Next;
        HexVal := 0;
        while CharInSet(BufPtr^,['0'..'9','A'..'F','a'..'f']) do begin
      ...
      '$': begin { Currency }
        ...
        if not CharInSet(BufPtr^,['0'..'9']) then begin
          AddError(8,'');
          Finished := false;
        end else begin
          while CharInSet(BufPtr^,['0'..'9']) do begin
      ...
      '/': begin { /, /=, // }
        ...
        if BufPtr^ = '=' then begin
          ...
          repeat
            Next;
          until CharInSet(BufPtr^,[#0,#10]);
          Finished := false;
        end; { else }
      end;
      '-': begin { -, -= or -> }
        Next;
        if CharInSet(BufPtr^,['='{,'>'}]) then begin
          Next;
        end; { if }
      end;
      '.': begin { . or .. }
      ...
      '0'..'9': begin { Number }
        repeat
          Next;
        until not CharInSet(BufPtr^,['0'..'9']);
        TokenType := ttInteger;
        if BufPtr^ = '.' then begin
          Next;
          if BufPtr^ = '.' then begin { .. - return int token first }
            ...
            if CharInSet(BufPtr^,['E','e']) then begin
              Next;
              if CharInSet(BufPtr^,['+','-']) then begin
                Next;
              end; { if }
...
{ Assign token to TokenName }
  ...
  if TokenType <> ttEOF then begin
    if TokenEnd = TokenStart then begin
      ...
      //!!!UNI!!! FTokenName[0] := Chr(TokenEnd - TokenStart);
      SetLength(FTokenName, TokenEnd - TokenStart);
      SrcPtr := TokenStart;
