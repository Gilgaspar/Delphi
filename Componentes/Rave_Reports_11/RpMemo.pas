...
type
  ...
  protected
    ...
    LastRPTF: string; { Last piece of RPTF code }
  ...
  public
    ...
    procedure SetData(var Buffer; BufSize: integer);
...
{ class TMemoBuf }
  ...
  { Calculate length of white space in front }
    WSLen := 0;
    repeat
      if {$IFDEF Unicode}{$IFDEF LEVELXE4}Result[WSLen + 1].IsWhiteSpace{$ELSE}TCharacter.IsWhiteSpace(Result[WSLen + 1]){$ENDIF}{$ELSE}AnsiChar(Result[WSLen + 1]) in [#9,#10,#13,' ']{$ENDIF} then begin
        Inc(WSLen);
      end else begin
        Break;
      end; { else }
  ...
  { Search for first non-whitespace character or end of buffer }
    while (FPos < Size) and {$IFDEF Unicode}({$IFDEF LEVELXE4}FBuffer[FPos].IsSeparator{$ELSE}TCharacter.IsSeparator(FBuffer[FPos]){$ENDIF} or (FBuffer[FPos] = #9)){$ELSE}CharInSet(FBuffer[FPos],[#9,' ']){$ENDIF} and
  ...
  { Check to see if we are on a carriage return }
    if (FPos < Size) and (FBuffer[FPos] = #13) then begin
      FNewParagraph := true;
      Inc(FPos); { Skip #13 }
      if (FPos < Size) and (FBuffer[FPos] = #10) then begin
        Inc(FPos); { Skip #10 }
      end; { if }
      Result := #13; { Return carriage return only }
    end else if (FPos < Size) and (FBuffer[FPos] = #10) then begin
      FNewParagraph := true;
      Inc(FPos); { Skip #10 }
      Result := #13; { Return carriage return only }
    end else begin
    { Search for first whitespace character or end of buffer }
      while (FPos < Size) and
       not {$IFDEF Unicode}{$IFDEF LEVELXE4}FBuffer[FPos].IsWhiteSpace{$ELSE}TCharacter.IsWhiteSpace(FBuffer[FPos]){$ENDIF}{$ELSE}(AnsiChar(FBuffer[FPos]) in [#9,#10,#13,' ']){$ENDIF} and
       ((FPos - StartPos) < MAXWORDLEN) do begin
        Inc(FPos);
      end; { while }
      ...
      if FPos <> StartPos then begin
        Move(FBuffer[StartPos],Result[1],{$IFDEF Unicode}2 * {$ENDIF}(FPos - StartPos));
      end; { if }
    end; { else }
  end;  { GetWord }
        ...
        if TestWidth > AdjWidth then begin
          if UnformatLen(Result) = 0 then begin { First word was too long, cut it down }
            repeat { Add on characters until enough width }
              ...
              S1 := S1 + FBuffer[FPos];
              if System.Pos(RPTFPrefix,StartState + S1) = 0 then begin
                TestWidth := SimpleTextWidth(BaseReport,S1,0);
              end else begin
                TestWidth := RPTFTextWidth(BaseReport,StartState + S1);
              end; { else }
              if TestWidth <= AdjWidth then begin
                if S1 = ' ' then begin
                  Result := '';
            ...
            until (TestWidth >= AdjWidth) or (FPos >= Size);
  ...
  begin { GetText }
    if Size >= MAXWORDLEN then begin { Only return the first MAXWORDLEN chars }
      SetLength(Result,MAXWORDLEN);
      Move(FBuffer^,Result[1],{$IFDEF Unicode}2 * {$ENDIF}MAXWORDLEN);
    end else begin
      if Size > 0 then begin
        SetLength(Result,FSize);
        Move(FBuffer^,Result[1],{$IFDEF Unicode}2 * {$ENDIF}FSize);
      end; { if }
    end; { else }
  end;  { GetText }
  ...
  begin { FreeBuffer }
    if Assigned(FBuffer) then begin
      FreeMem(FBuffer,{$IFDEF Unicode}2 * {$ENDIF}FMaxSize);
    end; { if }
  ...
  procedure TMemoBuf.SetData(var Buffer; BufSize: integer);
  var
    TextSize: integer;
    RPTFCh: PChar;
    RPTFStr: string;
  ...
  begin { SetData }
    ...
    if TextSize = BufSize then begin { No formatting }
      FSize := TextSize;
      FMaxSize := TextSize;
      GetMem(FBuffer,{$IFDEF Unicode}2 * {$ENDIF}FMaxSize);
      Move(Buffer,FBuffer^,{$IFDEF Unicode}2 * {$ENDIF}FSize);
        ...
        if IsRPTF then begin
          ...
          if RPTFCh^ = RPTFPrefix then begin
            if TextLen > 0 then begin { Write out TextStr and add RPTF item }
              ...
              RPTFStr := '';
              Move(TextCh^,FBuffer^[BufPos],{$IFDEF Unicode}2 * {$ENDIF}TextLen);
              Inc(BufPos,TextLen);
              TextLen := 0;
            end; { if }
            IsRPTF := true;
  ...
  begin { SetRTF }
    ...
    try
      ...
      SetData(RPTFStream.Memory^,RPTFStream.Size{$IFDEF Unicode}div 2{$ENDIF});
    finally
      RPTFStream.Free;
    end; { tryf }
  end;  { SetRTF }
  ...
  {$IFDEF Unicode}
    ...
    Stream.Read(Bytes, BufSize);
    Encoding := nil;
    Offset := TEncoding.GetBufferEncoding(Bytes, Encoding);
    SetText(Encoding.GetString(Bytes, Offset, BufSize - Offset));
  {$ELSE}
  ...
  { Check for BufPos out of range }
    ...
    end else if BufPos > Size then begin
      BufPos := Size; { Insert after last char }
    end; { else }

  { Allocate more space in buffer if not enough already }
    Needed := Size + MemoBuf.Size;
    if Needed > FMaxSize then begin { Allocate more space }
      Needed := ((Needed - 1) div FBufferInc + 1) * FBufferInc;
      GetMem(NewBuffer,{$IFDEF Unicode}2 * {$ENDIF}Needed);
      Move(FBuffer^,NewBuffer^,{$IFDEF Unicode}2 * {$ENDIF}BufPos);
    end else begin
      NewBuffer := FBuffer;
    end; { else }
  ...
  { Move any trailing data to make room for Text}
    Move(FBuffer^[BufPos],NewBuffer^[BufPos + MemoBuf.Size],{$IFDEF Unicode}2 * {$ENDIF}(FSize - BufPos));

  { Insert Text }
    Move(MemoBuf.Buffer^[0],NewBuffer^[BufPos],{$IFDEF Unicode}2 * {$ENDIF}MemoBuf.Size);
  ...
  begin { AppendMemoBuf }
    InsertMemoBuf(Size,MemoBuf);
  end;  { AppendMemoBuf }

  function TMemoBuf.Empty: boolean;
  begin { Empty }
    Result := (Size = 0) or (FPos >= Size);
  end;  { Empty }
  ...
  begin { SetMaxSize }
    ...
    { Copy characters over }
      GetMem(NewBuffer,{$IFDEF Unicode}2 * {$ENDIF}Actual);
      Move(FBuffer^,NewBuffer^,{$IFDEF Unicode}2 * {$ENDIF}FSize);
...
var
  ...
  { Move any trailing data to make room for Text}
    Move(FBuffer^[BufPos],NewBuffer^[BufPos + Length(Text)],{$IFDEF Unicode}2 * {$ENDIF}(FSize - BufPos));

  { Insert Text }
    if Text <> '' then begin
      Move(Text[1],NewBuffer^[BufPos],{$IFDEF Unicode}2 * {$ENDIF}Length(Text));
    end; { if }
  ...
  { Adjust RPTF commands }
    ...
    if (BufPos < Size) and (BufPos >= 0) then begin
      if DelLen >= (Size - BufPos) then begin { Take off end only }
        FSize := BufPos;
      end else begin
        Move(FBuffer^[BufPos + DelLen],FBuffer^[BufPos],{$IFDEF Unicode}2 * {$ENDIF}(FSize - (BufPos + DelLen)));
        FSize := FSize - DelLen;
      end; { else }
    end; { if }
  end;  { Delete }
  ...
  begin { SearchFirst }
    ...
    if not FCaseMatters then begin
      FSearchText := RaveUpperCase(FSearchText);
    end; { if }
    ...
    for I1 := Start to Finish do begin
      ...
      for I2 := 1 to FSearchLen do begin
        if FCaseMatters then begin
          ...
          Ch := BufPtr^;
          if RaveUpperCase(Ch) <> FSearchText[I2] then begin
            Found := false;
            Break;
          end; { if }
        end; { else }
  ...
  begin { SaveBuffer }
    FreeSaved;
    FSaveSize := FSize;
    GetMem(FSaveBuffer,{$IFDEF Unicode}2 * {$ENDIF}FSaveSize);
    Move(FBuffer^,FSaveBuffer^,{$IFDEF Unicode}2 * {$ENDIF}FSaveSize);
  ...
  begin { RestoreBuffer }
    if Assigned(FSaveBuffer) then begin
      MaxSize := FSaveSize;
      Move(FSaveBuffer^,FBuffer^,{$IFDEF Unicode}2 * {$ENDIF}FSaveSize);
      FSize := FSaveSize;
  ...
  begin { FreeSaved }
    if Assigned(FSaveBuffer) then begin
      FreeMem(FSaveBuffer,{$IFDEF Unicode}2 * {$ENDIF}FSaveSize);
