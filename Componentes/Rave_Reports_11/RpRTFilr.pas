...
uses
  ...
  Forms, Dialogs, StdCtrls, ExtCtrls, RPDefine, RPBase, RPDevice, RPRPTF {$IFDEF LEVELXE4}, AnsiStrings{$ENDIF};
...
{*************************************************************************}
    ...
    { Output tab settings if any }
      ...
      while Assigned(TempTab) do begin
        with TempTab do begin
          ...
          case TempJust of
            pjCenter: begin
              WriteStr('\tqc\tx' + IntToStr(Round(1440 * (Pos - FLeftMargin + Width / 2.0))));
            end;
            pjRight: begin
              WriteStr('\tqr\tx' + IntToStr(Round(1440 * (Pos - FLeftMargin + Width - Margin))));
            end;
            else begin
              WriteStr('\tx' + IntToStr(Round(1440 * (Pos - FLeftMargin + Margin))));
            end;
          end; { case }
          Inc(TabIdx);
          TempTab := GetTab(TabIdx);
        end; { with }
      end; { while }
    end; { if }
  end;  { WritePara }
  ...
  { Look for any special characters }
    ...
    while I1 < Length(Text) do begin
      Inc(I1);
      if CharInSet(Text[I1], ['\','{','}']) then begin
        Insert('\',Text,I1);
        Inc(I1);
      end; { if }
    end; { while }
    ...
    { Create final file }
      case FStreamMode of
        ...
        smTempFile: begin
          ...
          GetTempFileNameA(@TempPath[1],@S1[1],0,@S2[1]);
          FOutputFileName := {$IFDEF LEVELXE4}AnsiStrings.{$ENDIF}StrPas(PAnsiChar(@S2[1]));
