...
type
  ...
  protected
    ...
    FSource: string; { String containing original source }
  ...
  public
    ...
    procedure CompileStream(Source: string;
                            FileName: string;
                            AModule: TRCPUModule;
...
begin { CreateRaveCompiler }
  ...
  if Assigned(CompilerList) then begin
    for I1 := 0 to CompilerList.Count - 1 do begin
      if RaveCompareText(TRaveCompilerClass(CompilerList[I1]).SyntaxName,
...
procedure TRCPUModule.LoadFromStream(Stream: TStream;
                                     aOwner: TRaveProjectItem;
                                     aProjectItem: TRaveProjectItem);
var
...
begin { CreateFromStream }
  inherited Create;
  LoadFromStream(Stream,aOwner,aProjectItem);
...
begin { CreateFromFile }
  ...
  try
    ...
    LoadFromStream(Stream,AOwner,nil);
  finally
    Stream.Free;
  end; { tryf }
end;  { CreateFromFile }
  ...
  { Write out module header }
    ...
    for I1 := 0 to UsedModules.Count - 1 do begin
      ...
      if Assigned(TRCPUModule(UsedModules[I1]).ProjectItem) then begin
        {$IFDEF LEVEL6}
        Write('+' + TRCPUModule(UsedModules[I1]).ModuleName);
        {$ELSE}
...
begin { BuildSource }
  if not Assigned(CompilerList) or (CompilerList.Count = 0) then begin
    RaveError({Trans-}'Compiler not found.  Include RvLDCompiler in the uses.');
  end;
    ...
    try
      ...
      { Don't show non-event properties }
        ...
        if Assigned(RaveEvent) then begin
          RaveEvent.Name := string(PropInfo.Name);
          SourceRefList.Add(RaveEvent);
          AddLine('');
          EventList.AddObject(Instance.Name + '_' + string(PropInfo.Name),RaveEvent);
...
begin { Compile }
  ...
  Compiler.CompileStream(Source,ProjectItem.ModuleName,self,Definition);
...
begin { SetStringVar }
  ...
  {$IFDEF CPUX64}string(P1^){$ELSE}string(longint(P1^)){$ENDIF} := Value;
end;  { SetStringVar }

function TRCPUModule.GetStringVar(Name: string): string;
begin { GetStringVar }
  {$IFDEF CPUX64}
  Result := string(GetDataPtr(FindID(Name) as TVarID)^);
  {$ELSE}
