...
{ Get designer DLL filename }
  with TRegistry.Create do try
    ...
    if OpenKeyReadOnly(RegRoot) then begin
    ...
    With Component as TRvProject do begin
      If ProjectFile <> '' then begin
        ...
        IniFileName := IncludeTrailingPathDelimiter(TempBuf) + 'Rave.ini';
...
//!!PORT!! Need to handle the archived component types in here - Class function?
    if (Component is RpSystem.TRvSystem) or (Component is RpFiler.TRvNDRWriter){$IFDEF LEVELCLX} or
...
{$ENDIF}  
  ...
  GroupDescendentsWith(RpDefine.TRpComponent, Controls.TControl);
...
,
 // BEX only components
   RpRenderBitmap.TRvRenderBitmap,
   RpRenderMetafile.TRvRenderMetafile,
   RpRenderJPEG.TRvRenderJPEG,
