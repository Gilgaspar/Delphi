...
uses
  Messages, Windows, VCL.Controls, VCL.Graphics, VCL.Buttons, VCL.StdCtrls, VCL.Forms,
...
begin { RaveRegister }
  ...
  RegisterRaveComponents({Trans-}'Report',[TRaveDataText,TRaveDataMemo,TRaveCalcText,
   TRaveDataMirrorSection,TRaveRegion,TRaveBand,TRaveDataBand,
   TRaveDataCycle,TRaveCalcOp,TRaveCalcTotal,TRaveCalcController]);
...
function TRaveDataMemo.AllowPrint: boolean;
begin
  if DonePrinting then begin
    Result := false;
  end else begin
    Result := inherited AllowPrint;
  end; { else }
end;
...
begin { AddListener }
  ...
  if RaveCompareText(Conversation,{Trans-}'CalcNewData') = 0 then begin
...
//!!!CLR!!!
  ...
  end else if RaveCompareText(Conversation,{Trans-}'CalcInitData') = 0 then begin
...
//!!!CLR!!!
  if (RaveCompareText(Conversation,{Trans-}'CalcNewData') = 0) and
   Assigned(CalcNewDataListenList) then begin
    CalcNewDataListenList.RemoveMethod(TMethod(ListenMethod));
  end else if (RaveCompareText(Conversation,{Trans-}'CalcInitData') = 0) and
...
begin { Habla }
  Result := (RaveCompareText(Conversation,{Trans-}'CalcNewData') = 0) or
    (RaveCompareText(Conversation,{Trans-}'CalcInitData') = 0);
end;  { Habla }
    ...
    for I1 := 0 to DataMirrors.Count - 1 do begin
      with TRaveDataMirror(DataMirrors[I1]) do begin
        ...
        if (CompareCase and (AnsiCompareStr(DataText,DataValue) = 0)) or
         (not CompareCase and (RaveCompareText(DataText,DataValue) = 0)) then begin
...
begin { RestoreAdjusted }
  ...
  for I1 := 0 to FDataMirrors.Count - 1 do begin
    if Assigned(TRaveDataMirror(FDataMirrors[I1]).SectionMirror) then begin
      RestoreAdjustedCaller(TRaveDataMirror(FDataMirrors[I1]).SectionMirror);
    end; { if }
  end; { for }
...
procedure TRaveDataMirrorSection.SaveAdjusted;
var
  I1: Integer;
begin
  inherited;
  for I1 := 0 to FDataMirrors.Count - 1 do begin
    if Assigned(TRaveDataMirror(FDataMirrors[I1]).SectionMirror) then begin
      SaveAdjustedCaller(TRaveDataMirror(FDataMirrors[I1]).SectionMirror);
    end; { if }
  end; { for }
end; { RestoreAdjusted }
