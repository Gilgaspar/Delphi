...
type
  ...
  {$IFDEF DBG}
    ...
    DebugBuf: PAnsiChar; { Pointer to MMF }
  {$ENDIF}
    NormFileMap: THandle; { Handle to normal MMF }
    NormFileBuf: PAnsiChar; { Pointer to normal MMF }
    AltFileMap: THandle; { Handle to alternate MMF }
    AltFileBuf: PAnsiChar; { Pointer to alternate MMF }
    CurrentMapSize: longint; { Size of current MMF }
    CurrentFileMap: byte; { 0=FileBuf; 1=AltFileBuf[1]; 2=AltFileBuf[2] }
    FileBuf: PAnsiChar; { Pointer to current MMF }
    FilePtr: PAnsiChar; { Utility pointer for accessing FileBuf }
    RowHeaderPtr: PAnsiChar; { Where the row header in FileBuf is stored }
...
{ class TDataConnectManager }
  ...
  begin { FindConnection }
    for I1 := 0 to FConnectList.Count - 1 do begin
      ...
      if RaveCompareText(Result.Name,ConnectName) = 0 then begin
        Exit;
      end; { if }
    end; { for }
    Result := nil;
            ...
            if WaitResult = WAIT_OBJECT_0 then begin { Data Request }
              ...
              case I1 of
                ...
                DATAACKNOWLEDGE: begin
                  ...
                  WriteInt(integer(FDataCon)); { Connection Handle } //!!!CLR!!!
...
const
  DataTypeSize: array[TRPDataType] of byte =
  (SizeOf(string),
   SizeOf(integer),
   SizeOf(boolean),
   SizeOf(extended),
...
{ class TRvCustomConnection }
  ...
  begin { GetFieldAlias }
    ...
    for I1 := 0 to FFieldAliasList.Count - 1 do begin
      ...
      I2 := Pos('=',S1);
      if (I2 > 0) and (RaveCompareText(FieldName,Trim(Copy(S1,1,I2 - 1))) = 0) then begin
  ...
  {$IFDEF UNICODE}
    ...
    WriteInt(2); // Unicode string follows
    Data := Encoding.GetBytes(Value);
    I1 := Length(Data);
    WriteInt(I1);
    if I1 > 0 then begin
      Move(Data[0],FilePtr^,I1);
      FilePtr := PAnsiChar(integer(FilePtr) + I1);
    end; { if }

  // Convert unicode string to ANSI equivalent for non-unicode
    S1 := AnsiString(Value);
    WriteInt(1); // MBCS string follows
    I1 := Length(S1);
    WriteInt(I1);
    if I1 > 0 then begin
      Move(S1[1],FilePtr^,I1);
      FilePtr := PAnsiChar(integer(FilePtr) + I1);
    end;
  {$ELSE}
    WriteInt(1); // MBCS string follows
  ...
  begin { WriteBuf }
    ...
    FilePtr := PAnsiChar(integer(FilePtr) + Len);
  end;  { WriteBuf }
  ...
  begin { AddingNative }
    integer(pointer(integer(RowHeaderPtr) + (IncludedFieldIdx * 8))^) := integer(FilePtr) - integer(RowHeaderPtr);
  end;  { AddingNative }

  procedure TRvCustomConnection.AddingFormatted;
  begin { AddingFormatted }
    integer(pointer(integer(RowHeaderPtr) + (IncludedFieldIdx * 8) + 4)^) := integer(FilePtr) -
     integer(RowHeaderPtr);
  end;  { AddingFormatted }
  ...
  begin { AllocateSpace }
    ...
    { Copy data from old buffer to new }
      Move(FileBuf^,NewFileBuf^,integer(FilePtr) - integer(FileBuf));

    { Reassign utility pointers }
      FilePtr := PAnsiChar(integer(NewFileBuf) + (integer(FilePtr) - integer(FileBuf)));
      RowHeaderPtr := PAnsiChar(integer(NewFileBuf) + (integer(RowHeaderPtr) - integer(FileBuf)));
  ...
  begin { WriteNullData }
    ...
    if LocalFilterValid and CurrentFieldDef.Included then begin
      ...
      integer(pointer(integer(RowHeaderPtr) + (IncludedFieldIdx * 8))^) := -1;
    end; { if }
    NextField;
  end;  { WriteNullData }
  ...
  begin { WriteStrData }
    ...
    {$IFDEF UNICODE}
      if NativeData <> '' then begin
        Bytes := TEncoding.Unicode.GetBytes(NativeData);
        AddLen := Length(Bytes);
      end else begin
        AddLen := 0;
      end; { else }
    {$ENDIF}
      ...
      {$IFDEF UNICODE}
        Bytes := TEncoding.Unicode.GetBytes(FormatData);
        AddLen := AddLen + Length(Bytes);
        AllocateSpace(Length(FormatData) * SizeOf(Char) + Length(NativeData) * SizeOf(Char) + AddLen + 40);
      {$ELSE}
        AllocateSpace(Length(FormatData) * SizeOf(Char) + Length(NativeData) * SizeOf(Char) + 24);
      {$ENDIF}
        AddingFormatted;
        WriteStr(FormatData);
      end else begin
      {$IFDEF UNICODE}
        AllocateSpace(Length(NativeData) * SizeOf(Char) + AddLen + 20);
      {$ELSE}
        AllocateSpace(Length(NativeData) * SizeOf(Char) + 12);
      {$ENDIF}
      end; { else }
      AddingNative;
      WriteStr(NativeData);
  ...
  begin { WriteIntData }
    if CurrentFieldDef.Included and not LocalFilterAbort(NativeData) then begin
      ...
      if (IntToStr(NativeData) <> FormatData) and (FormatData <> '') then begin
        AllocateSpace(Length(FormatData) * SizeOf(Char) + 4 + SizeOf(NativeData) * SizeOf(Char));
        AddingFormatted;
        WriteStr(FormatData);
      end else begin
        AllocateSpace(SizeOf(NativeData) * SizeOf(Char));
      end; { else }
      AddingNative;
      WriteInt(NativeData);
    end; { if }
  ...
  {$ENDIF}
    Result := '';
    StringType := ReadInt;
    while StringType > 0 do begin
      Len := ReadInt;
      if StringType = 1 then begin // MBCS
      {$IFDEF UNICODE}
        if HaveUnicode then begin
          FilePtr := PAnsiChar(integer(FilePtr) + Len);
        end else begin
          if Len = 0 then begin
            Result := '';
      ...
      {$ELSE}
        ...
        Move(FilePtr^,Result[1],Len);
        Inc(FilePtr,Len);
      {$ENDIF}
      end else if StringType = 2 then begin // Unicode
      {$IFDEF UNICODE}
        if Len = 0 then begin
          Result := '';
        end else begin
      ...
      {$ELSE}
        Inc(FilePtr,Len); // skip data
      {$ENDIF}
      end else begin // Unknown format, skip data
        FilePtr := PAnsiChar(integer(FilePtr) + Len);
      end; { else }
      StringType := ReadInt;
    end; { while }
  ...
  begin { ReadInt }
    ...
    FilePtr := PAnsiChar(integer(FilePtr) + SizeOf(integer));
  end;  { ReadInt }
  ...
  { Free AltFileBuf }
    ...
    FilePtr := PAnsiChar(integer(FileBuf) + 2 * SizeOf(longint)); { Save space for bufsize & mapidx }
  end;  { ClearBuffer }
  ...
  { Write out buffer size and CurrentFileMap }
    SavePtr := FilePtr;
    FilePtr := FileBuf;
    WriteInt(integer(SavePtr) - integer(FileBuf)); { Buffer size }
...
{$ENDIF}
  ...
  { Update data size }
    I1 := integer(FilePtr) - integer(RowHeaderPtr);
    FilePtr := PAnsiChar(integer(RowHeaderPtr) - 4);
    WriteInt(I1);
    FilePtr := PAnsiChar(integer(RowHeaderPtr) + I1);
    FixupBufferHeader;
