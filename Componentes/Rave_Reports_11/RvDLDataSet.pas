...
type
  ...
  public
    procedure ResultSetGetRow(var Params: TDLResultSetRow); override;
  end;
...
const
  DataTypeMap : array[TFieldType] of byte =
    (dt_String,         {ftUnknown}
     ...
     dt_BCD{$IFDEF LEVEL2006}, {ftFMTBcd}
     dt_Blob,           {ftFixedWideChar}
     dt_Blob,           {ftWideMemo}
     dt_Blob,           {ftOraTimeStamp}
     dt_Blob{$IFDEF LEVEL2009}, {ftOraInterval}
     dt_Int64,          {ftLongWord}
     dt_Integer,        {ftShortint}
     dt_Integer,        {ftByte}
     dt_Float,          {ftExtended}
     dt_Blob,           {ftConnection}
     dt_Blob,           {ftParams}
     dt_Blob{$IFDEF LEVEL2010}, {ftStream}
     dt_DateTime,       {ftTimeStampOffset}
     dt_Blob,           {ftObject}
     dt_Float           {ftSingle}

     {$ENDIF}{$ENDIF}{$ENDIF}{$ENDIF}{$ENDIF}); {ftFMTBcd}

{ TDLDataSetDriver }

procedure TDLDataSetDriver.ResultSetGetRow(var Params: TDLResultSetRow);
type
...
begin
  ...
  { copy result set handle into convenient variable }
    ResultSet := TDLDataSetResultSet(GResultSetManager.Get(Params.ResultSetHandle));
...
//!!!!FIX!!!!                  ResultSet.FDataSet.Fields[Index].GetData({$IFDEF CLR}IntPtr({$ENDIF}Stream.Memory{$IFDEF CLR}){$ENDIF});
                end else begin
