...
begin { ProcessDataStrEx }
  ...
  repeat
    DataValue := '';
    if (S1 <> '') and (AnsiChar(S1[1]) in ['''','"']) then begin { Get string }
      ...
      if S2 <> '' then begin
        ...
        if S2 <> '' then begin { Process parent }
          ...
          if RaveCompareText(ParentName,{Trans-}'REPORT') = 0 then begin
          { Add on report variable }
            VarIdx := 0;
            for I1 := 1 to ReportVarNum do begin
              if RaveCompareText(S2,ReportVars[I1]) = 0 then begin
                VarIdx := I1;
                Break;
              end; { if }
            end; { for }
          ...
          end else if RaveCompareText(ParentName,{Trans-}'PIVAR') = 0 then begin
          { Add on data variable }
            DataValue := TRaveProjectManager(Self.Project).BaseReport.PIVar(S2);
          end else if RaveCompareText(ParentName,{Trans-}'PARAM') = 0 then begin
...
begin { CreateFieldName }
  ...
  while I1 <= Length(Result) do begin
    if AnsiChar(Result[I1]) in ['0'..'9','_','a'..'z','A'..'Z'] then begin
      Inc(I1);
...
// Process list of fields
  ...
  // Figure out if this is a new field of not
    ...
    for I2 := 0 to OrigFields.Count - 1 do begin
      ...
      if RaveCompareText(Field.Name,CompName) = 0 then begin
...
// Process lists
  ...
  if Assigned(FieldInfoList) then begin
    for I1 := 0 to FieldInfoList.Count - 1 do begin
      TRaveDataFieldInfo(FieldInfoList[I1]).Free;
    end;
    FreeAndNil(FieldInfoList);
  end;
end; { CreateFields }
...
{ Remove DataViewName. from beginning if it exists }
  S1 := Name + '.';
  if RaveCompareText(S1,Copy(FieldName,1,Length(S1))) = 0 then begin
...
{ Go through all data field components and change the name }
  for I1 := 0 to ChildCount - 1 do begin
    ...
    if RaveCompareText(Field.Name,CreateFieldName(OldName,Field.FieldName)) = 0 then begin
...
begin
  ...
  if DataOpened then begin
    FSaving := false;
    ValueListPtr := nil;
    if Assigned(ValueListHead) then begin
      while Assigned(ValueListHead) do begin
        ValueListPtr := ValueListHead.Next;
        ValueListHead.Free;
        ValueListHead := ValueListPtr;
      end; { while }
    end; { if }
...
begin { AddingNative }
  integer(pointer(RowBuf + (FieldIdx * 8))^) := BufferSize;
end;  { AddingNative }

procedure TRaveDataBuffer.AddingFormatted;
begin { AddingFormatted }
  integer(pointer(RowBuf + (FieldIdx * 8) + 4)^) := BufferSize;
end;  { AddingFormatted }
...
var
  ...
  NewSize: integer;
begin { AllocateSpace }
  if (BufferSize + AddSize) > RowBufSize then begin
  // Allocate new buffer
    NewSize := BufferSize + AddSize + FAddPad;
  ...
  // Clean up
    ...
    RowBufPtr := PChar(integer(NewRowBuf) + (integer(RowBufPtr) - integer(RowBuf)));
    FreeMem(RowBuf,RowBufSize);
    RowBuf := NewRowBuf;
    RowBufSize := NewSize;
  end; { if }
...
begin
  ...
  for I1 := 1 to Length(AValue) do begin
    Result := Result + UpperCase(IntToHex(ord(AValue[I1]), 4));
  end; { for }
end; { StrToHex }
*)
...
procedure TRaveDataBuffer.WriteStrData(FormatData: string; NativeData: string);
begin { WriteStrData }
  // MBCS-SAFE
  if (NativeData <> FormatData) and (FormatData <> '') then begin
    AllocateSpace(Length(FormatData) * SizeOf(Char) + Length(NativeData) * SizeOf(Char) + 8);
    AddingFormatted;
    WriteStr(FormatData);
  end else begin
    AllocateSpace(Length(NativeData) * SizeOf(Char) + 4);
  end; { else }
  AddingNative;
  WriteStr(NativeData);
  Inc(FieldIdx);
