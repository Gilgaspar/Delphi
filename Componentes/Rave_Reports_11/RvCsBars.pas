  ...
  if Assigned(FBarCode) then begin
    case BarCodeJustify of
      pjLeft,pjBlock: begin
        if FBarCode.Left <> Left then begin
          FBarCode.Left := Left;
        end;
      end;
      pjCenter: begin
        if FBarCode.Center <> Center then begin
          FBarCode.Center := Center;
        end;
      end;
      pjRight: begin
        if FBarCode.Right <> Right then begin
          FBarCode.Right := Right;
        end;
      end;
    end;
    FBarCode.Top := Top;
...
var
  SavePos: double;
begin { SetWidth }
  SavePos := FBarCode.Left;
  if FAutoSize then begin
    case BarCodeJustify of
      pjCenter: SavePos := FBarCode.Center;
      pjRight: SavePos := FBarCode.Right;
    end;
    inherited SetWidth(FBarCode.Width);
    case BarCodeJustify of
      pjCenter: Center := SavePos;
      pjRight: Right := SavePos;
    end;
  end else begin { else }
...
begin { Print }
  ...
  case BarCodeRotation of
    rot0: begin
      ...
      if AutoSize then begin
        FBarCode.Left := PageLeft + (FBarCode.Left - FLeft);
      end else begin
        case BarCodeJustify of
          pjLeft,pjBlock: begin
            FBarCode.Left := PageLeft - FLeft + FBarCode.Left;
          end;
          pjCenter: begin
            FBarCode.Center := PageLeft - FLeft + Center;
          end;
          pjRight: begin
            FBarCode.Right := PageLeft - FLeft + Right;
          end;
        end; { case }
      end; { else }
      FBarCode.Top := T1;
  ...
  case BarCodeJustify of
    pjCenter: FBarCode.Center := SavePos;
    pjRight: FBarCode.Right := SavePos;
    else FBarCode.Left := SavePos;
  end;
