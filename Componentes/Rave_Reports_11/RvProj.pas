...
type
  ...
  protected
    ...
    function GetRaveModule(Index: Integer): TRaveModule;
  public
    ...
    property Module[Index: Integer]: TRaveModule read GetRaveModule;
  ...
  public
    ...
    procedure AddForwardRef(AName: string; AOwner: TRaveComponent; AInstance: TObject;
    ...
    function FindRaveComponent(AName: string; DefRoot: TRaveComponent): TRaveComponent;
  ...
  { DataChanged will only change value when set to true, to clear call ClearChanged }
    ...
    property UnicodeEnabled: boolean read FUnicodeEnabled;
...
constructor TRaveForwardRef.Create(AInstance: TObject; APropInfo: PPropInfo; AParam: longint);
begin { Create }
  inherited Create;
  FInstance := AInstance;
  FPropInfo := APropInfo;
...
procedure TRaveProjectManager.AddForwardRef(AName: string; AOwner: TRaveComponent; AInstance: TObject;
 APropInfo: PPropInfo; AParam: longint);
begin { AddForwardRef }
    ...
    repeat
      with TRaveForwardRef(ForwardRefList.Objects[Index]) do begin
        if not Assigned(PropInfo) then begin
          ...
          SetOrdProp(TPersistent(Instance),PropInfo,integer(Target));
        end; { else }
      end; { with }
      ForwardRefList.Objects[Index].Free;
      ForwardRefList.Delete(Index);
    until (Index >= ForwardRefList.Count) or
     (RaveCompareText(ForwardRefList[Index],TargetName) <> 0);
  end; { if }
...
type
  ...
  TRaveControlAccess = class(TRaveControl)
  end; { TRaveComponentAccess }
...
begin { SaveToStreamHelper }
  with StreamHelper do begin
    {$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}(LastActivePage);
  ...
  { Write out list of owned editors }
    ...
    for I1 := 0 to (ChildCount - 1) do begin
      ...
      {$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}(Child[I1].Name);
    end; { for }

  { Write out list of loaded editors }
    WriteIndex(LoadedList.Count);
    for I1 := 0 to (LoadedList.Count - 1) do begin
      {$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}(TRavePage(LoadedList[I1]).Name);
    end; { for }
  ...
  { Add all owned pages if not already in FixupList to fix 3.0D bug }
    for I1 := 0 to ChildCount - 1 do begin
      ...
      for I2 := 0 to FixupList.Count - 1 do begin
        if RaveCompareText(TestChild.Name,FixupList[I2]) = 0 then begin
          NameFound := true;
          Break;
        end; { if }
      end; { for }
    ...
    repeat
      ...
      { Set up new page }
        ...
        if CurrPage.BinCustom <> '' then begin
          ...
          if (RPDev <> nil) and (RPDev.Bins <> nil) then begin
      ...
      TRaveControlAccess(CurrPage).PrintAll((Sender as TBaseReport));
...
var
  ...
  SysModule: TRaveModule;
begin { InternalExecute }
  TRaveProjectManager(Project).PrepareModule;

  SysModule := TRaveProjectManager(Project).ModuleManager.FindModule('SYSFUNC');
  if Assigned(SysModule) then begin
    SysModule.SetIntVar('nil',0);
  end; { if }
...
function TRaveModuleManager.ModuleCount: integer;
begin
...
function TRaveModuleManager.GetRaveModule(Index: Integer): TRaveModule;
begin
  Result := TRaveModule(ModuleList[Index]);
end;
...
begin { SaveModules }
  with StreamHelper do begin
    ...
    for I1 := 0 to ModuleList.Count - 1 do begin
      {$IFDEF LEVEL6}
      Write(TRaveModule(ModuleList[I1]).ModuleName);
      {$ELSE}
    ...
    begin { CheckComponent }
      if Assigned(C1) then begin
        if C1 is TRaveProjectItem then begin
          if not UseMasterCaller(TRaveProjectItem(C1)) and not
           TRaveProjectItem(C1).Included then begin
...
begin { LoadFromStreamHelper }
  with StreamHelper do begin
    ...
    {$ENDIF}
    if string(TestSig) <> Signature then begin
      RaveError(Trans('Invalid Project Format'));
      Exit;
    end; { if }
    {$IFDEF LEVEL6}
    Read(FVersion);
    FUnicodeEnabled := (FVersion >= 80000);
    {$ELSE}
...
begin { SaveToStreamHelper }
  {$IFDEF LEVEL6}
  StreamHelper.Write(AnsiChar(Signature[1]));
  StreamHelper.Write(AnsiChar(Signature[2]));
  StreamHelper.Write(AnsiChar(Signature[3]));
  StreamHelper.Write(AnsiChar(Signature[4]));
  {$ELSE}
  StreamHelper.WriteBuf(Signature[1],4);
  {$ENDIF}
  FVersion := RaveVersion;
  {$IFDEF LEVEL6}
  StreamHelper.Write(FVersion);
  {$ELSE}
  ...
  StreamHelper.{$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}(LastActiveReport);
  with TRaveWriter.Create(StreamHelper) do try
    WriteComponent(self,false);
  finally
    Free;
...
function TRaveProjectManager.FindRaveComponent(AName: string; DefRoot: TRaveComponent): TRaveComponent;
var
  I1: integer;
  I2: integer;
  P1: TRavePage;
...
//TODO: Add capability to process Owner.Owner.Component strings
  ...
  if I1 > 0 then begin
  { Find owner }
    ObjectName := MBCSCopy(AName,1,I1 - 1); { Owner portion }
      ...
      with ActiveReport do begin
        for I2 := 0 to ChildCount - 1 do begin
          if Child[I2] is TRavePage then begin
            ...
            if RaveCompareText(P1.Name,ObjectName) = 0 then begin
              Root := P1;
              Break;
            end; { if }
          end; { if }
  ...
  { Look for owner in ProjectItems (Reports/Global Pages/Data Views) }
    if not Assigned(Root) then begin
      for I2 := 0 to ChildCount - 1 do begin
        ...
        if RaveCompareText(Item.Name,ObjectName) = 0 then begin
          Root := Item;
          Break;
        end; { if }
      end; { for }
  ...
  { Look for owner = to project }
    if not Assigned(Root) then begin
      if RaveCompareText(self.Name, ObjectName) = 0 then begin
        Root := self;
      end; { if }
    end; { if }
  ...
  { Second half is object's name on Root }
    if Assigned(Root) then begin
      ObjectName := MBCSCopy(AName,I1 + 1,Length(AName) - I1);
    end; { if }
  end else begin
  ...
  if not Assigned(Root) then begin
    ...
    if (not Assigned(Result)) and (RaveCompareText(self.Name, ObjectName) = 0) then begin
      Result := self;
    end; { if }
  end; { else }
...
begin { Compile }
  ...
  // Create source for all modules
    PerformAction(self,@ActionBuildSource);
  // Perform Definition only compiles
    PerformAction(self,@ActionDefCompile);
  // Compile Implementation compiles
    PerformAction(self,@ActionImpCompile);
  // Fix up cross module references
    PerformAction(self,@ActionFixUp);
  // Point events to function locations
    PerformAction(self,@ActionEventConnect);
    CompileNeeded := false;
...
begin
  ...
  for i := 0 to ReportList.Count - 1 do begin
    ...
    if (AIsFullName and (RaveCompareText(LReport.FullName, AName) = 0))
     or (RaveCompareText(LReport.Name, AName) = 0) then begin
      Result := LReport;
      Break;
    end;
  end;
      ...
      { Figure out Target component }
        ...
        { Target was not renamed }
          Target := FindRaveComponent(ForwardRefList[I1],
           TRaveComponent(TRaveForwardRef(ForwardRefList.Objects[I1]).Instance).Parent);
