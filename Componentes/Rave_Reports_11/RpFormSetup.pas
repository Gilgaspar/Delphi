...
begin
  if RPDev <> nil then begin
    RpDev.SetupBin := RpDev.DevMode.dmDefaultSource;
    RPDev.PrinterSetupDialog;
    if RPDev.DevModeChanged then begin
      RpDev.IgnoreNDRBin := RpDev.SetupBin <> RpDev.DevMode.dmDefaultSource;
...
begin
  ...
  bbtnOk.Enabled := true;
  SetupBB.Enabled := (ssAllowPrinterSetup in ReportSystem.SystemSetups);
