...
type
  ...
  TBandDetail = class
    ...
    ControllerDetail: TBandDetail; { TBandDetail record of controller }
  end; { TBandDetail }

  TBandSearchRec = class
    Index: integer;
  ...
  public
    ...
    property ReplaceBandStyle: TRaveBandStyle read FReplaceBandStyle write FReplaceBandStyle;
...
var
  I1: integer;
  I2: integer;
  BandDetail: TBandDetail;
  BandDetail2: TBandDetail;
  ColorIdx: integer;
  Final: boolean;

  procedure FindControllerLevel(BandDetail: TBandDetail);
  ...
  { Initialize BandList }
    ...
    { Look for any bands already registered that are controlled by this band }
      for I2 := 0 to (Result.Count - 1) do begin
        BandDetail2 := TBandDetail(Result[I2]);
...
begin { Create }
  ...
  FColumnSpacing := 0.0;
  FCurrentColumn := 1;
  BandSearchRec := TBandSearchRec.Create;
end;  { Create }

destructor TRaveRegion.Destroy;
begin { Destroy }
  FreeAndNil(BandSearchRec);
...
begin { PushBandSearchRec }
  ...
  BandSearchSave.Push(BandSearchRec);
  BandSearchRec := TBandSearchRec.Create;
...
begin { PopBandSearchRec }
  if Assigned(BandSearchSave) then begin
    BandSearchRec.Free;
    BandSearchRec := TBandSearchRec(BandSearchSave.Pop);
  end; { if }
...
{ Paint all children }
  ...
  { Build list of bands }
    ...
    for I1 := 0 to (ChildCount - 1) do begin
      ...
      BandDetail := TBandDetail(BandList[I1]);
...
begin { Habla }
  Result := RaveCompareText(Conversation,{Trans-}'CalcNewData') = 0;
end;  { Habla }
...
begin { Print }
  ...
  Inc(RowCount);
  Active := DataView.CheckBeforeEOF and ((MaxRows = 0) or (RowCount < MaxRows));
end;  { Print }
