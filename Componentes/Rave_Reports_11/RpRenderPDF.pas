...
type
  ...
  {$IFDEF LevelXE2}
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]{$ENDIF}
  TRvRenderPDF = class;
  ...
  protected
    ...
    FFileOffset: longword;
    FLinks: TList;

    property Links: TList read FLinks write FLinks;
    property FileOffset: longword read FFileOffset write FFileOffset;
    ...
    function FormatPdfText(const AValue: string): string;
    procedure ClosePath;
    ...
    procedure AdjustRotationOffset(const ATextWidth: double;
                                   var AX, AY: double);
    procedure AddLink(ax1, ay1, ax2, ay2: double; uri: string);
    procedure InitData; override;
  ...
  protected
    ...
    property AX1: double read FAX1 write FAX1;
    property AY1: double read FAY1 write FAY1;
    property AX2: double read FAX2 write FAX2;
    property AY2: double read FAY2 write FAY2;
  end;
  ...
  protected
    ...
    procedure SetLastChar(AValue: integer);
    ...
    property LastChar: integer read FLastChar write SetLastChar;
  ...
  protected
    ...
    FUnicode: TRpPdfUnicode;
    FCharsUsed: array[0..8192] of byte;

    procedure SetFontData;
    procedure SetCharBits(AValue: string);
    procedure InitData; override;
  ...
  public
    ...
    property Unicode: TRpPdfUnicode read FUnicode write FUnicode;
    property CompositeFontEncoding: string read FCompositeFontEncoding write FCompositeFontEncoding;
    property FirstChar: integer read FFirstChar write FFirstChar;
  end;  { TRPPDFFontComposite }

  TRpPdfUnicode = class(TRpPdfObject)
  protected
    FCompositeFont: TRPPDFFontComposite;
    procedure InitData; override;
  public
    property CompositeFont: TRPPDFFontComposite read FCompositeFont write FCompositeFont;
  end;
  ...
  protected
    FBufferDocument: boolean;
    FCurrentID: integer;
    FXObjectCurrentID: integer;
    FDataStream: TStream;
    ...
    function IsCompositeFont: boolean;
    ...
    property XObjectCurrentID: integer read FXObjectCurrentID write FXObjectCurrentID;
    ...
    procedure PrintBitmapRect(const X1, Y1, X2, Y2: double; AGraphic: TBitmap); override;
    procedure PrintBitmap(const X1,Y1,ScaleX,ScaleY: double; AGraphic: TBitmap); override;
  ...
  public
    ...
    procedure WritePattern(Pattern: TRPPDFPattern);
  ...
  published
    ...
    property EmbedFonts: boolean read FEmbedFonts write FEmbedFonts default false;
    property OnCompress: TCompressEvent read FOnCompress write FOnCompress;
    property ImageQuality: TImageQualityRange read FImageQuality write FImageQuality default 90;
    property MetafileDPI: integer read FMetafileDPI write FMetafileDPI default 300;
    property FontEncoding: TPDFFontEncoding read FFontEncoding write FFontEncoding default feWinAnsiEncoding;
    property DocInfo: TRPPDFDocInfo read FDocInfo write FDocInfo;
    property BufferDocument: boolean read FBufferDocument write FBufferDocument default true;
    property DisableHyperlinks: boolean read FDisableHyperlinks write FDisableHyperlinks default false;
  end;
  {$IFDEF LEVEL6}{$WARN SYMBOL_DEPRECATED DEFAULT}{$ENDIF}
...
begin
  inherited;
  FCreator := 'Rave Reports (http://www.nevrona.com/rave)';
...
begin
  inherited;
  FDisableHyperlinks := false;
  FBufferDocument := true;
  FCurrentID := 0;
  FXObjectCurrentID := 0;
  ...
  FFontBitmap.Canvas.Font.PixelsPerInch := 3000;
...
var
  i1: integer;
begin
  if not BufferDocument then begin
    WriteXObjects;

    if Assigned(CurrentPage.Links) then begin
      for i1 := 0 to CurrentPage.Links.Count - 1 do begin
        RegisterObject;
        TPdfLink(CurrentPage.Links[i1]).InitData;
        DataStream.CopyFrom(TPdfLink(CurrentPage.Links[i1]).DataStream,
                            TPdfLink(CurrentPage.Links[i1]).DataStream.Size);
      end;
    end; { if }

    RegisterObject;
    CurrentPage.FileOffset := DataStream.Position;
    CurrentPage.InitData;
    DataStream.CopyFrom(CurrentPage.DataStream, CurrentPage.DataStream.Size);
    RegisterObject;
    with CurrentPage do begin
      PageContent.InitData;
      RvRenderPDF.DataStream.CopyFrom(PageContent.DataStream, PageContent.DataStream.Size);
    end; { with }
    ClearXObjects;
  end; { if }
end;
...
begin
  if (Converter.Pen.Style <> psClear) or
     (Converter.Brush.Style <> bsClear) then begin
...
var
  ...
  BrushColor: TColor;
begin
  if (Converter.Pen.Style <> psClear) then begin
    PenColor := Converter.Pen.Color;
    BrushColor := Converter.Brush.Color;
...
begin
  ...
  if not BufferDocument then begin
    if FOutputStream = nil then begin
      FDataStream := TFileStream.Create(OutputFileName,fmCreate or fmShareDenyWrite);
    end else begin
      FDataStream := FOutputStream;
    end;
...
{$ELSE}  
  GetLocaleFormatSettings(USEnglish, FormatSettings);
{$ENDIF}
  FormatSettings.ShortDateFormat := 'yyyy/mm/dd';
  FormatSettings.LongDateFormat := 'hh:mm:ss';
  s1 := DateTimeToStr(Value, FormatSettings);
{$ELSE}
...
procedure TRvRenderPDF.PrintBitmapRect(const X1, Y1, X2, Y2: double; AGraphic: TBitmap);
var
  JPGImage: TJPEGImage;
  ImageName: string;
...
procedure TRvRenderPDF.PrintBitmap(const X1,Y1,ScaleX,ScaleY: double; AGraphic: TBitmap);
var
  JPGImage: TJPEGImage;
  ImageName: string;
  X2, Y2: double;
...
begin
  if FFontChanged then begin
    ...
    FFontSize := Converter.FontData.Size;
...
begin
  if AReuseJPG then begin
    ...
    if BufferDocument then begin
      ...
      Inc(FXObjectCurrentId);
      Result := FormatEx('G%d',[XObjectCurrentId]);
    end; { else }
...
function TRvRenderPDF.IsCompositeFont: boolean;
begin
// need to change code to check for all composite fonts, not just CJK fonts
  ...
  //!!! Temporary
  if Converter.Font.Name = 'Arial Unicode MS' then begin
    Result := true;
  end; { if }
end;
...
begin
  if PatternList.IndexOf(APatternName) < 0 then begin
    CreatePattern(Converter.Brush.Style, Converter.Brush.Color);
  end; { if }
  Result := TRPPDFPattern(PatternList.Objects[PatternList.IndexOf(APatternName)]);
end;  { GetPattern}
...
begin { SetIDs }
  ...
  for i1 := 0 to PageList.Count - 1 do begin
   ...
   if Assigned(PdfPage.Links) then begin
     for i2 := 0 to PdfPage.Links.Count - 1 do begin
       Inc(CurrentID);
       TPdfLink(PdfPage.Links[i2]).SetID(CurrentID);
     end; { for }
   end; { if }
  end; { for }
  Inc(CurrentID);
...
begin
  ...
  with CurrentPage do begin
    if DocInfo.Title <> '' then begin
      PrintLnF('/Title (%s)',[FormatPdfText(DocInfo.Title)]);
    end; { if }
    if DocInfo.Author <> '' then begin
      PrintLnF('/Author (%s)',[FormatPdfText(DocInfo.Author)]);
    end; { if }
    if DocInfo.Subject <> '' then begin
      PrintLnF('/Subject (%s)',[FormatPdfText(DocInfo.Subject)]);
    end; { if }
    if DocInfo.Keywords <> '' then begin
      PrintLnF('/Keywords (%s)',[FormatPdfText(DocInfo.Keywords)]);
    end; { if }
    if DocInfo.Creator <> '' then begin
      PrintLnF('/Creator (%s)',[FormatPdfText(DocInfo.Creator)]);
    end; { if }
    if DocInfo.Producer <> '' then begin
      PrintLnF('/Producer (%s)',[FormatPdfText(DocInfo.Producer)]);
    end; { if }
...
begin
  for i1 := 0 to PageList.Count - 1 do begin
    ...
    if Assigned(PageObject.Links) then begin
      for i2 := 0 to PageObject.Links.Count - 1 do begin
        RegisterObject;
        TPdfLink(PageObject.Links[i2]).InitData;
        DataStream.CopyFrom(TPdfLink(PageObject.Links[i2]).DataStream, TPdfLink(PageObject.Links[i2]).DataStream.Size);
      end;
    end; { if }
  end; { for }
end;
...
procedure TRvRenderPDF.WriteFont;
var
...
begin { WriteFont }
  ...
  if FontObject is TRPPDFFontComposite then begin
    ...
    RegisterObject;
    CompositeObject.Unicode.InitData;
    DataStream.CopyFrom(CompositeObject.Unicode.DataStream, CompositeObject.Unicode.DataStream.Size);
  end; { if }
end;  { WriteFont }
...
procedure TRvRenderPDF.WritePattern(Pattern: TRPPDFPattern);
var
  SaveStream: TStream;
begin { WritePattern }
  SaveStream := ActiveStream;
  RegisterObject;
  Pattern.InitData;
  DataStream.CopyFrom(Pattern.DataStream, Pattern.DataStream.Size);
  RegisterObject;
  Pattern.PatternResource.InitData;
  DataStream.CopyFrom(Pattern.PatternResource.DataStream, Pattern.PatternResource.DataStream.Size);
  ActiveStream := SaveStream;
end;  { WritePattern }
...
var
  OrigSeparator: char;
begin
  OrigSeparator := {$IFDEF LevelXE}FormatSettings.{$ENDIF}DecimalSeparator;
  {$IFDEF LevelXE}FormatSettings.{$ENDIF}DecimalSeparator := '.';
  Result := Format(AData, AArgs);
  {$IFDEF LevelXE}FormatSettings.{$ENDIF}DecimalSeparator := OrigSeparator;
end;
...
var
  SpaceWidth: double;
begin
{ Print a block of spaces to fill the front space width for block justification}
...
begin
  if (Converter.Pen.Style <> psClear) or
    ...
    PenColor := Converter.Pen.Color;
    BrushColor := Converter.Brush.Color;
    if Converter.Brush.Style = bsClear then begin
      PDFArc(AX1, AY1, AX2, AY2, AX3, AY3, AX4, AY4, false, true);
      PrintLn('b');
    end else begin
...
procedure TRvRenderPDF.ClearXObjects;
var
  i1: integer;
begin
  for i1 := 0 to XObjectList.Count - 1 do begin
...
begin
  ...
  for I1 := 1 to Length(AValue) do begin
    Result := Result + UpperCase(IntToHex(ord(AValue[I1]), 2));
  end; { for }
end; { StrToHex }
...
begin
  ...
  if not RvRenderPdf.BufferDocument then begin
    RvRenderPdf.CurrentID := 1;
    SetId(RvRenderPdf.FCurrentID);
  end; { else }
...
begin
  for i1 := 0 to RvRenderPDF.PageList.Count - 1 do begin
    ...
    if ((i1 mod 25) = 0) and (i1 > 0) then begin
      Result := Result + #13#10;
    end;
  end; { for }
...
var
  ...
  procedure WritePagesToCatalog;
  begin
    RvRenderPDF.ActiveStream := RvRenderPdf.DataStream;
    RvRenderPDF.ActiveStream.Position := 72;
    RvRenderPDF.PrintF('%d',[id]);
    RvRenderPDF.ActiveStream.Position := RvRenderPdf.ActiveStream.Size;
  end;
  procedure WritePagesToParent;
  var
    I1: integer;
    Page: TRpPdfPage;
  begin
    RvRenderPDF.ActiveStream := RvRenderPdf.DataStream;
    for I1 := 0 to RvRenderPdf.PageList.Count - 1 do begin
      Page := TRPPdfPage(RvRenderPDF.PageList.Objects[i1]);
      RvRenderPDF.ActiveStream.Position := Page.FileOffset + 33 + longword(Length(IntToStr(Page.GetID)));
      RvRenderPDF.PrintF('%d',[id]);
    end;
    RvRenderPDF.ActiveStream.Position := RvRenderPdf.ActiveStream.Size;
  end;
...
begin
  if not RvRenderPdf.BufferDocument then begin
    RvRenderPDF.CurrentID := RvRenderPDF.CurrentID + 1;
    SetID(RvRenderPDF.CurrentID);
    id := GetID;
...
var
  ...
  SaveStream: TStream;
begin
  SaveStream := RvRenderPDF.ActiveStream;
  RvRenderPDF.ActiveStream := DataStream;
  if not RvRenderPdf.BufferDocument then begin
    RvRenderPDF.CurrentID := RvRenderPDF.CurrentID + 1;
    SetID(RvRenderPDF.CurrentID);
...
*)
  ...
  RvRenderPDF.ActiveStream := SaveStream;
end;
...
begin
  ...
  if Assigned(FLinks) then begin
    for i1 := 0 to FLinks.Count - 1 do begin
      TPdfLink(Links[i1]).Free;
    end;
  end; { if }
  FreeAndNil(FLinks);
end;
...
function TRPPDFPage.FormatPdfText(const AValue: string): string;
var
  s1: string;
  i1: integer;
begin
...
procedure TRPPDFPage.AddLink(ax1, ay1, ax2, ay2: double; uri: string);
var
  PdfLink: TPdfLink;
begin
  if RvRenderPdf.DisableHyperlinks then Exit;
  if not Assigned(Links) then begin
    FLinks := TList.Create;
  end;
  PdfLink := TPdfLink.Create(RvRenderPDF);
  PdfLink.AX1 := ax1;
  PdfLink.AY1 := ay1;
  PdfLink.AX2 := ax2;
  PdfLink.AY2 := ay2;
...
begin
  ...
  if RvRenderPdf.BufferDocument then begin
    ...
    RvRenderPDF.PrintLn('/Parent        0 R');
  end; { else }
  ...
  RvRenderPDF.PrintLn('>>');
  RvRenderPDF.PrintLnF('/MediaBox [0 0 %.0f %.0f]',[PageWidth * DPI_MULTIPLIER,PageHeight * DPI_MULTIPLIER]);
  RvRenderPDF.PrintLnF('/Contents %d 0 R',[id + 1]);
  if Assigned(Links) and (Links.Count > 0) then begin
    RvRenderPDF.PrintLn('/Annots [');
    for i1 := 0 to Links.Count - 1 do begin
      PdfLink := TPdfLink(Links[i1]);
      RvRenderPDF.PrintLnF('%d 0 R',[PdfLink.GetID]);
    end; { for }
    RvRenderPDF.PrintLn(']');
  end; { if }
  RvRenderPDF.PrintLn('>>');
...
procedure TRPPDFPage.PrintJPG(const AX1, AY1, AX2, AY2: double;
  AName: string);
begin
  if (RvRenderPdf.Converter.Hyperlink <> '') then begin
    AddLink(AX1, AY1, AX2, AY2, RvRenderPdf.Converter.Hyperlink);
  end; // if
  RvRenderPDF.ActiveStream := PageContent.PageStream;
  with RvRenderPDF do begin
    PrintLn('q');
...
begin
  ...
  RvRenderPDF.ActiveStream := PageContent.PageStream;
  CurrentFont := RvRenderPDF.CurrentFont;
  if CurrentFont is TRPPDFFontComposite then begin
    TRPPDFFontComposite(CurrentFont).SetCharBits(AText);
  end;
  if LowerCase(Copy(AText, 1, 7)) = 'http://' then begin
    AddLink(ax,
            ay + (RvRenderPdf.Converter.LineHeight * LineOfs),
            ax + CurrentFont.GetTextWidth(RvRenderPdf.FontSize, AText),
            ay - RvRenderPdf.Converter.LineHeight + (RvRenderPdf.Converter.LineHeight * (LineOfs * 2)),
            AText);
  end;
  if (RvRenderPdf.Converter.Hyperlink <> '') then begin
    AddLink(ax,
            ...
            ax + CurrentFont.GetTextWidth(RvRenderPdf.FontSize, AText),
            ay - RvRenderPdf.Converter.LineHeight + (RvRenderPdf.Converter.LineHeight * (LineOfs * 2)),
            RvRenderPdf.Converter.Hyperlink);
  end; // if
  x1 := AX;
  y1 := AY;
  with RvRenderPDF do begin
...
//    PrintCharSpacing;
//    PrintWordSpacing;
    PrintLnF('/%s %.2f Tf',[FontObjectName,RvRenderPDF.FontSize]);

    if Converter.FontData.Superscript then begin
      y1 := y1 - (Converter.LineHeight / 2.0);
    end else if Converter.FontData.Subscript then begin
      y1 := y1 + (Converter.LineHeight / 2.0);
    end;

    PrintLnF('%.4f %.4f Td',[InchesToUnitsX(x1), InchesToUnitsY(y1)]);
    ...
    if FontRotation <> 0 then begin
      ...
      PrintLnF('%.4f %.4f %.4f %.4f %.4f %.4f Tm',[
              ...
              InchesToUnitsX(x1),InchesToUnitsY(y1)]);
    end; { if }

    RvRenderPDF.PrintLnF('(%s)Tj',[FormatPdfText(AText)]);
  ...
  if (RvRenderPDF.FontRotation = 0)
    ...
    SavePenColor := RvRenderPdf.PenColor;
    x2 := x1 + CurrentFont.GetTextWidth(RvRenderPDF.FontSize, AText);
    if RvRenderPDF.UnderlineFont then begin
      LineWidth := (RvRenderPDF.FontSize + 1) * 0.0014;
      RvRenderPDF.PenWidth := LineWidth;
      RvRenderPdf.PenColor := RvRenderPdf.TextColor;
      y2 := y1 + (LineWidth * 1.5);
      MoveTo(x1, y2);
      LineTo(x2, y2, true);
    end; { if }
    if RvRenderPDF.StrikeoutFont then begin
      ...
      RvRenderPDF.PenWidth := LineWidth;
      RvRenderPdf.PenColor := RvRenderPdf.TextColor;
      y2 := y1 - (RvRenderPDF.FontSize / DPI_MULTIPLIER) * 0.22;
      MoveTo(x1, y2);
      LineTo(x2, y2, true);
    end; { if }
    RvRenderPDF.PenWidth := SaveLineWidth;
    RvRenderPdf.PenColor := SavePenColor;
  end; { if }
  RvRenderPdf.ActiveStream := SaveStream;
end;
...
//!!!CJK
  ...
  begin
    ...
    end else if Descriptor.FontName = 'Arial Unicode MS' then begin
      RvRenderPDF.PrintLnF('%d [', [3]);
    end else begin
...
begin { InitData }
  ...
  RvRenderPDF.PrintLnF('/DW %d',[FontWidths[FirstChar]]);
...
begin
  if not RvRenderPdf.BufferDocument then begin
    ...
    RvRenderPDF.CurrentID := RvRenderPDF.CurrentID + 1;
    Descriptor.SetID(RvRenderPDF.CurrentID);
  end; { if }
...
var
  ...
  begin
    ...
    while i1 < LastChar - FirstChar + 1 do begin
      FontWidths[i1 + FirstChar] := (lpABC[i1].abcA + word(lpABC[i1].abcB) + lpABC[i1].abcC) * 2;
      Inc(i1);
    end; // while
  end;
...
begin
  ...
  nSize := GetOutlineTextMetrics((RvRenderPDF.FontBitmap.Canvas.Handle),
                      sizeof(_OUTLINETEXTMETRIC),
                      nil);
  GetMem(OutlineTextMetric, nSize);
  try
      retVal := GetOutlineTextMetrics((RvRenderPDF.FontBitmap.Canvas.Handle),
                          nSize,
                          OutlineTextMetric);
...
begin
  ...
  FUnicode := TRpPdfUnicode.Create(AOwner);
  FUnicode.CompositeFont := self;
end;
...
//    PrintLn('/Encoding /' + CompositeFontEncoding);
    ...
    RvRenderPDF.PrintLnF('/ToUnicode %d 0 R',[Unicode.GetID]);
    PrintLn('>>');
    PrintLn('endobj');
    PrintLn;
    ActiveStream.Position := 0;
...
begin
  if RvRenderPDF.IsCompositeFont then begin
    RvRenderPDF.FontBitmap.Canvas.Font.Name := FontName;
    RvRenderPDF.FontBitmap.Canvas.Font.Style := FontStyles;
    RvRenderPDF.FontBitmap.Canvas.Font.Charset := FontCharset;
...
//debug
  Descendant.Supplement := 0;
        ...
        FirstChar := cardinal(OutlineTextMetric.otmTextMetrics.tmFirstChar) - 1;
        LastChar := cardinal(OutlineTextMetric.otmTextMetrics.tmLastChar);
        if GetCharABCWidths((RvRenderPDF.FontBitmap.Canvas.Handle),
                            FirstChar,
                            LastChar,
...
procedure TRPPDFFontComposite.SetCharBits(AValue: string);
const
...
begin
  for I1 := 1 to Length(AValue) do begin
    ByteValue := Ord(AValue[I1]);
    ByteOfs := ByteValue div 8;
    FCharsUsed[ByteOfs] := FCharsUsed[ByteOfs] or Bits[ByteValue mod 8];
  end; { for }
end;
...
begin
  ...
  with RvRenderPDF do begin
    PrintLnF('%d 0 obj',[GetID]);
    PrintLn('<<');
    PrintLnF('/A << /S /URI /URI (%s) >>', [URI]);
    PrintLn('/Type /Annot');
    PrintLn('/Subtype /Link');
    PrintLnF('/Rect [%.4f %.4f %.4f %.4f]',[
                             RvRenderPdf.CurrentPage.InchesToUnitsX(ax1),
                             RvRenderPdf.CurrentPage.InchesToUnitsY(ay1),
                             RvRenderPdf.CurrentPage.InchesToUnitsX(ax2),
                             RvRenderPdf.CurrentPage.InchesToUnitsY(ay2)
                             ]);
    PrintLn('/Border [16 16 0]');
...
begin
  if not RvRenderPdf.BufferDocument then begin
    ...
    RvRenderPDF.CurrentID := RvRenderPDF.CurrentID + 1;
    self.PatternResource.SetID(RvRenderPDF.CurrentID);
  end; { if }
...
procedure TRPPDFFont.SetLastChar(AValue: integer);
begin
  if AValue > 255 then begin
    FLastChar := 255;
  end else begin
    FLastChar := AValue;
  end; { else }
...
begin
  ...
  Result := (Result * 2 / 1000) * AFontSize / DPI_MULTIPLIER;
end;
...
procedure TRpPdfUnicode.InitData;
//!!!
  ...
  with RvRenderPDF do begin
    ...
    if UseCompression and Assigned(FOnCompress) then begin
      ...
      try
        ...
        CompressMethod := '[/FlateDecode /DCTDecode ]';
        Stream.Position := 0;
        FOnCompress(Stream,ImageStream,CompressMethod);
      finally
        Stream.Free;
      end;
    end else begin
    ...
    PrintLnF('/ColorSpace /DeviceRGB /Length %d /Filter %s',[ImageStream.Size,CompressMethod]);
    PrintLn('>>');
    PrintLn('stream');
