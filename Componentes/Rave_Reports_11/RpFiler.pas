...
//    JobPages: integer;
  ...
  { Stream output routines }
    ...
    procedure WriteHyperlink; override;
  ...
  { Abstract Output Methods - protected }
    procedure PrintCenterInch(Text: string; xPos: double); override;
    procedure PrintLeftInch(Text: string; xPos: double); override;
    procedure PrintRightInch(Text: string; xPos: double); override;
    procedure PrintBlockInch(Text: string; xPos, Width: double); override;
...
{ Stream output routines }
  ...
  begin { WriteText }
    ...
    if Len < 192 then begin { Write out all strings < 192 chars as one byte }
      ...
      TempLen := Len + $C000;
      WriteByte(byte(TempLen div 256));
      WriteByte(byte(TempLen mod 256));
    end; { else }
    if Len > 0 then begin
      WriteBuf(Value[1],{$IFDEF Unicode}2 * {$ENDIF}Len);
    end; { if }
  end;  { WriteText }
  ...
  procedure TRvNDRWriter.WriteHyperlink;
  begin { WriteHyperlink }
    if HoldOutput then Exit;

    WriteByte(rpcSetHyperlink);
    WriteText(Hyperlink);
  end;  { WriteHyperlink }
  ...
  { Page Width,Height }
    ...
    if DevMode <> nil then begin
      WriteInteger(DevMode.dmDefaultSource); // write bin
    end else begin
      WriteInteger(DM_DEFAULTSOURCE);
    end; { else }
...
{ Output routines }
  ...
  procedure TRvNDRWriter.PrintCenterInch(Text: string; xPos: double);
  var
    Width: double;
    NewPos: double;
  ...
  begin { PrintLeftInch }
    ...
    if (FAccuracyMethod = amPositioning) and (BaseFont.Rotation = 0) and
     (Pos(#253 + {Trans-}'$RPC',Text) = 0) and (Pos(RPTFPrefix,Text) = 0) and
     (Pos(' ',Text) <> 0) then begin
      PrintBlockInch(Text,xPos,XU2I(TextWidth(Text)) * 10000.0 / (FScaleX * FScaleX));
    end else begin
  ...
  procedure TRvNDRWriter.PrintRightInch(Text: string; xPos: double);
  var
    Width: double;
    NewPos: double;
  ...
  begin { PrintBlockInch }
    ...
    WriteFloat((FCursorYPos + FOriginY) * FScaleY / 100.0);
    WriteText(Text);
    WriteFloat((xPos + FOriginX) * FScaleY / 100.0);
    WriteFloat(Width * FScaleX / 100.0);

    FCursorXPos := xPos * FScaleX / 100.0;
    // AdjustXY is looking for units
    AdjustXY(XI2U(Width * FScaleX / 100.0));
...
{$ENDIF}
  ...
  { Initialize output stream }
    try
      case FStreamMode of
        ...
        smTempFile: begin
          S1 := {Trans-}'RV'#0;
          SetLength(TempPath, 128);
          SetLength(TempPath, GetTempPath(127,@TempPath[1]));
          SetLength(S2, 145);
          GetTempFileName(@TempPath[1],@S1[1],0,@S2[1]);
          FOutputFileName := StrPas(pchar(@S2[1]));
  ...
  { Add VarList item with pointer to buffer where data is supposed to go }
    ...
    if Defined then begin
      VarList.AddObject(VarName,TObject(-longint(OutputStream.Size))); // Defined=negative
      WriteByte(Length(VarData)); { Max buffer size }
      WriteByte(Length(VarData)); { Buffer length }
      if VarData <> '' then begin
        WriteBuf(VarData[1],{$IFDEF Unicode}2 * {$ENDIF}Length(VarData));
      end; { if }
    end else begin
      VarList.AddObject(VarName,TObject(longint(OutputStream.Size))); // Undefined=positive
  ...
  { Look for existing VarList item }
    ...
    { Set VarData:Defined }
      ...
      if B1 > 0 then begin
        VarValue := Copy(VarValue,1,B1);
        StreamWriteBuffer(OutputStream,VarValue);
      end; { if }
      OutputStream.Position := SavePos;
      VarList.Delete(VarIdx);
    end; { else }
