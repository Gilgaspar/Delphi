...
type
  ...
  protected
    ...
    function GetParam(Index: integer): TFuncParam;
  ...
  public
    ...
    function AddParam(AID: TIdentifier;
                      AParamType: TParamType): TFuncParam;
    function EqualTo(FuncType: TFuncType): boolean;

    property Param[Index: integer]: TFuncParam read GetParam;
...
{ class TBaseType }
  ...
  begin { CreateFromStreamHelper }
    inherited Create;
    {$IFDEF LEVEL6}
    FTypeStruc := TTypeStruc(StreamHelper.ReadByte);
    StreamHelper.Read(FSize);
    FTypeKind := TTypeKind(StreamHelper.ReadByte);
    {$ELSE}
  ...
  begin { SaveToStreamHelper }
    {$IFDEF LEVEL6}
    StreamHelper.Write(byte(FTypeStruc));
    StreamHelper.Write(FSize);
    StreamHelper.Write(byte(FTypeKind));
    {$ELSE}
...
{ class TSimpleType }
  ...
  begin { CreateFromStreamHelper }
    ...
    {$IFDEF LEVEL6}
    StreamHelper.Read(FLow);
    StreamHelper.Read(FHigh);
    {$ELSE}
  ...
  begin { SaveToStreamHelper }
    ...
    {$IFDEF LEVEL6}
    StreamHelper.Write(FLow);
    StreamHelper.Write(FHigh);
    {$ELSE}
...
{ class TArrayType }
  ...
  begin { GetDimType }
    if (Index < 0) or (Index >= Dims) then begin
      ...
      Result := TRange(FDim[Index]).BaseType;
    end; { else }
  end;  { GetDimType }
...
{ class TMethodMember }
  ...
  begin { CreateFromStreamHelper }
    inherited CreateFromStreamHelper(TypeHelper,StreamHelper,Structure);
    FOptions := [];
    StreamHelper.ReadByte;
    //StreamHelper.Read(FOptions);
  end;  { CreateFromStreamHelper }
  ...
  begin { SaveToStreamHelper }
    inherited SaveToStreamHelper(TypeHelper,StreamHelper);
    StreamHelper.WriteByte(0);
    //StreamHelper.Write(FOptions);
  end;  { SaveToStreamHelper }
  ...
  begin { CreateFromStreamHelper }
    ...
    with StreamHelper do begin
      ...
      for I1 := 1 to I2 do begin
        ID := ReadString;
        SavePos;
        MemberType := TMemberType(ReadByte);
        RestorePos;
  ...
  begin { SaveToStreamHelper }
    ...
    with StreamHelper do begin
      ...
      for I1 := 0 to (FTypeList.Count - 1) do begin
        {$IFDEF LEVEL6}
        Write(FTypeList.Strings[I1]);
        {$ELSE}
        WriteString(FTypeList.Strings[I1]);
        {$ENDIF}
        TStructureMember(FTypeList.Objects[I1]).SaveToStreamHelper(TypeHelper,StreamHelper);
      end; { for }
    end; { with }
  end;  { SaveToStreamHelper }
...
{ class TFuncType }
  ...
  function TFuncType.GetParam(Index: integer): TFuncParam;

  begin { GetParam }
    if (Index >= 0) and (Index < Params) then begin
      Result := TFuncParam(ParamList[Index]);
    end else begin
      Result := nil;
    end; { else }
  end;  { GetParam }
