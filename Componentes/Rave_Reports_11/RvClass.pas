...
{!!PORT!! HRgn not defined in CLX}
  ...
  protected
    ...
    FDesLeft: TRaveUnits;
    FDesTop: TRaveUnits;
    FDesWidth: TRaveUnits;
    FDesHeight: TRaveUnits;
    FAdjLeft: TRaveUnits;
    FAdjTop: TRaveUnits;
  ...
  public
    ...
    procedure SetPos(ALeft: TRaveUnits; ATop: TRaveUnits);
    procedure SetSize(P1: TRavePoint; P2: TRavePoint); virtual;
    procedure Resize; virtual;
    function AllowPrint: boolean; override;
    function HeightToPrint: TRaveUnits; virtual;
    procedure OverrideProp(PropName: string);
    procedure RevertProp(PropName: string);
    function PropOverridden(PropName: string): boolean;
    function AlwaysOverride: string;
  ...
  published
    ...
    property MirrorOverride: string read FMirrorOverride write FMirrorOverride;
  end; { TRaveControl }
...
{ Filer classes }
  ...
  public
    constructor CreateFromStream(Stream: TStream; AOwner: TRaveProjectItem; AProjectItem: TRaveProjectItem); virtual;
    constructor CreateFromFile(FileName: string; AOwner: TRaveProjectItem); virtual;
  ...
  procedure RegisterRavePropertyAlias(const ComponentClass: TRaveComponentClass;
                                      const PropertyName: string;
                                      const AliasName: string);
  function FindRavePropertyAlias(const ComponentClass: TRaveComponentClass;
                                 const PropertyName: string): string;
  procedure SetPropDesc(const CompClass: TRaveComponentClass;
                        const PropName: string;
                        const PropDesc: string);
  function PropertyVisible(ComponentClass: TClass;
  ...
  procedure PrintAllCaller(Component: TRaveComponent; Report: TBaseReport);
  procedure PagePaintCaller(Page: TRavePage; Canvas: TCanvas);
  function UseMasterCaller(Component: TRaveComponent): boolean;
  procedure SaveDesignedCaller(Component: TRaveComponent);
  procedure RestoreDesignedCaller(Component: TRaveComponent);
  procedure InitDataCaller(Component: TRaveComponent);
  procedure SetDepthCaller(Parent: TRaveComponent; Control: TRaveComponent; Adjustment: integer);
  procedure SaveAdjustedCaller(RaveControl: TRaveControl);
  procedure RestoreAdjustedCaller(RaveControl: TRaveControl);
...
(**)procedure(ProjectItem:TRaveProjectItem)=nil;RegisterRaveDataObjectProc:procedure(const
...
(**)PropertyVisibleProc:function(ComponentClass:TClass;PropertyName:string):boolean=nil;
(**)RegisterRavePropertyAliasProc:procedure(const ComponentClass:TRaveComponentClass;
(**)const PropertyName:string;const AliasName:string)=nil;FindRavePropertyAliasProc:
(**)function(const ComponentClass:TRaveComponentClass;const PropertyName:string):string=nil;
(*****************************************************************************}
( These variables are used internally by RAVE and should not be modified
(*****************************************************************************)
...
type
  TRaveModuleClassReg = class
  public
    ModuleName: string; { Module that class is defined in }
  end; { TRaveModuleClassReg }

var
  RaveModuleClassList: TStringList = nil; { List of TRaveModuleClassReg }

procedure SetDepthCaller(Parent: TRaveComponent; Control: TRaveComponent; Adjustment: integer);
begin
  Parent.SetDepth(Control,Adjustment);
end;

procedure SaveAdjustedCaller(RaveControl: TRaveControl);
begin
  RaveControl.SaveAdjusted;
end;

procedure RestoreAdjustedCaller(RaveControl: TRaveControl);
begin
  RaveControl.RestoreAdjusted;
end;
...
begin { CallRegisters }
  if Assigned(RegisterProcList) then begin
    for I1 := 0 to RegisterProcList.Count - 1 do begin
      if (AKind = '*') or (RaveCompareText(AKind,RegisterProcList[I1]) = 0) then begin
...
procedure PrintAllCaller(Component: TRaveComponent; Report: TBaseReport);
begin { PrintAllCaller }
  Component.PrintAll(Report);
end;  { PrintAllCaller }

procedure PagePaintCaller(Page: TRavePage; Canvas: TCanvas);
begin { PagePaintCaller }
  Page.Paint(Canvas);
end;  { PagePaintCaller }

function UseMasterCaller(Component: TRaveComponent): boolean;
begin { UseMasterCaller }
  Result := Component.UseMaster;
end;  { UseMasterCaller }

procedure SaveDesignedCaller(Component: TRaveComponent);
begin { SaveDesignedCaller }
  Component.SaveDesigned;
end;  { SaveDesignedCaller }

procedure RestoreDesignedCaller(Component: TRaveComponent);
begin { RestoreDesignedCaller }
  Component.RestoreDesigned;
end;  { RestoreDesignedCaller }

procedure InitDataCaller(Component: TRaveComponent);
begin { InitDataCaller }
  Component.InitData;
end;  { InitDataCaller }
...
procedure RegisterRavePropertyAlias(const ComponentClass: TRaveComponentClass;
                                    const PropertyName: string;
                                    const AliasName: string);
begin { RegisterRavePropertyAlias }
  if Assigned(RegisterRavePropertyAliasProc) then begin
    RegisterRavePropertyAliasProc(ComponentClass,PropertyName,AliasName);
  end; { else }
end;  { RegisterRavePropertyAlias }

function FindRavePropertyAlias(const ComponentClass: TRaveComponentClass;
                               const PropertyName: string): string;
begin { FindRavePropertyAlias }
  if Assigned(FindRavePropertyAliasProc) then begin
    Result := FindRavePropertyAliasProc(ComponentClass,PropertyName);
  end else begin
    Result := PropertyName;
  end; { else }
end;  { FindRavePropertyAlias }
...
function PropertyVisible(ComponentClass: TClass;
                         PropertyName: string): boolean;
begin { PropertyVisible }
  if Assigned(PropertyVisibleProc) then begin
    Result := PropertyVisibleProc(ComponentClass,PropertyName);
  end else begin
    Result := false;
  end; { else }
end;  { PropertyVisible }
...
var
  I1: integer;
  RaveModuleClassReg: TRaveModuleClassReg;
begin { RegisterRaveModuleClasses }
  if not Assigned(RaveModuleClassList) then begin
    RaveModuleClassList := TStringList.Create;
    RaveModuleClassList.Sorted := true;
  end; { if }

  for I1 := 0 to High(RaveClasses) do begin
    if FindRaveClassModule(RaveClasses[I1].ClassName) = '' then begin
      RaveModuleClassReg := TRaveModuleClassReg.Create;
      RaveModuleClassReg.ModuleName := ModuleName;
      RaveModuleClassList.AddObject(RaveClasses[I1].ClassName,RaveModuleClassReg);
    end; { if }
  end; { for }
...
begin { FindRaveClassModule }
  if Assigned(RaveModuleClassList) and RaveModuleClassList.Find(ClassName,I1) then begin
    Result := TRaveModuleClassReg(RaveModuleClassList.Objects[I1]).ModuleName;
  end else begin
    Result := '';
  end; { else }
...
begin { RaveCompileStatus }
  ...
  end else if Assigned(CompileStatus) then begin
    RaveError(CompileStatus.ErrorMsg);
  end; { else }
...
begin { AssignTo }
  ...
  if Count > 0 then begin
    ...
    try
      ...
      // Don't assign special properties
        if FoundString(UpperCase(string(PropInfo.Name)), AlwaysOverride) then Continue;

      // Don't assign if property value is overridden for mirroring
        if Dest is TRaveControl and TRaveControl(Dest).PropOverridden(string(PropInfo.Name)) then Continue;
        ...
        case PropInfo.PropType^.Kind of
          ...
          tkSet: SetOrdProp(Dest,PropInfo,GetOrdProp(self,PropInfo));
          tkClass: SetOrdProp(Dest,PropInfo,GetOrdProp(self,PropInfo));
        ...
        {$IFDEF Unicode}
          tkUString: {$IFDEF LEVELD10_2}SetStrProp{$ELSE}SetUnicodeStrProp{$ENDIF}(Dest,PropInfo,{$IFDEF LEVELD10_2}GetStrProp{$ELSE}GetUnicodeStrProp{$ENDIF}(self,PropInfo));
        {$ENDIF}
  ...
  if (not Assigned(Value) or (Value is self.ClassType) or
   (self is Value.ClassType)) and (Value <> FMirror) then begin
    ...
    if Assigned(FMirror) then begin
      ...
      FMirrorOverride := ''; // Clear all property overrides
    end; { else }
  end; { if }
end;  { SetMirror }
...
procedure TRaveControl.OverrideProp(PropName: string);
begin
  if FoundString(UpperCase(PropName), AlwaysOverride) then Exit;
  PropName := FindRavePropertyAlias(TRaveComponentClass(ClassType), PropName);
  if Assigned(Mirror) and not FoundString(PropName, FMirrorOverride) then begin
    FMirrorOverride := FMirrorOverride + '`' + PropName;
    if Assigned(ModifiedProc) then begin
      ModifiedProc(true); // Refresh all windows
    end; { if }
  end; { if }
end;

procedure TRaveControl.RevertProp(PropName: string);
var
  Index: integer;
begin { RevertProp }
  if Assigned(Mirror) then begin
    Index := FindStringPos(PropName, FMirrorOverride);
    if Index > 0 then begin
      Delete(FMirrorOverride, Index, Length(PropName) + 1);
      Mirror.Reflect;
      if Assigned(ModifiedProc) then begin
        ModifiedProc(true); // Refresh all windows
      end; { if }
...
function TRaveControl.PropOverridden(PropName: string): boolean;
begin { PropOverridden }
  Result := Assigned(Mirror) and FoundString(PropName, FMirrorOverride);
end;  { PropOverridden }

function TRaveControl.AlwaysOverride: string;
begin
  Result := {Trans-}'`DEVLOCKED`LEFT`LOCKED`MIRROR`MIRROROVERRIDE`NAME`TOP`';
end;
...
begin { SetParameters }
  ...
  for I1 := 0 to FParams.Count - 1 do begin
    ...
    for I2 := 1 to Length(S1) do begin
      if AnsiChar(S1[I2]) in ['<','>'] then begin
        S1[I2] := '_';
      end; { for }
    end; { for }
    FParams[I1] := S1;
...
begin { DefineProperty }
  if HasData and Assigned(WriteData) then begin
    ...
    try
      ...
      StreamHelper.{$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}(Name);
      StreamHelper.WriteIndex(Stream.Size);
      {$IFDEF LEVEL6}
      StreamHelper.Write(Stream.Memory,Stream.Size);
      {$ELSE}
    ...
    if Count > 0 then begin
      ...
      try
        ...
        for I1 := 0 to (Count - 1) do begin
          ...
          if PropertyOverride(string(PropInfo.Name),ReadData,WriteData) and
      ...
      if I1 <> PropInfo.Default then begin
        ...
        for I2 := 0 to 31 do begin
          if I2 in TWriteSet(I1) then begin
            StreamHelper.{$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}(GetEnumName(GetTypeData(PropInfo.PropType^).CompType^,I2));
          end; { if }
        end; { for }
        StreamHelper.{$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}('');
      end; { if }
    end;
    tkClass: begin
      ...
      if not Assigned(O1) then begin
        if (PropInfo.Default <> $00000000) and
         (PropInfo.Default <> integer($80000000)) then begin
          WritePropName(PropInfo);
          WriteInteger(0);
        end; { if }
      end else begin
        if O1 is TComponent then begin { Write out [Owner.]Name }
          ...
          with TRaveComponentList(O1) do begin
            for I1 := 0 to Count - 1 do begin
              StreamHelper.{$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}(GetComponentID(TComponent(Items[I1])));
            end; { for }
            StreamHelper.{$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}('');
          end; { with }
          ...
          with TRavePersistentList(O1) do begin
            for I1 := 0 to Count - 1 do begin
              ...
              if Assigned(O2) then begin
                StreamHelper.{$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}(O2.ClassName);
...
{$IFDEF Unicode}
    tkUString: begin
      S1 := {$IFDEF LEVELD10_2}GetStrProp{$ELSE}GetUnicodeStrProp{$ENDIF}(Instance,PropInfo);
      if S1 <> '' then begin
        WritePropName(PropInfo);
        WriteString(S1);
      end; { if }
...
{$ENDIF}
      ...
      if (TypeKind <> tkVariant) or (PropInfo.Name <> 'Tag') then begin
        RaveError(Trans('Attempt to write invalid property type.  TTypeKind = '
         + IntToStr(Ord(TypeKind))));
      end;
    end;
  end; { case }
end;  { WriteProperty }
...
begin { WritePropName }
  StreamHelper.{$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}(string(PropInfo.Name));
end;  { WritePropName }

procedure TRaveWriter.WriteID(Value: string);
begin { WriteID }
  StreamHelper.WriteByte(Ord(vkID));
  StreamHelper.{$IFDEF LEVEL6}Write{$ELSE}WriteString{$ENDIF}(Value);
end;  { WriteID }
...
begin { WriteFloat }
  ...
  {$IFDEF CPUX64}
  D1 := Value;
  DoubleToRaveExtended(D1, RE);
  StreamHelper.Write(@RE, SizeOf(RE));
  {$ELSE}
  StreamHelper.Write(Value);
  {$ENDIF}
  {$ELSE}
...
var
  ...
  Size: integer;
begin { DefineProperty }
  if not PropHandled and (RaveCompareText(Name,PropName) = 0) and
...
begin { ReadProperties }
  ...
  { Get list of properties to override }
    ...
    { Top and Left are special properties in Delphi used for Form Inheritance,
      ...
      if (PropName <> {Trans-}'Left') and (PropName <> {Trans-}'Top') and Assigned(Instance) then begin
        TRaveComponent(Instance).DefineProperties(self);
      end; { if }
      ...
      { Look for property in Override list }
        FOverrideList := SaveOverrideList;
        if PropertyOverride(PropName,ReadData,WriteData) and Assigned(ReadData) then begin
          ReadData(self);
...
begin { ReadProperty }
  ...
  case TypeKind of
    tkInteger: begin
      ...
      if not AbsorbProperty and (PropInfo.SetProc <> nil) then begin
    ...
    tkClass: begin
      case ValueKind of
        ...
        vkPropList: begin
          ...
          ReadProperties(O1 as TPersistent);
        end;
        vkComponentList: begin
          ...
          try
            ...
            while S1 <> '' do begin
              ...
              if not Assigned(O1) then begin { Register as forward reference }
                CompList.Add(nil);
                TRaveProjectManager(RootOwner.Project).AddForwardRef(S1,RootOwner,TObject(CompList),nil,CompList.Count - 1);
              end else begin
                CompList.Add(O1);
              end; { else }
              S1 := ReadString;
...
{$IFDEF Unicode}
    tkUString: begin
      S1 := ReadString;
      if not AbsorbProperty and (PropInfo.SetProc <> nil) then begin
        {$IFDEF LEVELD10_2}SetStrProp{$ELSE}SetUnicodeStrProp{$ENDIF}(Instance,PropInfo,S1);
      end; { if }
    end;
{$ENDIF}
    else begin
      if (TypeKind <> tkVariant) or (PropInfo.Name <> 'Tag') then begin
        RaveError(Trans('Attempt to read invalid property type.  TTypeKind = '
         + IntToStr(Ord(TypeKind))));
      end;
    end;
  end; { case }
...
function TRaveReader.ReadChar: ansichar;
begin { ReadChar }
  {$IFDEF LEVEL6}
  StreamHelper.Read(Result);
  {$ELSE}
...
begin { ReadFloat }
  {$IFDEF LEVEL6}
  {$IFDEF CPUX64}
  StreamHelper.Read(@RE, SizeOf(RE));
  RaveExtendedToDouble(RE, D1);
  Result := D1;
  {$ELSE}
  StreamHelper.Read(Result);
  {$ENDIF}
...
constructor TRaveModule.CreateFromStream(Stream: TStream; AOwner: TRaveProjectItem; AProjectItem: TRaveProjectItem);
begin
  inherited Create;
end;

constructor TRaveModule.CreateFromFile(FileName: string; AOwner: TRaveProjectItem);
begin
  inherited Create;
end;
...
finalization
  ...
  if Assigned(RaveModuleClassList) then begin
    ClearStringList(RaveModuleClassList);
    FreeAndNil(RaveModuleClassList);
  end; { if }
end.