...
//  TRaveDataLinkProc = procedure(Params : Pointer);
  ...
  { Following fields are used for dynamic Data Links}
    FFileName: string;
    FLibHandle: longint;
    FInformationProc: TDLInformationProc;
    FConnectionConfigureProc: TDLConnectionConfigureProc;
    FConnectionOpenProc: TDLConnectionOpenProc;
    FConnectionCloseProc: TDLConnectionCloseProc;
    FGetTablesProc: TDLGetTablesProc;
    FGetFieldsProc: TDLGetFieldsProc;
    FResultSetOpenProc: TDLResultSetOpenProc;
    FResultSetCloseProc: TDLResultSetCloseProc;
    FResultSetFirstProc: TDLResultSetFirstProc;
    FResultSetNextProc: TDLResultSetNextProc;
    FResultSetEOFProc: TDLResultSetEOFProc;
    FResultSetGetRowProc: TDLResultSetGetRowProc;
    FResultSetSetFilterProc: TDLResultSetSetFilterProc;
    FResultSetSetSortProc: TDLResultSetSetSortProc;
    FResultSetConfigureProc: TDLResultSetConfigureProc;
    FErrorTextProc: TDLGetErrorTextProc;
...
begin
  if FConnectionHandle >= 0 then begin
...
begin
  if FResultSetHandle >= 0 then begin
  ...
  if (FTransferList <> nil) then begin
    FreeMem(FTransferList,FFieldList.Count);
    FTransferList := nil;
  end; { if }
end;
...
begin
  Result := (FResultSetHandle < 0) or FConnection.FDataLink.ResultSetEOF(FResultSetHandle);
end;
...
begin
  ...
  try
    ...
    if Params.ResultCode = 0 then begin
      ...
      FieldList.Clear;
      for Index := 0 to Params.FieldCount-1 do begin
        with FieldList.Add do begin
          ...
          if Assigned(Params.FieldList[Index].FullName) then begin
            FullName := StrPas(Params.FieldList[Index].FullName);
          end; { if }
          if Assigned(Params.FieldList[Index].Description) then begin
    ...
    StrDispose(Params.QueryCommand);
