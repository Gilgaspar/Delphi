...
var
  DefaultCompressMethod: integer = NoCompression{!!!CompressMethod2};

type
{$IFDEF CPUX64}
  TRaveExtended = array[1..10] of byte;
{$ENDIF}
  ...
  public
    ...
    procedure WriteByte({$IFDEF LEVEL6}const {$ENDIF}Value: byte);
    procedure WriteIndex(const Value: longint);
    {$IFDEF LEVEL6}
    procedure Write(const Value: pointer; Count: integer); overload;
    procedure Write(const Value: byte); overload;
    procedure Write(const Value: boolean); overload;
    procedure Write(const Value: ansichar); overload;
    ...
    procedure Write(const Value: longint); overload;
    procedure Write(const Value: cardinal); overload;
    ...
    procedure Write(const Value: extended); overload;
    ...
    {$IFDEF LEVEL6}
    procedure Read(Value: pointer; Count: integer); overload;
    procedure Read(var Value: byte); overload;
    procedure Read(var Value: boolean); overload;
    procedure Read(var Value: ansichar); overload;
    procedure Read(var Value: shortInt); overload;
    procedure Read(var Value: smallInt); overload;
    procedure Read(var Value: word); overload;
    procedure Read(var Value: longint); overload;
    procedure Read(var Value: cardinal); overload;
    procedure Read(var Value: Int64); overload;
    procedure Read(var Value: single); overload;
    procedure Read(var Value: double); overload;
    procedure Read(var Value: extended); overload;
  ...
  function FindStringPos(InStr: string; SearchString: string): integer;
  function FoundString(InStr: string; SearchString: string): boolean;
  function FindString(InStr: string; SearchString: string): integer;
...
{$IFDEF CPUX64}
  procedure RaveExtendedToDouble(RE: TRaveExtended; var DValue: double);
  procedure DoubleToRaveExtended(var DValue: double; var RE: TRaveExtended);
{$ENDIF}
...
begin { CloneObject }
  ...
  if Count > 0 then begin
    ...
    try
      ...
      {TransOff}
        if (RaveCompareText(string(PropInfo.Name),'Name') = 0) or
         (RaveCompareText(string(PropInfo.Name),'DevLocked') = 0) or
         (RaveCompareText(string(PropInfo.Name),'Locked') = 0) or
         (RaveCompareText(string(PropInfo.Name),'Mirror') = 0) or
         (RaveCompareText(string(PropInfo.Name),'Top') = 0) or
         (RaveCompareText(string(PropInfo.Name),'Left') = 0) then Continue;
      {TransOn}
...
begin { ValidID }
  ...
  for I1 := 1 to Length(Value) do begin
    if AnsiChar(Value[I1]) in ['0'..'9','a'..'z','A'..'Z','_'] then begin
...
//{$IFDEF CLR}
  ToList.Clear;
  for I1 := 0 to FromList.Count - 1 do begin
    ToList.Add(FromList.Items[I1]);
  end; { for }
//{$ELSE}
...
{ Find first occurrence of SepStr characters }
  ...
  if CutLen >= 0 then begin
    if CutLen = 0 then begin
      ...
      Result := Trim(MBCSCopy(Value,1,CutLen));
    end; { else }
...
begin { AddJoinChars }
  ...
  NeedJoin := false;
  for I1 := 1 to Length(Value) do begin
    if not (AnsiChar(Value[I1]) in ['0'..'9','A'..'Z','_','a'..'z']) then begin
      NeedJoin := true;
      Break;
    end; { if }
  end; { for }
...
function FindStringPos(InStr: string; SearchString: string): integer;
begin
  Result := RavePos('`' + InStr + '`',SearchString + '`');
end;

function FoundString(InStr: string; SearchString: string): boolean;
begin
  Result := FindStringPos(InStr, SearchString) <> 0;
end;
...
var
  ...
  I1: integer;
begin { FindString }
  FoundPos := FindStringPos(InStr,SearchString);
  Result := 0;
...
begin { StripFloatStr }
  ...
  for I1 := 1 to Length(Value) do begin
    ...
    end else if AnsiChar(Value[I1]) in ['0'..'9'] then begin
      Result := Result + Value[I1];
    end else if (Value[I1] = {$IFDEF LevelXE}FormatSettings.{$ENDIF}DecimalSeparator) and not Decimal then begin
      Decimal := true;
      Result := Result + {$IFDEF LevelXE}FormatSettings.{$ENDIF}DecimalSeparator;
    end; { else }
  end; { for }
end;  { StripFloatStr }
...
var
  ErrCode: integer;
begin { StrToFloatDef }
  if {$IFDEF LevelXE}FormatSettings.{$ENDIF}DecimalSeparator <> '.' then begin
    ErrCode := Pos({$IFDEF LevelXE}FormatSettings.{$ENDIF}DecimalSeparator,Value);
...
procedure TStreamHelper.WriteByte({$IFDEF LEVEL6}const {$ENDIF}Value: byte);
begin
{$IFDEF LEVEL6}
  Write(Value);
{$ELSE}
...
procedure TStreamHelper.WriteIndex(const Value: longint);
begin { WriteIndex }
...
{$IFDEF LEVEL6}
procedure TStreamHelper.Write(const Value: pointer; Count: integer);
begin
  Stream.WriteBuffer(Value^,Count);
end;
...
procedure TStreamHelper.Write(const Value: ansichar);
begin
  Stream.WriteBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Write(const Value: shortInt);
begin
  Stream.WriteBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Write(const Value: smallInt);
begin
  Stream.WriteBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Write(const Value: word);
begin
  Stream.WriteBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Write(const Value: longint);
begin
  Stream.WriteBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Write(const Value: cardinal);
begin
  Stream.WriteBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Write(const Value: Int64);
begin
  Stream.WriteBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Write(const Value: single);
begin
  Stream.WriteBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Write(const Value: double);
begin
  Stream.WriteBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Write(const Value: extended);
begin
  Stream.WriteBuffer(Value,SizeOf(Value));
end;


procedure TStreamHelper.Write(const Value: string);
{$IFDEF UNICODE}
var
  Data: TBytes;
  Encoding: TEncoding;
...
begin { ReadIndex }
  ...
  if (B1 < $80) then begin
    ...
    B2 := ReadByte;
    B3 := ReadByte;
    Result := (((((longint(B1 - $E0) * $0100) + B2) * $0100) + B3) * $0100) + ReadByte;
  end; { else }
end;  { ReadIndex }

{$IFDEF LEVEL6}
procedure TStreamHelper.Read(Value: pointer; Count: integer);
begin
  Stream.ReadBuffer(Value^,Count);
end;

procedure TStreamHelper.Read(var Value: byte);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: boolean);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: ansichar);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: shortInt);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: smallInt);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: word);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: longint);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: cardinal);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: Int64);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: single);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: double);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: extended);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: TDateTime);
begin
  Stream.ReadBuffer(Value,SizeOf(Value));
end;

procedure TStreamHelper.Read(var Value: string);
var
  Len: longint;
{$IFDEF UNICODE}
var
...
begin
  ...
  {$ELSE}
    if Len = UnicodeMarker then begin
      ReadIndex;
      Stream.Seek(ReadIndex, soFromCurrent);
      Len := ReadIndex;
    end;
    SetLength(Value,Len);
    Read(@Value[1],Len);
