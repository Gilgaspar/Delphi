...
uses
  Classes, SysUtils,
  Windows, VCL.Dialogs, VCL.StdCtrls, VCL.Forms,
...
type
  ...
  public
    ...
    procedure CompileStream(Source: string; FileName: string; AModule: TRCPUModule; Definition: boolean); override;
...
//    {$IFDEF DOMAP}Rewrite(MapFile);{$ENDIF}
  ...
  { Add constants }
    ...
    P1 := nil;
    AddConst('NUL',FindID('pointer').BaseType,P1);
...
procedure TRaveDelphiCompiler.CompileStream(Source: string; FileName: string; AModule: TRCPUModule; Definition: boolean);
begin { CompileStream }
{$IFDEF DOMAP}
  if Definition then begin
...
//      {$IFDEF DOMAP}FreeAndNil(xMapFile);{$ENDIF}
  ...
  AModule.DataSize := AModule.Scope.DataSize;
...
begin { CompileFile }
  ...
  Source.LoadFromFile(FileName + '.rvs');
  StringStream := TStringStream.Create('');
  StringStream.WriteString(Source.Text);
  StringStream.Position := 0;
//  Stream := TFileStream.Create(FileName + '.rvs',fmOpenRead);
  try
    CompileStream(Source.Text,FileName,AModule,true);
    CompileStream(Source.Text,FileName,AModule,false);
  finally
//    Stream.Free;
    StringStream.Free;
    Source.Free;
