  ...
  begin { GetNextRPTFCommand }
    ...
    { Find beginning of next RPTF command }
      while (Pos <= Length(RPTFStr)) and
       not CharInSet(RPTFStr[Pos],[RPTFPrefix,RPTFInfix]) do begin
        Inc(Pos);
      end; { while }
    ...
    until (Pos > Length(RPTFStr)) or CharInSet(RPTFStr[Pos],[RPTFInfix,RPTFSuffix]);
...
{ Return the length of the RPTF formatted string without any format characters }
  ...
  begin { CompareStates }
    with New do begin
      ...
      if CompareText(Old.Name,Name) <> 0 then begin
  ...
  var
    ...
    procedure PushState;
    var
      TempState: TFormatState;
    begin { PushState }
      TempState := TFormatState.Create;
      TempState.Name := FormatState.Name;
      TempState.Size := FormatState.Size;
      TempState.Style := FormatState.Style;
      TempState.Para := FormatState.Para;
      ...
      case RTFCh of
        ...
        { Skip over extra spaces }
          repeat
            IncRTFCh;
          until RTFCh <> ' ';
          if CharInSet(RTFCh,['a'..'z']) then begin { Read Control Word }
            TokenType := ttControlWord;
          { Read letter sequence }
            repeat
              TokenStr := TokenStr + RTFCh;
              IncRTFCh;
            until not CharInSet(RTFCh,['a'..'z']);
            if CharInSet(RTFCh,['-','0'..'9']) then begin { Read numeric parameter }
              repeat
                TokenParam := TokenParam + RTFCh;
                IncRTFCh;
              until not CharInSet(RTFCh,['0'..'9']);
              Val(TokenParam,TokenNum,ErrCode);
            end; { if }
            if CompareText(RTFCh,' ') = 0 then begin { Absorb space }
              IncRTFCh;
            end; { else }
          ...
          TokenType := ttText;
          repeat
            if not {$IFDEF LEVEL2009}TCharacter.IsControl(RTFCh){$ELSE}(AnsiChar(RTFCh) in [#0..#31]){$ENDIF} then begin
              TokenStr := TokenStr + RTFCh;
            end; { if }
            IncRTFCh;
            if RTFCh = '\' then begin { Check next character for formatting char }
              ...
              end else if CharInSet(RTFCh,['\','{','}']) then begin { Add this character on as text }
                TokenStr := TokenStr + RTFCh;
                IncRTFCh;
              end else begin
                DecRTFCh;
    ...
    { Write out any formatting differences }
      ...
      if FormatStr <> '' then begin
        ...
        StreamWriteBuffer(RPTFStream,FormatStr);
        FormatStr := '';
      end; { if }

      if TextStr <> '' then begin
        StreamWriteBuffer(RPTFStream,TextStr);
      end; { if }

    { Save off current state }
      LastState.Assign(FormatState);
    end;  { OutputText }
...
//!!! Don't set default font until RTF string is found
    ...
    begin { ProcessControlWord }
      ...
      end else if TokenStr = 'v' then begin { hidden text }
        HiddenText := (TokenParam <> '0');
      end else if TokenStr = 'f' then begin { Select font }
        if Fonts.Count - 1 >= TokenNum then begin
  ...
  begin { RTFToRPTF }
    ...
    LastState := GetBaseRPTFState;
    FormatState := GetBaseRPTFState;
