...
type
  ...
  TDLDBXConfigureForm = class(TDLConfigForm)
    ...
    procedure sgridConnectionSettingsClick(Sender: TObject);
  protected
    FDataSource: string;
...
begin
  ...
  GetDataSources;
  FUserName := TempUserName;
  FPassword := TempPassword;
  ...
  try
    ...
    if Index >= 0 then begin
      ...
      DataSourceObject.UserName := UserName;
      DataSourceObject.Password := Password;
    end; { if }
...
begin
  ...
  try
    ...
    Result := Reg.ReadString('\Software\Borland\BDS\DBExpress', 'Connection Registry File', '')
  finally
    Reg.Free;
  end;
end;
...
begin
  ...
  try
    Reg.RootKey := HKey_Current_User;
    Result := Reg.ReadString('\Software\Borland\BDS\DBExpress', 'Driver Registry File', '')
  finally
    Reg.Free;
  end;
end;
...
begin
  if lboxConnection.ItemIndex >= 0 then begin
    ...
    try
      ...
      editPassword.Text := TDataSourceObject(lboxConnection.Items.Objects[lboxConnection.ItemIndex]).Password;
      editUserName.Text := TDataSourceObject(lboxConnection.Items.Objects[lboxConnection.ItemIndex]).UserName;
      ConnectionSettings.Text := TDataSourceObject(lboxConnection.Items.Objects[lboxConnection.ItemIndex]).DataSource;
      { get values from dbxconnections.ini file }
...
procedure TDLDBXConfigureForm.PageControl1Change(Sender: TObject);
begin
  if lboxConnection.ItemIndex >= 0 then begin
    TDataSourceObject(lboxConnection.Items.Objects[lboxConnection.ItemIndex]).Password := editPassword.Text;
    TDataSourceObject(lboxConnection.Items.Objects[lboxConnection.ItemIndex]).UserName := editUserName.Text;
  end; { if }
end;

procedure TDLDBXConfigureForm.Button1Click(Sender: TObject);
begin
  TestConnection(lboxTables.Items);
end;

procedure TDLDBXConfigureForm.sgridConnectionSettingsClick(
  Sender: TObject);
begin
