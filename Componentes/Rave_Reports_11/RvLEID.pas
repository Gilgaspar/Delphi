...
begin { CreateFromStreamHelper }
  inherited Create;
  FIDKind := TIDKind(StreamHelper.ReadByte);
...
begin { SaveToStreamHelper }
  {$IFDEF LEVEL6}
  StreamHelper.Write(byte(FIDKind));
  StreamHelper.Write(FID);
  {$ELSE}
...
begin { CreateFromStreamHelper }
  ...
  FAddress.Loc := TAddressLoc(StreamHelper.ReadByte);
end;  { CreateFromStreamHelper }
...
begin { SaveToStreamHelper }
  ...
  {$IFDEF LEVEL6}
  StreamHelper.Write(byte(FAddress.Loc));
  {$ELSE}
...
begin { CreateFromStreamHelper }
  ...
  GetMem(FData,DataSize);
{$IFDEF LEVEL6}
  StreamHelper.Read(FData,DataSize);
{$ELSE}
...
begin { SaveToStreamHelper }
  ...
  {$IFDEF LEVEL6}
  StreamHelper.Write(Data,DataSize);
  {$ELSE}
...
begin { CreateFromStreamHelper }
  inherited CreateFromStreamHelper(TypeHelper,StreamHelper);
  FOpKind := TOpKind(StreamHelper.ReadByte);
end;  { CreateFromStreamHelper }
...
begin { SaveToStreamHelper }
  inherited SaveToStreamHelper(TypeHelper,StreamHelper);
  {$IFDEF LEVEL6}
  StreamHelper.Write(byte(FOpKind));
  {$ELSE}
...
{ Find ID that is equal or just greater than }
  with IDList do begin
    ...
    while Low <= High do begin
      ...
      SearchResult := RaveCompareText(IDName,Result.ID);
