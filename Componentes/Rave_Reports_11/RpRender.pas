...
type
  ...
  protected
    ...
    function ReadPIVar(VarName: string): string;
    ...
    property AnsiStrings: boolean read FAnsiStrings write FAnsiStrings;
  public
  ...
  protected
    ...
    procedure SetHyperlink(Value: string); virtual;
  ...
  // Drawing Methods
    ...
    procedure PrintBitmapRect(const X1,Y1,X2,Y2: double; AGraphic: TBitmap); virtual;
    procedure PrintBitmap(const X1,Y1,ScaleX,ScaleY: double; AGraphic: TBitmap); virtual;
  ...
  published
    ...
    property FileExtension: string read GetFileExtension write FFileExtension;
  end; { TRPRender }
...
//  Font.Size := Round(FontHeight * Renderer.YDPI);
  FontData.Size := FontData.Height * 72;
  Font.Size := Round(FontData.Size);
  Renderer.FontChanged(Font);
end;
...
begin { GetNextRPTFCommand }
  ...
  { Find beginning of next RPTF command }
    while (Pos <= Length(RPTFStr)) and
     not (AnsiChar(RPTFStr[Pos]) in [RPTFPrefix,RPTFInfix]) do begin
      Inc(Pos);
    end; { while }
  ...
  until (Pos > Length(RPTFStr)) or (AnsiChar(RPTFStr[Pos]) in [RPTFInfix,RPTFSuffix]);
...
begin
  ...
  if not Renderer.OutputInvalid then begin
    ...
    // Restoring saved style must happen after setting the color
    // as setting the color also changes the style
...
begin
  ...
  if Renderer.ForceAnsi then begin
    FAnsiStrings := true;
  end;
  FReportDateTime := Now;
  ReadReportHeader;
...
begin
  ...
  repeat
    ...
    // Misc commands
      ...
      rpcSetBin: SetBin;
      rpcSetHyperlink: DoSetHyperlink;
      else begin
...
// OutputName - Output port for Printer
  ...
  FCurrPagePos := FStream.Position;
  if (FileVer >= 5130) and not AnsiStrings then begin // Auto-detect Ansistrings in ver 5130 or greater
    if (FLastPage < FFirstPage) or (FPageCount < 1) or (FPageCount > 99999) or
    (Reportheader.PaperWidth < 0) or (Reportheader.PaperWidth > 99999) or
    (Reportheader.PaperHeight < 0) or (Reportheader.PaperHeight > 99999) or
    (Reportheader.LeftWaste < -99999) or (Reportheader.LeftWaste > 99999) or
    (Reportheader.RightWaste < -99999) or (Reportheader.RightWaste > 99999) or
    (Reportheader.TopWaste < -99999) or (Reportheader.TopWaste > 99999) or
    (Reportheader.BottomWaste < -99999) or (Reportheader.BottomWaste > 99999) then begin
      AnsiStrings := true;
      ReadReportHeader;
    end;
  end;
...
begin
  ...
  if wLen > 0 then begin
    if AnsiStrings then begin
      SetLength(S1, wLen);
      FStream.ReadBuffer(S1[1], wLen);
      Result := string(S1);
    end else begin
      SetLength(Result, wLen);
      FStream.ReadBuffer(Result[1], {$IFDEF Unicode}2 * {$ENDIF}wLen);
    end;
...
function TRPConverter.ReadPIVar(VarName: string): string;
var
  BufMax: byte;
  BufLen: byte;
...
begin { ReadPIVar }
  ...
  if BufLen in [254,255] then begin { not defined, goto previous if any }
    ...
    if Buf[1] = 0 then begin
      Result := GetPIVar(VarName);
    end else begin
      SavePos := FStream.Position;
      FStream.Position := Abs(Buf[1]);
      Result := ReadPIVar(VarName);
      FStream.Position := SavePos;
    end;
  end else begin
    if (BufMax {$IFDEF Unicode} * 2{$ENDIF}) >= 128 then begin
      ReadBuf(Buf, 128);
    end else begin
      ReadBuf(Buf, BufMax {$IFDEF Unicode} * 2 {$ENDIF});
    end;
    if BufLen > 0 then begin
      if AnsiStrings then begin
        SetLength(S1, BufLen);
        Move(Buf,S1[1],BufLen);
        Result := string(S1);
      end else begin
        SetLength(Result,BufLen);
        Move(Buf,Result[1],BufLen {$IFDEF Unicode} * 2{$ENDIF});
      end;
    end else begin
      Result := GetPIVar(VarName);
    end; { else }
...
procedure TRPConverter.DoSetHyperlink;
begin
  Renderer.SetHyperlink(ReadString);
end;
...
procedure TRPRender.SetHyperlink(Value: string);
begin
  Converter.Hyperlink := Value;
end;
...
function TRPRender.GetFileExtension: string;
begin
...
begin
  if AData <> '' then begin
    S1 := AnsiString(AData);
    ActiveStream.WriteBuffer(S1[1],Length(S1));
  end; { if }
end;
