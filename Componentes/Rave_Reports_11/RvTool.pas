...
type
  ...
  function FindPropertyEditorClass(PropType: PTypeInfo; ControlClass: TClass; PropName: string): TRavePropertyEditorClass;
...
(**)TRaveProjectDesignerClass)=nil;RegisterFindPropertyEditorClassProc:function(PropType:PTypeInfo;
(**)ControlClass:TClass;PropName:string):TRavePropertyEditorClass=nil;
(*****************************************************************************}
( These variables are used internally by RAVE and should not be modified
(*****************************************************************************)
...
function FindPropertyEditorClass(PropType: PTypeInfo; ControlClass: TClass; PropName: string): TRavePropertyEditorClass;
begin { FindPropertyEditorClass }
  if Assigned(RegisterFindPropertyEditorClassProc) then begin
    Result := RegisterFindPropertyEditorClassProc(PropType,ControlClass,PropName);
  end else begin
    Result := nil;
  end; { else }
end;  { FindPropertyEditorClass }
...
begin { AddInst }
  ...
  Inc(FInstCount);
  if (AInstance is TRaveControl) and TRaveControl(AInstance).PropOverridden(string(APropInfo.Name)) then begin
    FOverridden := true;
  end; { if }
end;  { AddInst }
...
begin { SetOrdValue }
  for I1 := 0 to InstCount - 1 do begin
    ...
    if Inst is TRaveControl then begin
      TRaveControl(Inst).OverrideProp(Name);
