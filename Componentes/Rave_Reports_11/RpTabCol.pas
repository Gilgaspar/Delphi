...
{*************************************************************************}
  ...
  begin { GetAsFloat }
    ...
    while I1 <= Length(S1) do begin
      if not CharInSet(S1[I1], ['0'..'9',{$IFDEF LevelXE}FormatSettings.{$ENDIF}DecimalSeparator,'-']) then begin
        Delete(S1,I1,1);
      end else begin
        Inc(I1);
      end; { else }
    end; { while }
    if ({$IFDEF LevelXE}FormatSettings.{$ENDIF}DecimalSeparator <> '.') and (Pos({$IFDEF LevelXE}FormatSettings.{$ENDIF}DecimalSeparator,S1) > 0) then begin
      S1[Pos({$IFDEF LevelXE}FormatSettings.{$ENDIF}DecimalSeparator,S1)] := '.';
    end; { if }
    Val(S1,F1,ErrCode);
  ...
  begin { AddLine }
    ...
    if S1 <> '' then begin
      ColStream.WriteBuffer(S1[1],{$IFDEF Unicode}2 * {$ENDIF}Length(S1));
    end; { if }
    with ColHeader do begin
      Inc(Lines);
      Inc(DataSize,SizeOf(ColText) + {$IFDEF Unicode}2 * {$ENDIF}Length(S1));
      ...
      { Figure out how much height we can print and remaining amount }
        ...
        for Line := 1 to ColHeader.Lines do begin
          ...
          if ColText.Len > 0 then begin
            ColStream.ReadBuffer(S1[1],{$IFDEF Unicode}2 * {$ENDIF}ColText.Len);
          end; { if }
