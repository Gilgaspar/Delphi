...
uses
  ...
  Graphics,
  Classes, SysUtils, RpDefine, RpRender;
...
type
  ...
  protected
    ...
    procedure WriteDiv(const AText: string; const AX1, AY1: double; const AWidth: double = 0; const AHeight: double = 0; const AJustify: TPrintJustify = pjLeft);
    ...
    procedure PrintBitmapRect(const AX1, AY1, AX2, AY2: double; AGraphic: TBitmap); override;
  ...
  published
    ...
    property UseBreakingSpaces: boolean read FUseBreakingSpaces write FUseBreakingSpaces;
  end;
...
begin
  ffMaxY := 0;
  if RavePos(';',OutputFileName) > 0 then begin
    OutputFileName := MBCSCopy(OutputFileName, 0, RavePos(';',OutputFileName) - 1);
  end; { if }
  ...
  if (LExt = '') or (LExt = '.') then begin
    LExt := MBCSCopy(FileExtension, RavePos('.',FileExtension),Length(FileExtension));
    if RavePos(';',LExt) > 0 then begin
      LExt := MBCSCopy(LExt, 0, RavePos(';',LExt) - 1);
    end; { if }
  end; { if }
...
var
  ...
  function BeforeSpaces: integer;
  var
    I1: integer;
  begin
    Result := 0;
    for I1 := 1 to Length(LText) do begin
      if LText[I1] <> ' ' then exit;
      Inc(Result);
    end; { for }
  end;
...
{ Count the number of spaces in Text }
  LText := psText;
  Spaces := BeforeSpaces + CountSpaces(LText);
...
procedure TRvRenderHTML.WriteDiv(const AText: string; const AX1, AY1: double;
 const AWidth: double = 0; const AHeight: double = 0; const AJustify: TPrintJustify = pjLeft);
var
  s: string;
begin
  ...
  Print('<DIV STYLE="');
  if Pos('<a href', AText) > 0 then begin
    Print('z-index:1;');
  end;
  case AJustify of
    pjCenter: begin
      PrintLn('text-align:center;left:' + IntToStr(InchesToPixels(AX1) + FOffsetX) + ';top:'
       + IntToStr(InchesToPixels(AY1) + FOffsetY) + ';' + s + GetFontStyle(Converter.FontData) + '">'
       + AText + '</DIV>');
    end;
    pjRight: begin
      PrintLn('text-align:right;left:' + IntToStr(InchesToPixels(AX1) + FOffsetX) + ';top:'
       + IntToStr(InchesToPixels(AY1) + FOffsetY) + ';' + s + GetFontStyle(Converter.FontData) + '">'
       + AText + '</DIV>');
    end else begin
      PrintLn('left:' + IntToStr(InchesToPixels(AX1) + FOffsetX) + ';top:'
       + IntToStr(InchesToPixels(AY1) + FOffsetY) + ';' + s + GetFontStyle(Converter.FontData) + '">'
       + AText + '</DIV>');
    end;
  end;
...
function TRvRenderHTML.HTMLText(const psText: string): string;
  ...
  begin
    Result := '';
    if not UseBreakingSpaces then begin
      for I1 := 1 to Length(psText) do begin
        if psText[I1] = ' ' then begin
          Result := Result + '&nbsp;';
        end else begin
          Result := Result + psText[I1];
        end; { else }
      end; { for }
    end else if (Length(psText) > 2) and (psText[1] = ' ') and (psText[2] = ' ') then begin
      for I1 := 1 to Length(psText) do begin
        if psText[I1] = ' ' then begin
          Result := Result + '&nbsp;';
        end else begin
          Result := Result + Copy(psText,I1, Length(psText));
          exit;
        end;
      end;
    end else begin
...
begin
  ...
  if Converter.Hyperlink <> '' then begin
    Result := '<a href="' + Converter.Hyperlink + '">' + Result + '</a>';
  end; { else }
end;

function TRvRenderHTML.GetFontStyle(AFont: TSaveFont): string;
begin
  Result := 'font:';
  if AFont.Italic then begin
  ...
  Result := Result + FloatToStr(AFont.Size) + 'pt ''' + AFont.Name + '''';
  if AFont.Underline then begin
...
procedure TRvRenderHTML.PrintBitmapRect(const AX1, AY1, AX2, AY2: double; AGraphic: TBitmap);
var
  LFilename: string;
  LPathURL: string;
  LImageDef: string;
begin
  ...
  if ServerMode then begin
    ...
    if not (AnsiChar(LPathURL[Length(LPathURL)]) in ['/', '\']) then begin
      LPathURL := LPathURL + '/';
    end;
  ...
  LImageDef := '<IMG Src="' + LPathURL + '" width='
   + IntToStr(InchesToPixels(AX2 - AX1)) + ' height=' + IntToStr(InchesToPixels(AY2 - AY1)) + '>';
  if Converter.Hyperlink <> '' then begin
    LImageDef := '<a href="' + Converter.Hyperlink + '">' + LImageDef + '</a';
  end; { if }
  WriteDiv( LImageDef, AX1, AY1, AX2 - AX1, AY2 - AY1);
end;
...
begin
  ...
  WriteDiv(LImageDef, Rect.Left / 72, Rect.Top / 72, (Rect.Right / 72) - (Rect.Left / 72)
...
begin
  ...
  if ActiveCacheDir[Length(ActiveCacheDir)] <> PathDelim then begin
    ActiveCacheDir := ActiveCacheDir + PathDelim;
  end; { if }
  repeat
    Result := ActiveCacheDir + PadIntToStr(LastFileNameIndex) + '.' + AExtension;
...
begin
  WriteDiv(HTMLText(psText), 0, pfY - Converter.FontData.Height - Pixel, pfX * 2,0,pjCenter);
end;
...
//   + '</TD></TR></TABLE>', 0, pfY - Converter.FontHeight - Pixel);
  ...
  WriteDiv(HTMLText(psText), 0, pfY - Converter.FontData.Height, pfX, 0, pjRight);
end;
...
begin
  if AValue = '' then begin
    ...
    i := RavePos('{%REPORT%}', RaveUppercase(AValue));
    if i = 0 then begin
    ...
    //
    //TODO: Pos on uppercase is not efficient.
    i := RavePos('</HEAD', RaveUppercase(FTemplatePre));
    if i = 0 then begin
