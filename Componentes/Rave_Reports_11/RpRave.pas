...
uses
  ...
  RvUtil, RvCmHuff, RvData, RvDirectDataView, RvDataField, RvSecurity,
  RvDatabase, RvDriverDataView, RvDataLink,
  RvLEModule,
  RvLESystem,
  RvCsStd, RvCsRpt, RvCsDraw, RvCsData, RvCsBars;
...
type
  ...
  TRaveDesignerSaveCallbackProc = procedure(Buf: pointer;
                                            Size: longint;
...
type
  ...
  public
    procedure SetBuf(Buf: pointer; Size: longint);
...
begin { RaveSaveCallback }
  Project := TObject(Data) as TRvProject;
  ...
  if Assigned(Project.FOnDesignerSave) then begin
    Project.FOnDesignerSave(Project);
  end; { if }
...
begin { RaveSaveAsCallback }
  ...
  if Assigned(Project.FOnDesignerSaveAs) then begin
    Project.FOnDesignerSaveAs(Project);
  end; { if }
...
function TRaveMemoryStream.Write(const Buffer; Count: longint): longint;
begin { Write }
  Result := 0;
end;  { Write }
...
{$IFDEF Level2006}
  if not FileAge(ProjectFile, FileDate) then begin
    FileDate := 0;
  end;
{$ELSE}
...
var
  ...
  Size: longint;
  SaveProc: TRaveDesignerSaveCallbackProc;
  SaveAsProc: TRaveDesignerSaveCallbackProc;
begin { Design }
  LoadRaveDLL(DLLFile);
  try
    Open;
...
begin { GetReportCategoryList }
  ValidCats := RaveUpperCase(';' + Categories + ';');
  ReportList.Clear;
  with ProjMan.ReportList do begin
    for I1 := 0 to Count - 1 do begin
      Report := TRaveReport(Items[I1]);
      if RavePos(';' + RaveUpperCase(Report.Category) + ';',ValidCats) > 0 then begin
        if FullNames then begin
          ReportList.AddObject(Report.FullName,Report);
        end else begin
          ReportList.AddObject(Report.Name,Report);
