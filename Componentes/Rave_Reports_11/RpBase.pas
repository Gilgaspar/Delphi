...
type
  ...
  TTabList = class(TList)
  public
    procedure Assign(AValue: TList);
  end;
  ...
  protected
  { Internal variables }
    CurrentTabList: TTabList;
    SavePosArr: array[1..MAXSAVEPOS] of TPosition; { Array of saved positions }
    SaveTabsArr: array[1..MAXSAVETABS] of TTabList; { Array of saved tabs }
    SaveFontArr: array[1..MAXSAVEFONT] of TSaveFont; { Array of saved fonts }
    PosStack: TList; { Saved position stack }
    FontStack: TList; { Saved font stack }
    TabsStack: TList; { Saved tabs stack }
    BlockTextOp: boolean;

    BaseFont: TSaveFont; { Saved font settings }
    FontList: TList; { List of fonts that are used }
    ...
    CurrentTab: TTab; { Current tab in list }
    ...
    VLineList: TList; { Contains all vertical lines }
    HLineList: TList; { Contains all horizontal lines }
  ...
  { Property fields }
    ...
    FHyperLink: string; // Hyperlink to print
  ...
  {$ENDIF}
    ...
    procedure WriteHyperlink; virtual;
    procedure GrabFontOnChange; virtual;
    procedure ReleaseFontOnChange; virtual;
    procedure AdjustXY(Width: double);
    procedure ReleaseTabs(var TabList: TTabList);
    procedure UpdateFont;
    procedure InitPrinter;
    procedure CreateLineLists;
    procedure ClearLineLists;
    procedure ClearLineList(var List: TList);
  ...
  { Report Methods - protected }
    ...
    procedure ClearPosArrList;
    procedure ClearFontList;
    procedure ClearFontArrList;
  ...
  public
    ...
    procedure SetBaseFont(var Font: TSaveFont);
  ...
  { Report Text Printing Methods }
    ...
    procedure SetPIVar(VarName: string; VarValue: string); virtual;
  ...
  { Canvas Drawing Object Methods }
    ...
    procedure StartLink(AHyperlink: string);
    procedure EndLink;
  ...
  { Properties }
    ...
    property ParaJustify: TTabJustify read FParaJustify write FParaJustify default tjNone;
    ...
    property NoPrinterPageHeight: double read FNoPrinterPageHeight write FNoPrinterPageHeight;
    property NoPrinterPageWidth: double read FNoPrinterPageWidth write FNoPrinterPageWidth;
  ...
  { Properties - potential published }
    ...
    property LinesPerInch: integer read FLinesPerInch write SetLinesPerInch default 6;
    ...
    property TextBKMode: TBKMode read FTextBKMode write SetTextBKMode default bkTransparent;
    property Copies: integer read FCopies write SetCopies default 1;
    property Orientation: TOrientation read FOrientation write SetOrientation nodefault;
...
var
  ...
  { Misc Inits }
    ...
    FontStack := nil;
    TabsStack := nil;
    for I1 := 1 to MAXSAVEPOS do begin
      SavePosArr[I1] := nil;
      SavePosArr[I1] := nil;
    end; { for }
...
{ Property read/write methods }
  ...
  procedure TBaseReport.SetHyperlink(Value: string);
  begin
    FHyperlink := Value;
    WriteHyperlink;
  end;
...
{ Report Methods }
  ...
  procedure TBaseReport.CreateLineLists;
  begin { CreateLineLists }
    if not Assigned(VLineList) then begin
      VLineList := TList.Create;
    end;
    if not Assigned(HLineList) then begin
      HLineList := TList.Create;
    end;
  end;  { CreateLineLists }
  ...
  begin { ClearLineLists }
    if Assigned(HLineList) then begin
      for I1 := 0 to HLineList.Count - 1 do begin
        TLineList(HLineList.Items[I1]).Free;
      end; { for }
      FreeAndNil(HLineList);
    end; { if }
    if Assigned(VLineList) then begin
      for I1 := 0 to VLineList.Count - 1 do begin
        TLineList(VLineList.Items[I1]).Free;
      end; { for }
      FreeAndNil(VLineList);
    end; { if }
  end;  { ClearLineLists }

  procedure TBaseReport.ClearLineList(var List: TList);
  var
    I1: integer;
  begin
    if Assigned(List) then begin
      for I1 := 0 to List.Count - 1 do begin
        TLineList(List[I1]).Free;
      end; { for }
    end; { if }
    FreeAndNil(List);
  end;
  ...
  const
    ...
    procedure AddLineList(var LineList: TList;
                              X1,X2,Common: double);
    ...
    { same pen settings and that overlap or connect points }
      for I1 := LineList.Count - 1 downto 0 do begin
        LineNode := TLineList(LineList[I1]);
        with LineNode do begin
  ...
  var
    ...
    procedure DrawList(var LineList: TList;
                           Vertical: boolean);
    ...
    begin { DrawList }
      ...
      if Assigned(LineList) then begin
        for I1 := 0 to LineList.Count - 1 do begin
          LineNode := TLineList(LineList.Items[I1]);
          with LineNode do begin
...
{ Printing Methods }
  ...
  begin { PrintJustify }
    case Justify of
      pjCenter: begin
        PrintCenterInch(Text,xPos + (Width / 2.0));
      end;
      pjLeft: begin
        BlockTextOp := true;
        Piece := Text;
        ...
        while FoundPos > 0 do begin { Process substrings }
          ...
          X1 := xPos + (Trunc((X1 - xPos + 0.001) / XU2I(FixedTabWidth)) + 1) *
           XU2I(FixedTabWidth);
          Delete(Piece,1,FoundPos);
          FoundPos := Pos(#9,Piece); { Look for another tab }
        end; { while }
      ...
      pjRight: begin
        PrintRightInch(Text,xPos + Width - Margin);
      end;
      pjBlock: begin
        PrintBlockInch(Text,xPos + Margin,Width - (Margin * 2.0));
      end;
    end; { case }
  end;  { PrintJustify }
  ...
  begin { PrintPiece }
    if not OnTab or not (CurrentTabList.Count > 0) then begin { Print from current pos }
  ...
  begin { PrintCharJustify }
    ...
    PrintJustify(Copy(Text,1,I1 - 1),XU2I(xPos),pjRight,0,0);
    if I1 <= Length(Text) then begin
      PrintJustify(Copy(Text,I1,Length(Text) - I1 + 1),XU2I(xPos),pjLeft,0,0);
    end; { if }
...
{ Positioning Methods }
  ...
  begin { PopPos }
    if Assigned(PosStack) then begin
      TempPos := TPosition(PosStack.Items[PosStack.Count - 1]);
      FCursorXPos := TempPos.X;
      FCursorYPos := TempPos.Y;
      FreeAndNil(TempPos);
      PosStack.Delete(PosStack.Count - 1);
      if PosStack.Count < 1 then begin
        FreeAndNil(PosStack);
      end; { if }
      Result := true;
    end else begin
  ...
  begin { PushPos }
    TempPos := TPosition.Create;
    TempPos.X := FCursorXPos;
    TempPos.Y := FCursorYPos;

    if not Assigned(PosStack) then begin
      PosStack := TList.Create;
    end; { if }
    PosStack.Add(TempPos);
    Result := true;
  end;  { PushPos }
  ...
  begin { SavePos }
    if (Index >= 1) and (Index <= MAXSAVEPOS) then begin
      if Assigned(SavePosArr[Index]) then begin
        SavePosArr[Index].Free;
      end; { if }
      TempPos := TPosition.Create;
      TempPos.X := FCursorXPos;
      TempPos.Y := FCursorYPos;
      SavePosArr[Index] := TempPos;
      Result := true;
    end else begin
      Result := false;
    end; { else }
  ...
  begin { PopTabs }
    if Assigned(TabsStack) then begin
      ReleaseTabs(CurrentTabList);
      ResetTabs;
      CurrentTabList := TTabList(TabsStack[TabsStack.Count - 1]);
      CurrentTab := TTab(CurrentTabList.Items[0]);
      TabsStack.Delete(TabsStack.Count - 1);
      if TabsStack.Count <= 0 then begin
        FreeAndNil(TabsStack);
      end; { if }
      Result := true;
    end else begin
      Result := false;
  ...
  begin { PushTabs }
    if not Assigned(CurrentTabList) then begin
      Result := false;
      Exit;
    end; { if }

    if not Assigned(TabsStack) then begin
      TabsStack := TList.Create;
    end; { if }

    TabList := TTabList.Create;
    TabList.Assign(CurrentTabList);
    TabsStack.Add(TabList);
    Result := true;
  end;  { PushTabs }
  ...
  begin { RestoreTabs }
    ...
    if (Index >= 1) and (Index <= MAXSAVETABS) then begin
      if Assigned(SaveTabsArr[Index]) then begin
        ReleaseTabs(CurrentTabList);
        CurrentTabList := TTabList.Create;
        CurrentTabList.Assign(TTabList(SaveTabsArr[Index]));
        ResetTabs;
        Result := true;
      end;
    end; { if }
  ...
  begin { SaveTabs }
    ...
    if (Index >= 1) and (Index <= MAXSAVETABS) then begin
      ReleaseTabs(TTabList(SaveTabsArr[Index]));
      TabList := TTabList.Create;
      TabList.Assign(CurrentTabList);
      SaveTabsArr[Index] := TabList;
      Result := true;
    end;
  end;  { SaveTabs }
  ...
  procedure TBaseReport.SetBaseFont(var Font: TSaveFont);
  ...
  begin { PopFont }
    ...
    if Assigned(FontStack) then begin
      TempFont := TSaveFont(FontStack.Items[FontStack.Count -1]);
      SetBaseFont(TempFont);
      FreeAndNil(TempFont);
      FontStack.Delete(FontStack.Count - 1);

      if FontStack.Count < 1 then begin
  ...
  begin { PushFont }
    ...
    if not Assigned(FontStack) then begin
      FontStack := TList.Create;
    end;

    TempFont := TSaveFont.Create;
    TempFont.Assign(BaseFont);
    FontStack.Add(TempFont);
  end;  { PushFont }
  ...
  begin { SaveFont }
    if (Index >= 1) and (Index <= MAXSAVEFONT) then begin
      TempFont := TSaveFont.Create;
      TempFont.Assign(BaseFont);
      if Assigned(SaveFontArr[Index]) then begin
        FreeAndNil(SaveFontArr[Index]);
      end;
      SaveFontArr[Index] := TempFont;
      Result := true;
    end else begin
      Result := false;
    end; { else }
...
{ Canvas Drawing Methods }
  ...
  procedure TBaseReport.StartLink(AHyperlink: string);
  begin { StartLink }
    HyperLink := AHyperlink;
  end;  { StartLink }

  procedure TBaseReport.EndLink;
  begin { EndLink }
    Hyperlink := '';
  end;  { EndLink }

  function TBaseReport.MakeLink(ADisplayText: string; AHyperlink: string): string;
  begin { MakeLink }
    Result := RPTFPrefix + RPTFHyperlink + AHyperLink + RPTFSuffix;
    Result := Result + ADisplayText + RPTFPrefix + RPTFHyperlink + RPTFSuffix;
  end;  { MakeLink }
...
{ Tab Methods }
  ...
  begin { SetTab }
    Tab := TTab.Create;
    with Tab do begin
      if not Assigned(CurrentTabList) then begin { No tabs in list yet }
        CurrentTabList := TTabList.Create;
        BeforeTabs := true;
      end; { else }

      if NewPos < 0 then begin { Append from previous tab, if any }
        TabPos := CurrentTabList.IndexOf(CurrentTab);
        if TabPos >= 0 then begin
          PrevTab := TTab(CurrentTabList.Items[TabPos]);
          Pos := PrevTab.Pos + PrevTab.Width;
        end else begin
          Pos := FLeftMargin;
        end; { else }
      end else begin
    ...
    CurrentTab := Tab;
    CurrentTabList.Add(CurrentTab);
  end;  { SetTab }

  procedure TBaseReport.ReleaseTabs(var TabList: TTabList);
  var
    I1: integer;
  begin { ReleaseTabs }
    if not Assigned(TabList) then Exit;
    for I1 := 0 to TabList.Count - 1 do begin
      TTab(TabList[I1]).Free;
    end; { for }
    FreeAndNil(TabList);
  end;  { ReleaseTabs }
  ...
  begin { ClearAllTabs }
    ClearTabs;
    for I1 := 1 to MAXSAVETABS do begin
      ReleaseTabs(TTabList(SaveTabsArr[I1]));
    end; { for }
  end;  { ClearAllTabs }
  ...
  begin { Tab }
    ...
    if BeforeTabs then begin { Get first tab }
      if Assigned(CurrentTabList) then begin
        CurrentTab := TTab(CurrentTabList.Items[0]);
      end else begin
        CurrentTab := nil;
      end;
      BeforeTabs := false;
    end else if (Assigned(CurrentTabList) and Assigned(CurrentTab) and (CurrentTabList.IndexOf(CurrentTab) < CurrentTabList.Count - 1)) then begin { Get next tab }
      CurrentTab := TTab(CurrentTabList.Items[CurrentTabList.IndexOf(CurrentTab) + 1]);
    end else begin
      CurrentTab := nil;
    end;
  ...
  function TBaseReport.GetTab(Index: integer): TTab;
  begin { GetTab }
    Result := nil;
    if Index = 0 then begin
      Result := CurrentTab;
    end else begin
      if Assigned(CurrentTabList) and (Index <= CurrentTabList.Count) then begin
        Result := TTab(CurrentTabList.Items[Index - 1]);
      end; { if }
    end; { if }
  end;  { GetTab }
...
{ Memo Methods }
  ...
  procedure TBaseReport.ClearPosArrList;
...
{ Font Routines }
  ...
  begin { ClearFontList }
    ...
    while Assigned(FontList) do begin
      TempFont := TSaveFont(FontList.Items[0]);
      if TempFont.Handle <> 0 then begin
        DeleteObject(TempFont.Handle);
      end; { if }
      TSaveFont(FontList.Items[0]).Free;
      FontList.Delete(0);
      if FontList.Count < 1 then begin
        FreeAndNil(FontList);
      end; { if }
    end; { while }
  end;  { ClearFontList }

  procedure TBaseReport.ClearFontArrList;
  ...
  begin { ClearFontList }
    for FontIndex := 1 to MAXSAVEFONT do begin
      if Assigned(SaveFontArr[FontIndex]) then begin
        TempFont := SaveFontArr[FontIndex];
        if TempFont.Handle <> 0 then begin
          DeleteObject(TempFont.Handle);
        end; { if }
        FreeAndNil(SaveFontArr[FontIndex]);
      end; { if }
    end;
  end;  { ClearFontList }
  ...
  { Search for BaseFont in FontList }
    Fonts := 0;
    if (BaseFont.Handle = 0) and Assigned(FontList) then begin
      for FontIndex := 0 to FontList.Count - 1 do begin
        TempFont := TSaveFont(FontList.Items[FontIndex]);
        Inc(Fonts);
        with TempFont do begin
          if (Name = BaseFont.Name) and (Pitch = BaseFont.Pitch) and
           ...
           (Charset = BaseFont.Charset) then begin
            ...
            FontList.Move(FontIndex, 0); { Move font to front of list }
            Break;
          end; { if }
        end; { with }
      end; { for }
  ...
  { Assign or create new font handle }
    ...
    { Save BaseFont on FontList }
      TempFont := TSaveFont.Create;
      TempFont.Assign(BaseFont);
      if not Assigned(FontList) then begin
...
{ Set to virtual and override in TFileReport }
  ...
  procedure TBaseReport.WriteHyperlink;
  begin { WriteHyperlink }
  end;  { WriteHyperlink }
...
procedure TTabList.Assign(AValue: TList);
var
  Tab: TTab;
  I1: integer;
begin
  Count := AValue.Count;
  for I1 := 0 to AValue.Count - 1 do begin
    Tab := TTab.Create;
    Tab.Assign(TTab(AValue[I1]));
    self[I1] := Tab;
  end; { for }
end;
