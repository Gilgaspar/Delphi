...
{ class TRvSystem }
  ...
  published
    ...
    property SystemOptions: TSystemOptions read FSystemOptions write FSystemOptions
     default [soShowStatus,soAllowPrintFromPreview,soAllowSaveFromPreview,soPreviewModal];
...
begin { Create }
  ...
  FSystemSetups := [ssAllowSetup,ssAllowCopies,ssAllowCollate,ssAllowDuplex,
   ssAllowDestPreview,ssAllowDestPrinter,ssAllowDestFile,ssAllowPrinterSetup,
   ssAllowPreviewSetup];
  FSystemOptions := [soShowStatus,soAllowPrintFromPreview,soAllowSaveFromPreview,soPreviewModal];
...
begin { Execute }
  ...
  if not Aborted then begin
    ...
    try
      if SystemFiler.StreamMode <> smFile then begin
        SystemFiler.FileName := ''; // Clear to prevent old ndr being used
      end; { if }
      ...
      if SystemFiler.StreamMode = smMemory then begin
        FreeAndNil(FSystemFiler.FStream);
      end else if SystemFiler.StreamMode = smTempFile then begin { Erase temp file }
        DeleteFile(SystemFiler.FileName);
      end; { if }
    end; { tryf }
  end; { if }
end;  { Execute }
