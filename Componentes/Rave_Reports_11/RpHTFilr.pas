...
uses
  ...
  Forms, Dialogs, StdCtrls, ExtCtrls, RpDefine, RpBase, RpDevice {$IFDEF LEVELXE4}, AnsiStrings{$ENDIF};
...
begin { Tab }
  if OnTab then begin
    ...
    if (not FTableTagStarted) and (not FInTabCell) then begin
      ...
      WriteStr('<td width="' + IntToStr(round(GetTab(1).Pos * ScreenDPI)) + '">' + CrLfStr);
      WriteStr('&nbsp;</td>' + CrLfStr);
      WriteStr('<td>' + CrLfStr + CrLfStr);

      if Trim(FTableAttributes) = '' then begin
        if GetTab(1).Left or GetTab(1).Right or GetTab(1).Top or GetTab(1).Bottom then begin
