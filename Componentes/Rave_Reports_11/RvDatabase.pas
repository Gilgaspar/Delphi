...
{ If global list of data links is allocated and at least one link }
  ...
  { We have not loaded data links or there were no data links when we loaded }
    S1 := LinkType;
    if S1 = 'IBX' then begin
      S1 := 'Interbase';
    end; { if }
    raise Exception.Create('No DataLink drivers have been loaded.  Use the DataLink' + S1 +
     '.RVD driver file for the Rave server/IDE or include the unit RvDL' + S1 + ' for applications.');
  end; { else }
end;
