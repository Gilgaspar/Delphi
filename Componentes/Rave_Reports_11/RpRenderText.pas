...
type
  ...
  protected
    ...
    procedure AddTextItem(var TextItem: TTextItem; Line: integer);
...
procedure TRvRenderText.AddTextItem(var TextItem: TTextItem; Line: integer);
var
  I1: integer;
begin { AddTextItem }
  ...
  with TList(PageList[Line - 1]) do begin { Find insert spot }
    for I1 := 0 to (Count - 1) do begin
      if TTextItem(Items[I1]).Col > TextItem.Col then begin
        Insert(I1,TextItem);
        Exit;
      end; { if }
    end; { for }
...
{ Process through PageList and output TextItems }
  for Line := 1 to PageList.Count do begin
    LineList := TList(PageList[Line - 1]);
    CurrentPos := 1;
    for I1 := 0 to LineList.Count - 1 do begin
      TextItem := TTextItem(LineList[I1]);
      with TextItem do begin
