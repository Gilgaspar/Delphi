...
uses
  RpDefine, RvUtil, SysUtils, TypInfo, Dialogs;
...
begin
  ...
  {if now at end of result set}
    ...
    {if not at end of result set}
      ...
      {if not saving then use data view row buffer}
        DR := DataRow;
      end; { else }

      {transfer result set buffer into row buffer}
      DR.Init(RowBufSize);
      ReadBuf(DR.DataPtr^,RowBufSize);
  ...
  begin
    repeat
      ...
      if I1 > 0 then begin
        ...
        while CharInSet(AString[I2],['a'..'z','A'..'Z','0'..'9','_']) do begin
          Inc(I2);
        end;
