...
type
  ...
  TRaveDataRow = class
  protected
    function ReadInt(var CalcPtr: PChar): integer;
    function ReadStr(CalcPtr: PChar): string;
  public
    ...
    function GetNativeStr(FieldIdx: integer): string;
    function GetFormattedStr(FieldIdx: integer): string;
...
{ class TRaveDataRow }
  ...
  function TRaveDataRow.ReadInt(var CalcPtr: PChar): integer;
  begin
    Result := longint(pointer(CalcPtr)^);
    CalcPtr := pointer(integer(CalcPtr) + SizeOf(longint));
  end;

  function TRaveDataRow.ReadStr(CalcPtr: PChar): string;
  var
    Len: integer;
    StringType: integer;
  {$IFDEF Unicode}	
  ...
  begin { ReadStr }
    Result := '';
    if CalcPtr = nil then Exit;
  {$IFDEF UNICODE}
    HaveUnicode := false;
  {$ENDIF}
    StringType := ReadInt(CalcPtr);
    while StringType > 0 do begin
      Len := ReadInt(CalcPtr);
      if StringType = 1 then begin // MBCS
      {$IFDEF UNICODE}
        if not HaveUnicode then begin // MBCS
          if Len = 0 then begin
            Result := '';
          end else begin
            SetLength(S1, Len);
            Move(CalcPtr^, S1[1], Len);
            Result := string(S1);
          end;
        end;
      {$ELSE}
        SetLength(Result,Len);
        Move(CalcPtr^,Result[1],Len);
      {$ENDIF}
        CalcPtr := PChar(integer(CalcPtr) + Len); // move CalcPtr past string data
  ...
  begin { GetNativePtr }
    ...
    I1 := integer(pointer(integer(DataPtr) + (FieldIdx * 8))^);
    if I1 >= 0 then begin
      Result := pointer(integer(DataPtr) + I1);
    end; { if }
  end;  { GetNativePtr }
  ...
  begin { GetFormattedPtr }
    I1 := integer(pointer(integer(DataPtr) + (4 + FieldIdx * 8))^);
    if I1 = 0 then begin
      Result := nil;
    end else begin
      Result := pointer(integer(DataPtr) + I1);
  ...
  function TRaveDataRow.GetNativeStr(FieldIdx: integer): string;
  begin { GetNativeStr }
    Result := ReadStr(GetNativePtr(FieldIdx));
  end;  { GetNativeStr }

  function TRaveDataRow.GetFormattedStr(FieldIdx: integer): string;
  begin { GetFormattedStr }
    Result := ReadStr(GetFormattedPtr(FieldIdx));
  end;  { GetFormattedStr }
  ...
  begin { GetNativeData }
    ...
    if Assigned(CalcPtr) then begin
      ...
      CalcPtr := pointer(integer(CalcPtr) + 4);
    end else begin
      CalcSize := 0;
    end; { else }
  end;  { GetNativeData }
...
(*
  ...
  {$IFDEF CLR}
    I1 := GetFormattedPtr(FieldIdx);
    if I1 > 0 then begin
      I2 := BitConverter.ToInt32(DataPtr,I1);
      Inc(I1,4);
      Result := UnicodeEncoding.GetString(DataPtr,I1,I2);
    end else begin
      Result := '';
    end; { else }
  {$ELSE}
...
{ class TRaveDataField }
  ...
  begin { GetAsString }
    Result := (Parent as TRaveBaseDataView).ActiveDataRow.GetFormattedStr(DataIndex);
  end;  { GetAsString }
  ...
  begin { GetAsBlob }
    ...
    if Assigned(Result) and (DataType in [dtString,dtBlob,dtMemo,dtGraphic]) then begin
      Result := pointer(integer(Result) + 4);
    end; { if }
  end;  { GetAsBlob }
  ...
  begin { GetDataSize }
    case DataType of
      ...
      dtDateTime: Result := SizeOf(TDateTime);
        ...
        (Parent as TRaveBaseDataView).ActiveDataRow.GetNativeData(DataIndex,DataPtr,Result);
      end;
    end; { case }
  end;  { GetDataSize }
  ...
  begin { GetAsString }
    if IsNull then begin
      ...
      if Result = '' then begin { Get data from native format }
        Result := (Parent as TRaveBaseDataView).ActiveDataRow.GetNativeStr(DataIndex);
      end; { if }
    end; { else }
  end;  { GetAsString }
...
{ class TRaveBlobField }
  ...
  begin { GetAsString }
    if IsNull then begin
      ...
      (Parent as TRaveBaseDataView).ActiveDataRow.GetNativeData(DataIndex,CalcPtr,CalcSize);
      SetLength(Result,CalcSize{$IFDEF Unicode} div 2{$ENDIF}); 
