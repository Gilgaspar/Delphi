...
uses
  Windows,
  Messages, VCL.Controls, VCL.Graphics, VCL.Dialogs,
...
begin { InitMemoBuf }
  if not Prepared then begin
    if Assigned(MemoBuf) then begin
      FreeAndNil(MemoBuf);
    end; { if }
    MemoBuf := TMemoBuf.Create;
    MemoBuf.BaseReport := TRaveProjectManager(Project).BaseReport;
    ...
    if ContainsRTF or (RaveCompareText(Copy(Trim(Text),1, 5),'{\rtf') = 0) then begin
...
begin { InitData }
  if not Assigned(MemoBuf) or
   (Assigned(MemoBuf) and (MemoBuf.MemoLinesLeft <= 0)) then begin
    Prepared := false;
  end;
  InitMemoBuf;
end;  { InitData }
