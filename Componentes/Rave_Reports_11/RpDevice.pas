...
type
  ...
  { Capability methods }
    procedure SetBin(BinNum: integer); virtual;
		function GetBin: integer; virtual;
  ...
  { Output methods }
    ...
    procedure RawOut(var ABuffer; ALength: word); virtual;
  ...
  { Device properties }
    ...
    property Bin: integer read GetBin write SetBin;
...
*)
  ...
  protected
    FIgnoreNDROrientation: boolean;
    FSetupOrientation: TOrientation;
    FIgnoreNDRBin: boolean;
    FSetupBin: integer;
  ...
  { Capability methods }
    procedure SetBin(BinNum: integer); override;
  ...
  { Setup properties }
    property SetupBin: integer read FSetupBin write FSetupBin;
    property IgnoreNDRBin: boolean read FIgnoreNDRBin write FIgnoreNDRBin;
...
procedure TRPBaseDevice.SetBin(BinNum: integer);
begin
end;

function TRPBaseDevice.GetBin: integer;
begin
  Result := RpDev.DevMode.dmDefaultSource;
end;
...
function DeviceCapabilities; external winspl name {$IFDEF UNICODE}{Trans-}'DeviceCapabilitiesW'{$ELSE}{Trans-}'DeviceCapabilitiesA'{$ENDIF};
...
{ Allocate DevNames structure }
  DeviceNames := GlobalAlloc(GHND,SizeOf(TDevNames) + 256 * 3 + 5); // Allocate space for 256 bytes each
  try
...
begin { SetDeviceIndex }
  ...
  if (Value <> FDeviceIndex) and (Value < Printers.Count) then begin
    if Value < 0 then begin { Set to default printer }
      ...
      try
        GetProfileString({Trans-}'WINDOWS',{Trans-}'DEVICE','',DeviceBuf,DeviceBufSize);
...
{ Get the selected RPDeviceItem }
  ...
  FDeviceName := StrNew(TRPDeviceItem(FPrinterList.Objects[FDeviceIndex]).FDeviceName);
  FDriverName := StrNew(TRPDeviceItem(FPrinterList.Objects[FDeviceIndex]).FDriverName);
  FOutputName := StrNew(TRPDeviceItem(FPrinterList.Objects[FDeviceIndex]).FOutputName);
...
{ Get printer driver handle }
  ...
  { Figure out DevSize }
    TempDevMode := nil;
    DevSize := DocumentProperties(0,FDriverHandle,FDeviceName,TempDevMode^,TempDevMode^,0);
...
{ Release canvas }
  FCanvas.Handle := 0;
//!!!TODO!!! Causes error in .NET when quitting app
...
begin { GetBins }
  if FBinList = nil then begin
    ...
    try
      try
        if DeviceCapabilities(FDeviceName,FOutputName,DC_BINNAMES,PChar(BinBuf),nil) >= 0 then begin
          DeviceCapabilities(FDeviceName,FOutputName,DC_BINS,PChar(BinNumBuf),nil);
          for Bin := 1 to Bins do begin
            BinNum := BinNumBuf^[Bin];
            //!!PORT!! Added StrPas back in to get it to compile.  MBCS issue
            FBinList.AddObject(BinBuf^[Bin],pointer(BinNum));
...
procedure TRPPrinterDevice.SetBin(BinNum: integer);
begin { SetBin }
    RpDev.DevMode.dmDefaultSource := BinNum;
    RpDev.DevMode^.dmFields := RpDev.DevMode^.dmFields or DM_DEFAULTSOURCE;
    RpDev.DevModeChanged := true;
end;  { SetBin }
...
{ Look for BinName in FBinList }
  ...
  for Bin := 1 to FBinList.Count do begin
    if Exact then begin
      ...
      Found := (RavePos(RaveUpperCase(BinName),RaveUpperCase(FBinList[Bin - 1])) > 0);
    end; { else }
    if Found then begin
...
begin { GetPapers }
  if FPaperList = nil then begin
    ...
    try
      try
        if DeviceCapabilities(FDeviceName,FOutputName,DC_PAPERNAMES,PChar(PaperBuf),nil) >= 0 then begin
          DeviceCapabilities(FDeviceName,FOutputName,DC_PAPERS,PChar(PaperNumBuf),nil);
          for Paper := 1 to Papers do begin
            PaperNum := PaperNumBuf^[Paper];
            //!!PORT!! Added StrPas back in to get it to compile.  MBCS issue
            FPaperList.AddObject(PaperBuf^[Paper],pointer(PaperNum));
...
{ Look for PaperName in FPaperList }
  ...
  for Paper := 1 to FPaperList.Count do begin
    if Exact then begin
      ...
      Found := (RavePos(RaveUpperCase(PaperName),RaveUpperCase(FPaperList[Paper - 1])) > 0);
    end; { else }
    if Found then begin
    ...
    try
      ...
      try
        ...
        for I1 := 0 to Returned - 1 do begin
          if Win32Platform = VER_PLATFORM_WIN32_NT then begin
            S1 := PPrinterInfo4(PrinterInfo)^.pPrinterName;
            FPrinterList.AddObject(S1,TRPDeviceItem.Create(S1,nil,nil));
            PrinterInfo := pointer(integer(PrinterInfo) + SizeOf(TPrinterInfo4));
          end else begin
            S1 := PPrinterInfo5(PrinterInfo)^.pPrinterName;
            Ports := PPrinterInfo5(PrinterInfo)^.pPortName;

            Port := GetWord(Ports);
            while Assigned(Port) do begin
              FPrinterList.AddObject(Format({Trans-}'%s on %s',[S1,Port]),TRPDeviceItem.Create(S1,nil,Port));
              Port := GetWord(Ports);
            end; { while }
            PrinterInfo := pointer(integer(PrinterInfo) + SizeOf(TPrinterInfo5));
          end; { else }
        end; { for }
      finally
    ...
    with TRPDeviceItem(FPrinterList.Objects[Printer - 1]) do begin
      if Exact then begin
        if Pos({Trans-}' on ',PrinterName) > 0 then begin
          Found := (RavePos(RaveUpperCase(PrinterName),
           RaveUpperCase(StrPas(FDeviceName) + {Trans-}' on ' + StrPas(FOutputName))) > 0);
        end else begin
          Found := (FDeviceName = PrinterName);
        end; { else }
      end else begin
        Found := (RavePos(RaveUpperCase(PrinterName),
         RaveUpperCase(FDeviceName + {Trans-}' on ' + FOutputName)) > 0);
      end; { else }
    end; { with }
    if Found then begin
...
begin { GetOrientation }
  State := dsIC;
  if DevMode <> nil then begin
    Result := TOrientation(DevMode.dmOrientation - 1);
  end else begin
...
begin { GetDuplex }
  if (DevMode <> nil) and (DevMode.dmDuplex in [1..3]) then begin
    Result := TDuplex(DevMode.dmDuplex - 1);
  end else begin
    Result := dupSimplex;
  end; { else }
end;  { GetDuplex }
...
begin { SetPaperSize }
  ...
  if (Width <> 0) and (Height <> 0) then begin
    if (DevMode.dmPaperSize <> DMPAPER_USER) or
     (DevMode.dmPaperWidth <> Width) or
     (DevMode.dmPaperLength <> Height) then begin
