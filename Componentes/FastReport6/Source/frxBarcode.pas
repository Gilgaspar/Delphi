...
begin
  ...
  else if BarType = bcPharmacode then
    FText := '123456'
  else if BarType = bcDeutsche_Post_Identcode then
    FText := '01234567890'
  else if BarType = bcDeutsche_Post_Leitcode then
  begin
    FText := '0123456789012' ;
    FCalcCheckSum := True;
  end
  ...
  try
    ...
    begin
      if FBarCode.Typ = bcCodeUSPSIntelligentMail then
        ...
        FText := '(01)12345678901234'
      else if BarCode.Typ = bcPharmacode then
        FText := '123456'
      else if BarType = bcDeutsche_Post_Identcode then
        FText := '01234567890'
      else if BarType = bcDeutsche_Post_Leitcode then
...
{$ENDIF}
  ...
  frxObjects.RegisterObject1(TfrxBarcodeView, nil, 'Pharmacode', 'Barcode', 27, 23);
  frxObjects.RegisterObject1(TfrxBarcodeView, nil, 'Deutsche Post Identcode', 'Barcode', 28, 23);
  frxObjects.RegisterObject1(TfrxBarcodeView, nil, 'Deutsche Post Leitcode', 'Barcode', 29, 23);
{$IFNDEF RAD_ED}
{$IFNDEF ACADEMIC_ED}
