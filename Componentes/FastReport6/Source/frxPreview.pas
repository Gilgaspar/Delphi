...
uses
  ...
  ToolWin, frxPreviewPages, frxClass, frxBaseForm, frxSearchForm, frxComCtrls, System.Actions, Vcl.ActnList
{$IFDEF DELPHI17}
  , System.Actions
{$ENDIF}
{$IFDEF FPC}
...
uses
  ...
  frxPreviewPageSettings, frxDMPClass, frxInPlaceEditors, frxIOTransportIntf, frxIOTransportDialog, frxPlatformServices{$IFDEF ISVISION}, fmPreview, uEtiqWMS{$ENDIF};
...
{$IFDEF ISVISION}
  //desviar para impressão zpl ou ppla no caso de etiquetas wms
    if DMEtiqWMS.ConfigWMS.ConfigImpressao.Caller = tcEtiqueta then
    begin
      if DMEtiqWMS.ConfigWMS.ConfigImpressao.Modo in [miZPL, miPPLA] then
      begin
        try
          DMEtiqWMS.PrintScriptLabel;
        finally
          Unlock;
        end;
        Exit;
...
begin
  if Locked or (PageCount = 0) then Exit;
...
{$IFNDEF FPC}
  ZoomCB.Resize;
  Toolbar.ButtonHeight := 0;//Toolbar.Images.Height;
  Toolbar.ButtonWidth := 0;// Toolbar.Images.Width;
  //Sep3.AutoSize := True;
...
{$ELSE}
  ...
  Toolbar.ButtonHeight := Toolbar.Images.Height + 7;
