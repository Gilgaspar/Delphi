...
uses
  ...
  frxClass, Menus, ComCtrls, frxBaseForm, frxCtrls
  {$IFDEF FPC}
...
type
  ...
  TfrxObjectInspector = class(TfrxBaseForm)
    ObjectsCB: TComboBox;
...
begin
  ...
  frxDrawIcon(ImgList, FFilterEdit.BitmapActive, 121);
  frxDrawIcon(ImgList, FFilterEdit.BitmapUnactive, 122);
...
begin
  if not Assigned(FDesigner) then Exit;
    ...
    ItemIndex := n;
    p := GetItem(ItemIndex);
    if p.Editor.PropInfo^.PropType^.Kind in [tkString, tkLString, tkWString{$IFDEF Delphi12}, tkUString{$ENDIF}
