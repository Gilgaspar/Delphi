...
begin
  ...
  begin
    ...
    aScale := bSize / frx_DefaultBtnSize;
    DrawButtonFace(aCanvas, r, Round(aScale), bsNew, False, False, False);
...
begin
  Result := False;
  bSize := GetButtonSize;
  if not((X >= aRect.Right - bSize) and
    (X <= aRect.Right) and (Y >= aRect.Top) and
    (Y <= aRect.Top + bSize)) then
    Exit;
...
{$ENDIF}
    ...
    r.Top := aRect.Top + bSize + 2;
    r.Right := aRect.Right;
    r.Left := r.Right - Round(140 * FDevicePPI / frx_DefaultPPI);
    r.Bottom := r.Top + Round(162 * FDevicePPI / frx_DefaultPPI);// * FDevicePPI / frx_DefaultPPI);
    ...
    FPopupForm.SetBounds(p.X, p.Y, Round((r.Right - r.Left) / (FDevicePPI / frx_DefaultPPI)), Round((r.Bottom - r.Top) / (FDevicePPI / frx_DefaultPPI)));
