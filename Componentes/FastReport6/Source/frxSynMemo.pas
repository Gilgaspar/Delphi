...
uses
  ...
  Forms, frxCtrls, fs_iparser, frxPopupForm, fs_xml, fs_iinterpreter, Menus, frxRes
  {$IFDEF FPC}
...
{$ENDIF}
    ...
    procedure BuildCClist(const sName: String);
    procedure DoCtrlI;
    procedure DoCtrlU;
    procedure DoCtrlR;
    procedure DoCtrlL;
...
procedure TfrxSyntaxMemo.BuildCClist(const sName: String);
var
  members: TStringList;
  i: Integer;
  Item: TfrxCompletionItem;
...
begin
  inherited;
  FGutterWidth := MulDiv(FGutterWidth, aNewPPI, FCurrentPPI);
end;
...
function TfrxSyntaxMemo.GetCompletionString: String;
  ...
  begin
    Result := '';
    s := LineAt(FPos.Y - 1, False);
    s := frxCopy(s, 1, FPos.X - 1);
...
var
  ...
  begin
    ...
    begin
      ...
      defGW := MulDiv(DefGutterWidth, FCurrentPPI, 96);
    ...
    if IsBookmark(ALine) >= 0 then
      ...
      begin
        ...
        Font.Size := 7;
        Canvas.Font.Height := MulDiv(Canvas.Font.Height, FCurrentPPI, Canvas.Font.PixelsPerInch);
        tw := MulDiv(10, FCurrentPPI, 96);
        h := MulDiv(11, FCurrentPPI, 96);
        Brush.Color := clBlack;
...
begin
  FShowGutter := Value;
  if Value then
    FGutterWidth := MulDiv(DefGutterWidth, FCurrentPPI, 96) else
    FGutterWidth := 0;
{$IFDEF FPC}
      frxUpdateControl(Self);
{$ELSE}
