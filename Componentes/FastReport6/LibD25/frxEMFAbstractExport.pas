...
type
  ...
  private
    ...
    procedure TransformPoint(var DP: TfrxPoint; IsScalingOnly: Boolean = False);
  ...
  protected
    ...
    function MonoBrushAverageColor(ih: LongWord): TColor;
    ...
    function LogToDevPoint(LP: TSmallPoint; IsScalingOnly: Boolean = False): TfrxPoint; overload;
    function LogToDevPoint(LP: TPoint; IsScalingOnly: Boolean = False): TfrxPoint; overload;
    function LogToDevPoint(LP: TfrxPoint; IsScalingOnly: Boolean = False): TfrxPoint; overload;
    function LogToDevPoint(LP: TDoublePoint; IsScalingOnly: Boolean = False): TfrxPoint; overload;
    function LogToDevPoint(X, Y: Extended; IsScalingOnly: Boolean = False): TfrxPoint; overload;
...
begin
  ...
  if ShowComments then
    ...
    begin
      ...
      SetLength(A, PBI^.bmiHeader.biSizeImage div SizeOf(LongWord));
      Move(Bytes[CreateMonoBrush.offBits], A[0], Length(A) * SizeOf(A[0]));
      Parsing := 'EMR_CreateMonoBrush' + Dlm +
        ...
        Format('biBitCount: %u', [PBI^.bmiHeader.biBitCount]) + Dlm + // Specifies the number of bits per pixel (bpp)
...
function TEMFAbstractExport.LogToDevPoint(LP: TDoublePoint; IsScalingOnly: Boolean = False): TfrxPoint;
begin
  Result := LogToDevPoint(LP.X, LP.Y, IsScalingOnly);
end;

function TEMFAbstractExport.LogToDevPoint(LP: TfrxPoint; IsScalingOnly: Boolean = False): TfrxPoint;
begin
  Result := LogToDevPoint(LP.X, LP.Y, IsScalingOnly);
end;

function TEMFAbstractExport.LogToDevPoint(LP: TPoint; IsScalingOnly: Boolean = False): TfrxPoint;
begin
  Result := LogToDevPoint(LP.X, LP.Y, IsScalingOnly);
end;

function TEMFAbstractExport.LogToDevPoint(LP: TSmallPoint; IsScalingOnly: Boolean = False): TfrxPoint;
begin
  Result := LogToDevPoint(LP.X, LP.Y, IsScalingOnly);
end;

function TEMFAbstractExport.LogToDevPoint(X, Y: Extended; IsScalingOnly: Boolean = False): TfrxPoint;
begin
  Result.X := LogToDevX(X);
  Result.Y := LogToDevY(Y);

  TransformPoint(Result, IsScalingOnly);
end;
...
function TEMFAbstractExport.MonoBrushAverageColor(ih: LongWord): TColor;
begin
  Result := FDC.MonoBrushAverageColor(ih, FEMRLastCreated);
end;
...
procedure TEMFAbstractExport.TransformPoint(var DP: TfrxPoint; IsScalingOnly: Boolean = False);
begin
  if EnableTransform then
    if IsScalingOnly then
...
const
  ...
  function Bit1InWord(W: Word): Integer;
  var
    i: Integer;
  begin
    Result := 0;
    if W <> 0 then
      for i := 0 to WordBit - 1 do
        if Odd(W shr 1) then Result := Result + 1;
  end;
...
begin
  if EnhMetaObjArray[ih] = nil then
    ...
    begin
      ...
      Bit1Count := 0;
      for i := 0 to High(A) do
        Bit1Count := Bit1Count + Bit1InWord(A[i]);
      Part1 := Bit1Count / (Length(A) * WordBit);
      Result := RGB(
