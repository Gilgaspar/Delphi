...
{$ENDIF}
  ...
  frxImageConverter, frxExportBaseDialog, frxPlatformServices
{$IFDEF DELPHI16}
, System.UITypes
{$ENDIF}
;
...
{$ENDIF}
  //Result := Format('<w:rPr><w:rFonts w:ascii="%s" w:hAnsi="%s" w:cs="%s" w:eastAsia="%s" w:hint="eastAsia"/>%s<w:bCs/>%s%s<w:color w:val="#%.6X"/><w:w w:val="105"/>' +
    ...
    [FontName, FontName, FontName, FontName, BStr(fsBold in f.Style, '<w:b/>'), BStr(fsItalic in f.Style, '<w:i/>'), BStr(fsStrikeOut in f.Style, '<w:strike/>'), RGBSwap(f.Color), f.Size * 2, f.Size * 2, BStr(fsUnderline in f.Style, '<w:u w:val="single"/>'), st, BStr(RTL, '<w:rtl/>')]);
  end;
    ...
    try
      ...
      repeat
        ...
        begin
          ...
          iPos := i;
          if sChunk <> '' then
            AWriter.Write(['<w:r>', GetFont(AStyle.Font, ASubType, AObj.RTL), '', sChunk, '</w:r>'], {$IFDEF Delphi12}True{$ELSE}False{$ENDIF});
        end;

        if bEOL then
        begin
          iPos := i + Length(sLineBreak);
          AWriter.Write(['<w:r>', GetFont(SaveFont, SaveSubType, AObj.RTL), BStr(i < Length(sw) - 1, '<w:br/>'), '</w:r>'], {$IFDEF Delphi12}True{$ELSE}False{$ENDIF});
        end;
  ...
  function GetPicture(m: {$IFDEF FPC}TBitMap{$ELSE}TMetafile{$ENDIF}; URL: string = ''): string;
  var
    w, h: Integer;
    id, pic: string;
  begin
