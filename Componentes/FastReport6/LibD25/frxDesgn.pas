...
uses
  ...
  frxPictureCache, Variants, frxDPIAwareBaseControls
{$IFDEF Delphi17}
 , System.Actions
{$ENDIF}
{$IFDEF Delphi9}
...
uses
  ...
  frxRegistredEditorsDialog, frxIOTransportDialog, frxDPIAwareInt;
...
begin
  ...
  while (Result <> nil) and not (Result is TForm) do
    Result := Result.Parent;
  if not(Result is TForm) then Result := Self;
end;
...
{$IFDEF FPC}
  ...
  PanelTB3.Left := SepTB20.Left + SepTB20.Width;
  PanelTB2.Left := FrameStyleB.Left + FrameStyleB.Width;
  StandardTB.Width := SepTB20.Left + SepTB20.Width + PanelTB3.Width;
...
begin
  ...
  CodeWindow.SetScaledFontSize(Ini.ReadInteger(lName, 'ScriptFontSize', 10));
...
var
  ...
  Ini.WriteInteger(lName, 'ScriptFontSize', CodeWindow.GetScaledFontSize);
  ...
  {$IFNDEF FPC}
  frxSaveFormPosition(Ini, FWatchList, CurrentFormPPI / frx_DefaultPPI);
  {$ELSE}
...
begin
  if Assigned(FPage) and FPage.IsSupportsGuidlines then
  begin
