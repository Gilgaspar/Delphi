...
{$ENDIF}
  ...
  frxBarcodeAztec, frxBarcodeMaxiCode, frxGS1Databar, frxBarcode2DBase, frxBarcodeProperties, frx2DBarcodesPresets;

type

  // Типы штрих-кодов ////////////////////////////////////////////////////////////////////////
  TfrxBarcode2DType = (bcCodePDF417, bcCodeDataMatrix, bcCodeQR, bcCodeAztec, bcCodeMaxiCode, bcGS1DatabarE, bcGS1DatabarES);
...
{$IFNDEF NO_EDITORS}
  frxBarcode2DEditor,
{$ENDIF}
  frxRes, frxUtils, frxDsgnIntf, frxBarcode2DRTTI, Math, frxQRCodeSwissPreset, frxQRCodeSberPreset;
...
function TfrxBarcode2DView.GetVectorGraphic(DrawFill: Boolean): TGraphic;
begin
  CalcSize;
...
begin
  ...
  begin
    ...
    try
      Metafile.Width := MetaFactor * FDX;
      Metafile.Height := MetaFactor * FDY;
      MetafileCanvas := TMetafileCanvas.Create(Metafile, 0);
      try
        MetafileCanvas.Lock;
        FBarCode.Draw2DBarcode(MetafileCanvas, FScaleX * MetaFactor, FScaleY * MetaFactor, 0, 0);
      finally
        MetafileCanvas.Unlock;
        MetafileCanvas.Free;
      end;
      Canvas.StretchDraw(Bounds(FX, FY, FDX, FDY), Metafile);
    finally
      Metafile.Free;
    end;
  end
