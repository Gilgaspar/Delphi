...
uses
  ...
  SHDocVw, Math, Registry, URLMon, frxBrowser, frxBrowserWeb;
const
...
begin
  ...
  NavigateHistory := TStringList.Create;
  WebBrowser.SetOnNavComplete(NavComplete);
end;

destructor TBrowserForm.Destroy;
var aNavigateHistory: TStringList;
begin
  aNavigateHistory := NavigateHistory;
  FreeAndNil(aNavigateHistory);
  NavigateHistory := aNavigateHistory;
  inherited;
end;
...
//  if frxBrowserGlobal.ClassName = 'TfrxBrowserWeb' then
//  begin
//    if pos('dropbox',URL) <> 0 then
//      UrlMkSetSessionOption(URLMON_OPTION_USERAGENT, PChar('Mozilla/1.0'), Length('Mozilla/1.0'), 0);
//  end;
//{$ENDIF}
...
begin
  ...
  if Assigned(OnTestURL) and FOnTestURL(aURL) then
    ModalResult := mrOK;
end;
