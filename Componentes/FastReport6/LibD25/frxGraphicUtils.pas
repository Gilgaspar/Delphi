...
uses
  frxPrinter, SyncObjs {added by Samuray}, frxPlatformServices, frxUtils;
    ...
    repeat
      ...
      {$IFNDEF FPC} and (Win32Platform = VER_PLATFORM_WIN32_NT) {$ENDIF} then
        ...
        begin
          ...
          if VC <> nil then
            VC.ExtTextOut(X + Round(add1 * SinA), Y + Round(add1 * CosA),
              FOptions, @FScaledRect, PChar(s) + j1, i1 - j1, @FTempArray^[j], C.Font);
          {$ELSE}
        ...
        begin
          ...
          if VC <> nil then
            VC.ExtTextOut(X + Round(add1 * SinA), Y + Round(add1 * CosA),
              FOptions, @FScaledRect, PChar(AnsiStr) + j1, i1 - j1, @FTempArray^[j], C.Font);
          {$ELSE}
...
begin
  ...
  try
    ...
    begin
      ...
      DrawTextLine(C, FText[i], x + Round(gx * cos(pi / 180 * FRotation)), y - Round(gx * sin(pi / 180 * FRotation)), dx - gx, i, al, fh, oldfh);
