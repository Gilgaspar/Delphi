    ...
    begin
      ...
      case FTyp of
        bcCodeEAN8:            // 8 digits, 4+4
          if FCheckSum then
            begin
              TextOutR(3 + barOffset, 3 + barOffset, 31 + barOffset, Copy(DoCheckSumming(copy(FText,length(FText)-6,7)), 1, 4));
              TextOutR(35 + barOffset, 35 + barOffset, BarWidth - 4, Copy(DoCheckSumming(copy(FText,length(FText)-6,7)), 5, 4));
            end
          else
            begin
              TextOutR(3 + barOffset, 3 + barOffset, 31 + barOffset, Copy(FText, 1, 4));
              TextOutR(35 + barOffset, 35 + barOffset, BarWidth - 4, Copy(FText, 5, 4));
            end;
        bcCodeCodabar:
          begin
            s := FText;
        ...
        bcCodeEAN13:           // 13 digits, 1+6+6 or 12 digits, 6+6
          begin
            ...
            TextOutR(3 + barOffset, 3 + barOffset, 45  + barOffset, Copy(FText, 2, 6));
            TextOutR(49 + barOffset, 49 + barOffset, BarWidth - 4, Copy(FText, 8, 6));
          end;
        bcCodeUPC_A:           // 12 digits, 1+5+5+1
          begin
            TextOutR(-8 + barOffset, -8 + barOffset, -2, Copy(FText, 1, 1));
            TextOutR(10 + barOffset, 10 + barOffset, 45 + barOffset, Copy(FText, 2, 5));
            TextOutR(49 + barOffset, 49 + barOffset, 84 + barOffset, Copy(FText, 7, 5));
            TextOutR(BarWidth + 1, BarWidth + 1, BarWidth + 8, Copy(FText, 12, 1));
          end;
        bcCodeUPC_E0,
        bcCodeUPC_E1:          // 7 digits, 6+1
          begin
            TextOutR(3 + barOffset, 3 + barOffset, 45 + barOffset, Copy(FText, 1, 6));
