...
uses
  ...
  {$IFNDEF FPC}frxAggregate, {$ENDIF} frxThreading,
...
var
  ...
  begin
    ...
    GetClipRgn(Canvas.Handle, rgn);
    IntersectClipRect(Canvas.Handle, Round(FX), Round(FY), Round(FX1),
      Round(FY1));
    try
...
{$IFDEF FPC}
  with (MForm) do
    for i := 0 to ComponentCount-1 do
      if (Components[i].HasParent) and (Components[i].GetParentComponent is TGroupBox) then
...
begin
  ...
  for i := 0 to FEditorsGlasses.Count - 1 do
    ...
    begin
      if Assigned(CopyFrom) and ((not(ferAllowInPreview in CopyFrom.Editable) and (rfDontEditInPreview in CopyFrom.Restrictions){just for back compat })or (EventParams.EditRestricted)) then Exit;
...
begin
  Result := False;
  if Assigned(PasteTo) and (not(ferAllowInPreview in PasteTo.Editable) or (EventParams.EditRestricted)) then Exit;
  for i := 0 to FEditorsGlasses.Count - 1 do
  begin
...
begin
  for i := 0 to Count - 1 do
  begin
    EList := EditorsClasses[i].FEditorsClasses;
    j := 0;
