...
// 7/23/09  Allow lines[x] to return non-blank value (unicode fix)
// 1/29/09 - Fix bug that caused OLE graphics to become metafiles when they had an icon
//           To go back to old behavior set Patch[12] to true
}
...
{$warnings Off}
  ...
  StdCtrls, ComCtrls, RichEdit, menus, dbctrls, wwstr, wwcommon, printers,
  wwintl, extctrls, wwtrackicon, wwtypes, wwrichole, olectnrs, activex,
...
type
  ...
  TwwRichEditNumberingStyle = (
	renRightParens=0,        //    Follows the number with a right parenthesis.
	renEncloseParens=$100,  // 	Encloses the number in parentheses.
        renPeriodAfter=$200,    //	Follows the number with a period.
        renOnlyNumber=$300,     //    Displays only the number.
	renContinueNumber=$400, //    Continues a numbered list without applying the next number or bullet.
	renNewNumber=$8000      //    Starts a new number with wNumberingStart.
        );


  TwwRichEditNumberingLetter = (
		renNone = 0,
		renBullet = 1,
		renArabic = 2, 	       // Uses Arabic numbers (1, 2, 3, ...).
		renLowercaseLetter = 3,   // Uses lowercase letters (a, b, c, ...).
		renUppercaseLetter = 4,   // Uses uppercase letters (A, B, C, ...).
		renLowercaseRoman = 5,    // Uses lowercase Roman numerals (i, ii, iii, ...).
		renUppercaseRoman = 6,    // Uses uppercase Roman numerals (I, II, III, ...).
		renCharacterSequence = 7  // Uses a sequence of characters beginning with the Unicode character specified by the wNumberingStart member.
                );
  ...
  TwwRichEditOption=(reoShowLoad, reoShowSaveAs, reoShowSaveExit, reoShowPrint, reoShowPrintPreview, reoShowPageSetup,
                     ...
                     reoShowZoomCombo, reoShowHorzScroll);
...
//     property OnDrawText: TwwRTFDrawHeaderFooter read FOnDrawText write FOnDrawText;
  ...
  TwwRTFNotifyEvent = procedure (Form: TForm; RichEdit: TwwCustomRichEdit;
                                 var DoDefault: boolean) of object;
  TwwRTFSaveFormat = (wwrsfRTF, wwrsfPlainText, wwrsfPlainTextUnicode);
...
//     FUseClipboardForSpelling: boolean;
     ...
     FNumberingStyle: TwwRichEditNumberingStyle;
     FZoomMultiplier: double;
     FRightMargin: integer;
     FOnHorzScroll: TNotifyEvent;
     FSaveFormat: TwwRTFSaveFormat;
     class constructor Create;
     procedure SetSaveFormat(value: TwwRTFSaveFormat);
...
//     procedure InheritedCreateParams(var Params: TCreateParams);
     ...
     procedure SendZoomFactor(zoom: double);
     function ZoomPixelsPerInch: integer;
     function GetSelectionHangingIndent : integer;
     procedure SetSelectionHangingIndent(value: integer);
     function GetSelectionIndent : integer;
     procedure SetSelectionIndent(value: integer);
     function GetSelectionRightIndent : integer;
     procedure SetSelectionRightIndent(value: integer);
  ...
  protected
     ...
     procedure SetNumbering(val: TwwRichEditNumberingLetter);
     function GetNumbering: TwwRichEditNumberingLetter;
     procedure SetBulletIndent(val: integer);
     function GetBulletIndent: integer;
     procedure SetNumberingStyle(val: TwwRichEditNumberingStyle);
     function GetNumberingStyle: TwwRichEditNumberingStyle;
     function StoreZoomFactor: boolean;
  ...
  public
    ...
    // 11/12/09 - Remove virtual as this cause C++ error when inheriting from TwwDBRichEdit
     function FindText(const SearchStr: string;
        StartPos, Length: Integer; Options: TSearchTypes): Integer; // virtual;
     function FindTextAfter(const SearchStr: string;
        StartPos, Length: Integer; Options: TSearchTypes): Integer; virtual;
     ...
     {$ifdef IPRichEditSupportInsertImage}
...
{ to be published}
    ...
    property EditorOptions: TwwRichEditOptions read FEditorOptions write FEditorOptions default
           [reoShowSaveExit, reoShowPrint, reoShowPrintPreview, reoShowPageSetup,
            reoShowFormatBar, reoShowToolbar, reoShowStatusBar,
            reoShowHints, reoShowRuler, reoShowInsertObject ,reoCloseOnEscape,
            reoFlatButtons, reoShowMainMenuIcons, reoShowSpellCheck, reoShowZoomCombo, reoShowHorzScroll];
...
//                 [reoSpellCheck, reoGrammarCheck];
    ...
    property Numbering: TwwRichEditNumberingLetter read GetNumbering write SetNumbering;
    property BulletIndent: integer read GetBulletIndent write SetBulletIndent default 0;
    property NumberingStyle: TwwRichEditNumberingStyle read GetNumberingStyle write SetNumberingStyle default renPeriodAfter;
    property ZoomFactor: double read GetZoomFactor write SetZoomFactor stored StoreZoomFactor;
    property SelectionHangingIndent : integer read GetSelectionHangingIndent write SetSelectionHangingIndent;
    property SelectionIndent: integer read GetSelectionIndent write SetSelectionIndent;
    property SelectionRightIndent: integer read GetSelectionRightIndent write SetSelectionRightIndent;
    property RightMargin: integer read GetRightMargin write SetRightMargin;
  published
    property ScrollBars: System.UITypes.TScrollStyle read GetScrollBars write SetScrollBars stored StoreScrollBars;
//    property ScrollBars stored StoreScrollBars;  { Publish before Lines as this causes recreatewnd }
    ...
    property SaveFormat: TwwRTFSaveFormat read FSaveFormat write SetSaveFormat default wwrsfRTF;
...
//    property Lines stored False;  { Backwards compatibility } //Required to be published for proper streaming }
  ...
  TwwRulerPanel = class(TPanel)
    private
      ...
      //Function PixelsPerUnit: integer;
      //Function PixelsToRoundedUnits(int pixels): double;
    protected
...
//    property ErrorMsg: string read FErrorMsg;
  ...
  TwwRichEditStyleHook = class(TRichEditStyleHook)
  strict private
...
uses consts,
  ...
  {$endif}
   ...
   comstrs, wwrich, wwrchdlg, wwrichtb, comobj, clipbrd, wwricholecallback,
     wwrichobjinfo, wwrtfprintpreview, wwricheditbar;
...
{$ifdef IPWorkaroundOleMetaFileBug}
type
	TClipboardData = class
		m_nFormat: integer;
		m_szFormatName: packed array [1..100] of char;
		m_nLength: integer;
		m_pData : Pointer;
  end;
  ...
  public
     ...
     procedure ClearBackupData;
     procedure SetBuffer(Format: Word; var Buffer; Size: Integer);
  end;

  procedure TClipboardBackup.ClearBackupData;
  begin
  ...
  begin
     ClearBackupData;
     for i:= 0 to clipdata.Count-1 do
     begin
        TClipboardBackup(clipData[i]).Free;
     end;
     clipData.Free;
  end;

  function TClipboardBackup.Backup;
  var format: integer;
      data: TClipboardData;
      hMem: THandle;
      pMem: ^THandle;
  begin
     ...
     while true do begin
        ...
        data:= TClipboardData.create;
        data.m_nFormat := format;

        if (format <=14) then
           data.m_szFormatName[1]:= char(0)
        else if (GetClipboardFormatName(format, @data.m_szFormatName, 100)=0) then
           data.m_szFormatName[1]:= char(0);

        //CopyStreamFromClipboard(format, stream);
        ...
        try
          data.m_pData :=
             System.AllocMem(data.m_nLength);
          //SetLength(data.m_pData, data.m_nLength);
          move(pMem^, data.m_pdata^, data.m_nLength);
        finally
          clipdata.Add(data);
          GlobalUnlock(hMem);
        end;
...
procedure TClipboardBackup.SetBuffer(Format: Word; var Buffer; Size: Integer);
var
  DataPtr: Pointer;
  Data: THandle;
begin
  ...
  function TClipboardBackup.Restore: boolean;
  var i: integer;
      data: TClipboardData;
      format, u: integer;
      hMem: THandle;
  ...
  begin
     Clipboard.Open;
     EmptyClipboard;
     for i:=0 to clipdata.Count-1 do begin
        data:= TClipboardData(clipdata[i]);
        format:= data.m_nFormat;
        if data.m_szFormatName[1] <> char(0) then
        begin
           u:= RegisterClipboardFormat(@data.m_szFormatName);
           if (u>0) then format:= u;
        end;
        SetBuffer(format, data.m_pData^, data.m_nLength);
        FreeMem(data.m_pData, data.m_nLength);
     end;
     Clipboard.Close;
  end;

  procedure CopyRichEdit(FromRichEdit, ToRichEdit: TCustomRichEdit);
  var backup: TClipboardBackup;
  ...
  begin
   ...
   try
     backup.Backup;
     OldSelStart:= FromRichEdit.SelStart;
     OldSelLength:= FromRichEdit.SelLength;
     FromRichEdit.SelectAll;
     FromRichEdit.CopyToClipboard;

     SendMessage(ToRichEdit.Handle, WM_SETREDRAW, ord(true), 0);

     try
       ToRichEdit.SelectAll;
       ToRichEdit.PasteFromClipboard;
       FromRichEdit.SelStart:= OldSelStart;
       FromRichEdit.SelLength:= OldSelLength;
       ToRichEdit.SelStart:=0;
     finally
       SendMessage(ToRichEdit.Handle, WM_SETREDRAW, ord(true), 0);
     end;
...
destructor TwwRichEditStrings.Destroy;
begin
  inherited Destroy;
end;
...
var
  ...
  L: Integer;
begin
  Word((@Text)^) := Length(Text); // 7/23/09 - Use Length for unicode
...
var
  Stream: TStream;
begin
  if (RichEdit.Patch[12]=true) or RichEdit.PlainText then
  begin
...
begin
  if (RichEdit.Patch[12]=true) or RichEdit.PlainText then
    ...
    RichEdit.LoadPacketsFromStream(Stream, false, 0);
end;


procedure TwwRichEditStrings.SaveToFile(const FileName: string);
begin
   if (RichEdit.SaveFormat = wwrsfPlainTextUnicode) then
//   if RichEdit.PlainText and (RichEdit.Patch[13]=true) then
      RichEdit.ILines.SaveToFile(FileName, TEncoding.Unicode)
   else
...
begin
   if (RichEdit.SaveFormat = wwrsfPlainTextUnicode) then
//   if RichEdit.PlainText and (RichEdit.Patch[13]=true) then
      RichEdit.ILines.SaveToStream(Stream, TEncoding.Unicode)
   else
...
{$WARNINGS OFF}
{$ifdef wwDelphiXE5Up}
function EditSelectStreamInCallback(dwCookie: DWORD_PTR; pbBuff: PByte;
...
{$ifdef wwDelphiXE5Up}
Function EditSelectStreamOutCallback(dwCookie: DWORD_PTR; pbBuff: PByte;
     cb: Longint; var pcb: Longint): Longint; stdcall;
{$else}
...
constructor TwwRichEditStyleHook.Create(AControl: TWinControl);
begin
  inherited;
  Control:= AControl;
//  OverridePaintNC := true;
  OverrideEraseBkgnd :=true; // Reduces flicker in WMEraseBgnd
end;

procedure TwwRichEditStyleHook.EMSetBkgndColor(var Message: TMessage);
begin
//  inherited;
//  exit;
  if not (Control as TwwDBRichEdit).PlainText then
    Message.LParam := ColorToRGB((Control as TwwCustomRichEdit).Color)
  else
...
procedure TwwRichEditStyleHook.EMSetCharFormat(var Message: TMessage);
type
...
var
  Format: PCharFormat2;
begin
  if not (Control as TwwDBRichEdit).PlainText then exit;
...
procedure TwwRichEditStyleHook.PaintNC(Canvas: TCanvas);
begin
  if not (Control as TwwCustomRichEdit).Frame.Enabled then
     inherited PaintNC(Canvas)
end;

procedure TwwRichEditStyleHook.WMEraseBkgnd(var Message: TMessage);
begin
  // paint with style if have focus or if framing is not enabled - otherwise just use class behavior
  // Current seems to cause flicker when scrolling if call inherited. Let us disable for now
  // to see if any side effects
  if Control.Focused or (not (Control as TwwCustomRichEdit).Frame.Enabled) then
  begin
     if false then inherited
  end;
end;

class constructor TwwDBRichEdit.Create;
begin
  TCustomStyleEngine.RegisterStyleHook(TwwDBRichEdit, TwwRichEditStyleHook);
end;
...
begin
  ...
  FNumberingStyle:= renPeriodAfter;
...
//  SpellCheckOptions:= [reoSpellCheck, reoGrammarCheck];
  ...
  Patch:= VarArrayCreate([0, 14], varVariant);
  ...
  Patch[7]:= false; //True; { Set to True to disable checking of focus for MSWordSpellChecker}
  Patch[8]:= 0; { StartPage for Printing }
  Patch[9]:= 0; { EndPage for Printing}
  Patch[10]:=false; // Set to true to use old RICHEDIT20A
  Patch[11]:= false;
  Patch[12]:= false; //Set to true to use old streaming behavior of TRichEdit implementation. We don't use
                     // it anymore due to it streaming incorrectly with ole graphics
  Patch[13]:= false; // Set to true to enable unicode plaintext streaming
  Patch[14]:= false; // Set to true to disable typography. Typography does not recognize internal \line in rtf which can
                     // cause problems when loading
                     // You will need to call sendmessage(handle, cm_recreatewnd, 0, 0) after setting this patch variable to true
  if wwGridEssentials then
  begin
      DisplayProfessionalVersionOnlyMessage(self, 'TwwDBRichEdit is only supported in the professional version');
  end;
...
begin
  ...
  if FMemoLoaded then
  begin
    if IsValidKey(Key) and (FDataLink.Field <> nil) and
...
begin
  ...
  begin
    ...
    try try
      ...
      begin
         if PlainText then
            Lines.Text:= FDataLink.Field.AsString // 8/11/11
         else begin
      ...
      if not (csPaintCopy in ControlState) and
         ...
         // 5/29/11 - If blank need more default settings (new record)
...
begin
  if FDataLink.Field <> nil then begin
    ...
    begin
      ...
      if (parent<>nil) and (parent.parent is TwwRichEditBar) then
         with TwwRichEditBar(parent.parent) do
            if RichEditBarFrame.ZoomCombo.visible then
              RichEdit.zoomfactor:= 1;       // 11/11/12 - Maintain consistency when scrolling
    ...
    begin
       ...
       begin
         if PlainText then
           ...
           try
              if (SaveFormat = wwrsfPlainTextUnicode) then
//              if PlainText and (Patch[13]=true) then
                ILines.SaveToStream(Stream, TEncoding.Unicode)
              else
    ...
    else if (FDataLink.Field is TStringField) then // 5/13/11 - Support TStringField
    begin
       Stream := TStringStream.Create('');
       try
          ILines.SaveToStream(Stream);
...
class constructor TwwCustomRichEdit.Create;
begin
  TCustomStyleEngine.RegisterStyleHook(TwwCustomRichEdit, TwwRichEditStyleHook);
end;
...
begin
   ...
   EditorOptions:=
         [reoShowSaveExit, reoShowPrint, reoShowPrintPreview, reoShowPageSetup,
            reoShowFormatBar, reoShowToolbar, reoShowStatusBar,
            reoShowHints, reoShowRuler, reoShowInsertObject, reoCloseOnEscape,
            reoFlatButtons, reoShowMainMenuIcons, reoShowSpellCheck, reoShowZoomCombo];
...
procedure TwwRulerPanel.AdjustClientRect(var Rect: TRect);
begin
//  Rect := Bounds(-100, 0, Rect.Right, Rect.Bottom);
...
begin
   ...
   ScreenPixelsPerUnit:= DialogRichEdit.ScreenPixelsPerInch * DialogRichEdit.ZoomFactor;
   ...
   SI.cbSize := sizeof(SI);
   SI.fMask := SIF_ALL;
   GetScrollInfo(DialogRichEdit.Handle, SB_HORZ, SI);
   ...
   with Canvas do begin
      Brush.Color:= clWhite;
      if wwUseThemes(DialogRichEdit) then
        wwFillEditThemeBackgroundDefault(RulerRect, Canvas)   // StyleServices
      else
        FillRect(RulerRect);
...
//         WidthPixels:= Trunc(PageWidth * DialogRichEdit.ScreenPixelsPerInch);
         ...
         PageWidth:= (RulerRect.Right-RulerRect.Left)/ScreenPixelsPerUnit;
      end;
      ...
      for i:= 0 to trunc(pageWidth)+1 do begin
         ...
         if i>0 then begin
            wwWriteTextLinesT(Canvas, self.ClientRect, //RulerRect,
               scrollOffset + Trunc(RulerRect.Left + i*ScreenPixelsPerUnit - TextWidth(inttostr(i)) div 2),
               RulerRect.Top+3,
                   PChar(inttostr(i)), taLeftJustify, WriteOptions);
//            TextOut(scrollOffset + Trunc(RulerRect.Left + i*ScreenPixelsPerUnit - TextWidth(inttostr(i)) div 2),
//                  RulerRect.Top+3, inttostr(i));
         end;
         if DialogRichEdit.MeasurementUnits=muCentimeters then Divisions:= 4
         else Divisions:= 8;
         if DialogRichEdit.ZoomFactor<0.51 then
            Divisions:= Trunc(Divisions/2);

         for j:= 1 to Divisions-1 do
         begin
            Pen.color:= clBlack;
            if wwGetThemedTextColor(ThemeTextColor) then
               Pen.Color:= ThemeTextColor;
            if (i + j/Divisions >= PageWidth) then continue;

            if (j=Divisions div 2) then begin
               MoveTo(scrollOffset + RulerRect.Left + Trunc((i+(j/Divisions))*ScreenPixelsPerUnit), RulerRect.Top+7);
               LineTo(scrollOffset + RulerRect.Left + Trunc((i+(j/Divisions))*ScreenPixelsPerUnit), RulerRect.Bottom-5);
            end
            else begin
               MoveTo(scrollOffset + RulerRect.Left + Trunc((i+(j/Divisions))*ScreenPixelsPerUnit), RulerRect.Top+8);
               LineTo(scrollOffset + RulerRect.Left + Trunc((i+(j/Divisions))*ScreenPixelsPerUnit), RulerRect.Bottom-7);
            end
         end
      end;
...
begin
   ...
   begin
      ...
      WidthPixels:= Trunc(PageWidthInches * DialogRichEdit.ZoomPixelsPerInch);
      Width:= 12 + WidthPixels;
      self.Left:= DialogRichEdit.Left + Trunc((DialogRichEdit.GutterWidth)*DialogRichEdit.ZoomFactor)-4;
      RulerRect:= Rect(5, 4, self.Width-6, self.Height-4);
   end
   else begin
       if DialogRichEdit.BorderStyle=bsNone then borderOffset:=0
       else borderOffset:=1;

       self.Left:= wwMax(0, DialogRichEdit.Left + Trunc((DialogRichEdit.GutterWidth-(2+borderoffset))*DialogRichEdit.ZoomFactor)-(4-borderOffset));
			 self.Width := DialogRichEdit.Left + DialogRichEdit.Width; // + Trunc(DialogRichEdit.GutterWidth*DialogRichEdit.ZoomFactor);
       RulerRect:= Rect(5, 4,
            DialogRichEdit.ClientWidth - (self.Left-DialogRichEdit.Left),
            //DialogRichEdit.Left +
            //(2-Trunc(DialogRichEdit.GutterWidth*DialogRichEdit.ZoomFactor)) + DialogRichEdit.ClientWidth,
                  self.Height-4);
   end;
...
procedure TwwRulerPanel.CreateDragButtons;
var dragButton: TwwTrackIcon;
    firstTime: boolean;
    zoom: double;
var DialogRichEdit: TwwCustomRichEdit;
begin
    firstTime:= LeftDragButton=nil;

    if LeftDragButton=nil then
    begin
      dragButton:= TwwTrackIcon.create(self);
      dragButton.parent:= self;
      dragButton.BitmapName:= 'wwRulerUp';
      LeftDragButton:= dragButton;
    end;
    dragButton:= LeftDragButton;

    dragButton.MinValue:= RulerRect.Left -
        ((dragButton.Width) div 2);
    dragButton.MinValue:= 0; // 8/17/10
   if (parent is TwwRichEditForm) then
      DialogRichEdit:= (parent as TwwRichEditForm).RichEdit1
   else
      DialogRichEdit:= RichEdit;
    if DialogRichEdit<>nil then zoom:= DialogRichEdit.ZoomFactor
    else
       zoom:=1.0;

    dragButton.MaxValue:= Trunc(RulerRect.Right/zoom)-(dragButton.Width div 2);
    if firstTime then begin
       dragButton.Left:= dragButton.MinValue;
       dragButton.Top:= RulerRect.Bottom - dragButton.Height + 4;
       dragButton.OnEndDrag:= UpdateRulerProp;
    end;
    ...
    begin
      ...
      dragButton.parent:= self;
      dragButton.BitmapName:= 'wwRulerUp';
      RightDragButton:= dragButton;
    end;
    dragButton:= RightDragButton;

    dragButton.MinValue:= RulerRect.Left -
        ((dragButton.Width) div 2);

    dragButton.MaxValue:= Trunc((RulerRect.Right-(dragButton.Width div 2))/zoom);
    if firstTime then begin
    ...
    begin
      ...
      FirstLineDragButton:= dragButton;
    end;
    dragButton:= FirstLineDragButton;
    ...
    if firstTime then begin
       ...
       dragButton.Top:= RulerRect.Top -2;
       dragButton.OnEndDrag:= UpdateRulerProp;
    end;
end;
{
Function TwwRulerPanel.PixelsToRoundedUnits(int pixels): double;
var intervals: integer;
begin
			if ((RichEdit<>nil) and (RichEdit.MeasurementUnits = muCentimeters)) then
				intervals := 4;
			else intervals := 8;

			double result =
				Math.Floor(((double)pixels)/PixelsPerUnit() * 1000.0) -
				Math.Floor(((double)pixels)/PixelsPerUnit() * 1000.0) % (1000 / intervals);
			result = result/1000.0;
//            if (openMemo.ZoomFactor != 1.0)
//                result = (int)(result / openMemo.ZoomFactor);
			return result;

end;

function TwwRulerPanel.PixelsPerUnit;
const CentimetersPerInch = 2.537;
begin
		if RichEdit=nil then
       result:= RichEdit.ScreenPixelsPerInch
    else
       result:= 96;
		if ((RichEdit<>nil) and (RichEdit.MeasurementUnits = muCentimeters)) then
				result := trunc(pixelsPerUnit/CentimetersPerInch);
    result := trunc(pixelsPerUnit * RichEdit.ZoomFactor);
...
begin
   ...
   if not DialogRichEdit.HandleAllocated then exit;


   DialogRichEdit.GetParaIndent(tempLeft, tempRight, tempFirst);
   //temp:= TwipsToPixels(tempLeft) + TwipsToPixels(tempFirst);;
	 FirstLineDragButton.Position := DialogRichEdit.SelectionIndent;
   //RightDragButton.Left:= ((Width - DialogRichEdit.SelectionRightIndent) - RightDragButton.Width);
   RightDragButton.Left := Trunc((RulerRect.Left + (RulerRect.Right-RulerRect.Left) -
       RightDragButton.Width / 2){/DialogRichEdit.ZoomFactor}) - Trunc(DialogRichEdit.ZoomFactor * DialogRichEdit.SelectionRightIndent); //TwipsToPixels(tempRight); //(int)(openMemo.SelectionRightIndent);
   LeftDragButton.Position := DialogRichEdit.SelectionHangingIndent + DialogRichEdit.SelectionIndent;
...
//    modPixel: integer;
   ...
   begin
      ...
      if DialogRichEdit.MeasurementUnits=muCentimeters then
         ScreenPixelsPerUnit:= ScreenPixelsPerUnit / wwCentimetersPerInch;
      result := trunc(ScreenPixelsPerUnit * DialogRichEdit.ZoomFactor);
   ...
   begin
      ...
      if PixelsLeft>LeftDragButton.MaxValue{*DialogRichEdit.ZoomFactor} then
         PixelsLeft:= Trunc(LeftDragButton.MaxValue {* DialogRichEdit.ZoomFactor});

      PixelsRight:= Trunc(UnitsRight * PixelsPerUnit {+ 1});
      if PixelsRight>RightDragButton.MaxValue {* DialogRichEdit.ZoomFactor }then
         PixelsRight:= Trunc(RightDragButton.MaxValue{ * DialogRichEdit.ZoomFactor});

      PixelsFirstLine:= Trunc(UnitsFirstLine * PixelsPerUnit{ + 1});
      if PixelsFirstLine>FirstLineDragButton.MaxValue{ *DialogRichEdit.ZoomFactor }then
         PixelsFirstLine:= Trunc(FirstLineDragButton.MaxValue {* DialogRichEdit.ZoomFactor});
...
//   ModPixel:= 0.25 * 1440/2.537;
   tempLeft:= LeftDragButton.Position;
   tempLeft:= PixelsToTwipsX(tempLeft);
   tempFirst:= FirstLineDragButton.Position;
   ...
   if (Sender = LeftDragButton) then begin
      ...
      //dialogrichedit.SelectionHangingIndent:= LeftDragButton.Position-FirstLineDragButton.Position;
      //dialogrichedit.SelectionIndent:= LeftDragButton.Position + FirstLineDragButton.Position-LeftDragButton.Position;


		 dialogRichEdit.SelectionHangingIndent:= LeftDragButton.Position-DialogRichEdit.SelectionIndent;
     // 	openMemo.SelectionHangingIndent = LeftDragButton.Position-openMemo.SelectionIndent;
      ...
      begin
         ...
         tempPixels:= Trunc(tempPixels/DialogRichedit.ZoomFactor);
         tempRight:= tempPixels;
      end
      else begin
          tempPixels := Trunc((RightDragButton.MaxValue - RightDragButton.Left)/DialogRichedit.ZoomFactor);
          //tempPixels := Trunc(RightDragButton.MaxValue/DialogRichEdit.ZoomFactor) - RightDragButton.Position;
					tempRight := tempPixels;
...
//         tempRight:= PixelsToTwipsX(tempPixels);
      ...
      if (tempRight >= 0) then
           DialogRichEdit.SelectionRightIndent := tempRight;
...
//              '', False, 0, tempRight, 0, 0, nil, 0, 0, 0, 0);
      ...
      dialogrichedit.SetParaFormat([rpoLeftIndent, rpoFirstLineIndent],
...
begin
   ...
   with form do try
      ...
      with wwRulerPanel do begin
         ...
         align:= alTop;
         BevelOuter:= bvNone;
         wwRulerPanel.RulerRect:= Rect(RulerIndent, 4, RulerPanel.Width-6, RulerPanel.Height-4); //RulerHeight);
      end;
      ...
      RichEditBar1:= TwwRichEditBar.create(form);
      RichEdit1:= RichEditBar1.RichEdit;
...
//         WordWrap := False;
      ...
      if (EditWidth=rewPrinterSize) and (reoShowHorzScroll in EditorOptions) then
         RichEdit1.ScrollBars := ssBoth;  // 8/24/10 - Allow horizontal scrollbars if editwidth is printer
      ...
      RichEdit1.HideScrollBars:= self.HideScrollBars;
      ...
      begin
         RichEdit1.EditorOptions:=
            [reoShowLoad, reoShowSaveAs, reoShowSaveExit, reoShowPrint, reoShowPrintPreview, reoShowPageSetup,
             ...
             reoCloseOnEscape,
             reoFlatButtons, reoShowSpellCheck, reoShowMainMenuIcons,
             reoShowJustifyButton, reoShowZoomCombo, reoShowHorzScroll];
      ...
      //RichEdit1.selStart:= self.selStart;  Do in form's onshow
      ...
      if result then begin
         if RichEdit1.modified then begin
             ...
             begin
                ...
                begin
                  {$ifdef wwDelphi2006Up}
                   if ((GetField as TBlobField).BlobType = ftWideMemo) then // Don't use TStringStream anymore
                   begin
                      if PlainText then
                         GetField.AsString:= RichEdit1.ILines.Text
                      else begin
                        Stream := TStringStream.Create('');
                        try
                ...
                else if (GetField is TStringField) then begin  //5/13/11
                   Stream := TStringStream.Create('');
                   try
                      RichEdit1.ILines.SaveToStream(Stream);
                      GetField.AsString := Stream.DataString;
...
//function TwwRichMemoryStream.Realloc(var NewCapacity: NativeInt): Pointer;
...
begin
   ...
   try
      ...
      if RichEditCopyMethod = recmByTempFile then begin
         ...
         // 8/11/11 - Support ftWideMemo
         {$ifdef wwDelphi2009Up}
         if (self  is TwwDBRichEdit) and (TwwDBRichEdit(self).PlainText) then
            if (TwwDBRichEdit(self).FDataLink.Field is TBlobField) and
               ((TwwDBRichEdit(self).FDataLink.Field as TBlobField).BlobType = ftWideMemo) then
            begin
               (val as TwwDBRichEdit).Lines.Text:= self.Lines.Text;
               exit;
            end;
         {$endif}
         ...
         try
            ...
            if (val as TwwDBRichEdit).PlainText then
            begin
...
{            if PlainText or (val as TwwDBRichEdit).PlainText then
            begin
               (val as TwwDBRichEdit).Lines.Text:= self.Lines.Text;
            end
            else }if (Patch[12]=true) or PlainText or (val as TwwDBRichEdit).PlainText then
              TRichEdit(val).Lines.LoadFromStream(stream1)
            else
              (val as TwwDBRichEdit).Lines.LoadFromStream(stream1);
         finally
            if MaxLength>0 then { 3/16/99 }
               TRichEdit(val).MaxLength:= TRichEdit(val).MaxLength - 1;
            stream1.free;
...
//      stream1.size:= TBlobField(Field).BlobSize;
      ...
      stream1.position:= 0;
      ILines.Clear;
      if (Patch[12]=true) or PlainText then
...
// 3/1/10 - Fix code so that it uses StringStream for Unicode support
...
begin
  ...
  try
    ...
    repeat
       ...
       while StrCharMatch(s[APos+len], '0123456789') do inc(len);
...
begin
  ...
  if not NewDataFormat then begin {Backwards compatibility }
     ...
     stream1.position:= 0;
     {
     if parent is TwwRichEditBar then
        plainText:=true
     else
        PlainText:= (self.SaveFormat <> TwwRTFSaveFormat.wwrsfRTF);
     }
     if (Patch[12]=true) or (PlainText) then
        ILines.LoadFromStream(stream1)
     else
...
begin
   ...
   if not wwIsClass(self.classtype, 'TwwDBRichEditMSWord') and
      not Assigned(OnSpellCheck) then
   ...
   PopupBullet.checked:= Numbering=renBullet; //Paragraph.Numbering <> nsNone;
...
//    SystemDefaultCharSize = 3 - Win32Native.lstrlen(ptr);
  ...
  begin
     // 2/10/10 - Use Unicode version even by default for Delphi 2010 instead of ansi version
...
begin
   ...
   if EffectiveEditWidth<>rewWindowSize then begin
      with Printer do begin
        ...
        PageWidthInches := PrinterPixelsToTwipsX(PrnPhysPageSize.x)/1440;
        if MeasurementUnits=muCentimeters then
           PageWidthInches:= PageWidthInches * wwCentimetersPerInch;
...
//        r.left:= GutterWidth+10;
        ...
        if (not wwIsDesigning(self)) then // 10/24/10 - Use RightMargin instead of using EditRect which doesn't work with zoom
        begin
           FRightMargin:= 0; // Reset it so RightMargin is set
           self.RightMargin := Trunc({ *ZoomFactor} (pageWidthInches - XMarginInches) * ScreenPixelsPerInch - 1);
        end
...
function TwwCustomRichEdit.PixelsToTwips(Pixels: integer): integer;
begin
   result := (pixels * wwTwipsPerInch) div ScreenPixelsPerInch;
end;
...
//  FLastSetRect:= Rect(0,0,0,0);
  ...
  if (RichEditVersion>=2) and (patch[14]=false) then
  ...
  if FZoomMultiplier<>1.0 then
     SendZoomFactor(FZoomMultiplier);
...
{$ENDIF}
  ...
  try
    ...
    begin
      Buffer[pcb] := 0;
      // 1/29/09 - Remove this code as it messes up OLE in some cases
   ...
   if wwInPaintCopyState(ControlState) {and (FDataLink.Field<>nil) }then begin
     ...
     begin
        with TwwDBRichEdit(FPaintControl) do begin
           ...
           // 3/24/09 - Paint widememo properly in csPaintCopy state
           // 5/13/11 - Support TStringField
           {$ifdef wwDelphi2006Up}
           if ((self.FDataLink.Field is TBlobField) and ((self.FDataLink.Field as TBlobField).BlobType = ftWideMemo)) or
              (self.FDataLink.Field is TStringField) then
           begin
              if self.PlainText then
                 Lines.Text:= self.FDataLink.Field.AsString
              else begin
                Stream := TStringStream.create(self.FDataLink.Field.AsString);
                try
                   Lines.LoadFromStream(Stream);
                finally
                   Stream.Free;
...
begin
  ...
  try
//     PlainText:= False;
     PlainText:= SaveFormat<>TwwRTFSaveFormat.wwrsfRTF;
     ...
     if (Size>0) and (GetField=Nil) then begin
        ...
        if ((Patch[12]=true) and not (csDesigning in ComponentState)) or PlainText then
           ILines.LoadFromStream(TempStream)
        else
           Lines.LoadFromStream(TempStream);
        //showmessage(inttostr(size) + ' ' + GetRTFText);
     end;
     NewDataFormat:= True;
  finally
     TempStream.Free;
...
begin
   ...
   try{
      if plaintext then
         showmessage('write plain: ' + inttostr(length(GetRTFText)) + ' ' + GetRtfText)
      else begin
         showmessage('write rtf: ' + inttostr(length(GetRTFText)) + ' ' + GetRtfText);
//         plainText:=true;
      end;
      }
...
procedure TwwCustomRichEdit.WMHScroll(var Msg: TWMHScroll);
...
begin
{
  case Msg.ScrollCode of
    ...
    SB_PAGEDOWN:
     begin
       msg.result:=1;
       ScrollWindowEx(Handle, -40, 0, nil, nil,
            0, nil, SW_Invalidate);
//       inherited;
       exit;
       scrollpos:= msg.Pos;
...
begin
  ...
  {$ifdef Unicode}
  else if ((Message.Msg = EM_FINDTEXT) or (Message.Msg = EM_FINDTEXTW)) and (Patch[0]=False) then begin {8/24/98 }
  {$else}
...
{$ifdef IPRichEditSupportInsertImage}
function TwwCustomRichEdit.InsertImage(fileName: string): boolean;
...
begin
  ...
  try
    ...
    FormatEtc.ptd := nil; //IntPtr.Zero;
    FormatEtc.dwAspect := DVASPECT_CONTENT;
    FormatEtc.lindex := -1;
    FormatEtc.tymed := TYMED_NULL;

    wideFileName:= fileName;
    hr := OleCreateFromFile(GUID_NULL, PWideChar(widefilename), IID_IOleObject,
         OLERENDER_DRAW, @FormatEtc, ClientSite, {pOleClientSite, }Storage, FOleObject);
    if (FOleObject=nil) then
    begin
       result:= false;
       exit;
    end;
    FOleObject.GetUserClassID(guid);
  ...
  if Assigned(FRichEditOle) then begin
    ...
    try
      ...
      with Data do begin
        ...
        lpszFile := NameBuffer;
        {$ifdef wwDelphi2009up}
        cchFile := SizeOf(NameBuffer) div 2; // Ansi adjustment
        {$else}
...
begin
   result:= FindTextAfter(SearchStr, StartPos, Length, Options);
end;

function TwwCustomRichEdit.FindTextAfter(const SearchStr: string;
  StartPos, Length: Integer; Options: TSearchTypes): Integer;
const FR_DOWN = $00000001;
var
  Find: TFindText;
...
//  if copy(t, 1,8)='Spelling' then
  ...
  begin
     ...
     begin
       ...
       // 12/5/08 - Hide window so no flicker
       SetWindowPos(window, HWND_TOP, -500, -500, 100, 100,
...
begin
   ...
   try
     MemoryStream.WriteString(InsertText);
...
begin
  ...
  try
     ...
     memoryStream.position:= 0;
     result:= memoryStream.ReadString(memoryStream.Size);
  finally
     memoryStream.Free;
  end;
end;
...
var
  bi, bb, rtf: string;
  bis, bbs: Cardinal;
//  achar: ShortString;  why short string
  achar: string;
  hexpict: string;
  I: Integer;
begin
...
procedure TwwCustomRichEdit.SetSaveFormat(value: TwwRTFSaveFormat);
begin
   if FSaveFormat<>value then
   begin
     if value = TwwRTFSaveFormat.wwrsfRTF then
     begin
        plainText:=false;
     end
     else begin
...
procedure TwwCustomRichEdit.SetScrollBars(Value: System.UITypes.TScrollStyle);
begin
   inherited ScrollBars := Value;
end;

function TwwCustomRichEdit.GetScrollBars: System.UITypes.TScrollStyle;
begin
...
procedure TwwCustomRichEdit.SetNumbering(val: TwwRichEditNumberingLetter);
var paraformat1: TwwParaFormat2;
begin
   if (val=Numbering) then exit;
   if RichEditVersion<2 then exit;
   GetParaFormat(paraFormat1);
   paraformat1.dwMask := PFM_NUMBERING or PFM_OFFSET; //Win32API.ParaFormatFlags.Offset;
   if (val>renBullet) then  paraformat1.dwMask := paraformat1.dwMask or PFM_NumberingStyle;

   if (val=renNone) then
   begin
      paraformat1.wNumbering := 0;
      paraformat1.dxOffset := 0;
      paraformat1.wNumberingStyle:=0;
   end
   else begin
     if (Numbering<=renBullet) then
     begin
         // Not currently numbering but want to
	if (val>renBullet) then begin
           paraformat1.dwMask := paraformat1.dwMask or PFM_NumberingStart;
	   paraformat1.wNumberingStart := 1;
           if BulletIndent =0 then FBulletIndent:= 360;
        end;
	paraformat1.wNumbering := ord(val);
        if BulletIndent>0 then paraformat1.dxOffset := BulletIndent; //Pixel2Twip(IntPtr.Zero, this.BulletIndent, true);
	paraformat1.wNumberingStyle := ord(NumberingStyle);
     end;
   end;
   SendMessage(Handle, EM_SETPARAFORMAT, 0, LPARAM(@paraFormat1));

end;

function TwwCustomRichEdit.GetNumbering: TwwRichEditNumberingLetter;
var paraformat: TwwParaFormat2;
begin
   ...
   //paraformat1.rgxTabs = new int[0x20];
   //Win32API.SendMessage(Handle, Win32API.EM_GETPARAFORMAT, 0, paraformat1);
   if (($20 and paraformat.dwMask) <> 0) then
   begin
      result:= TwwRichEditNumberingLetter(paraformat.wNumbering);
   end
   else
      result:= renNone
end;

function TwwCustomRichEdit.GetBulletIndent: integer;
begin
   result:= fBulletIndent;
end;

procedure TwwCustomRichEdit.SetBulletIndent(val: integer);
begin
   if val<0 then exit;
   fBulletIndent:= val;

   if HandleAllocated and (Numbering = renBullet) then
   begin
      self.SetBullet(true);
   end;
end;

procedure TwwCustomRichEdit.SetNumberingStyle(val: TwwRichEditNumberingStyle);
begin
   FNumberingStyle:= val;
   if HandleAllocated and (Numbering = renBullet) then
      self.SetBullet(true)
   else if HandleAllocated and (Numbering>renBullet)  then
       Numbering := Numbering; // refresh
end;

function TwwCustomRichEdit.GetNumberingStyle: TwwRichEditNumberingStyle;
begin
    result:= FNumberingStyle;
end;

function TwwCustomRichEdit.GetZoomFactor: double;
...
begin
    ...
    begin
        wparam := 0;
        lparam := 0;
        SendMessage(Handle, $04e0, Longint(@wparam), Longint(@lparam)); // LPARAM(@lparam));
        if ((wparam <> 0) and (lparam <> 0)) then
        begin
            fZoomMultiplier := wparam / lparam;
        end
        else begin
            FZoomMultiplier := 1.0;
        end;
    end;
    result:= FZoomMultiplier;
end;

procedure TwwCustomRichEdit.SetZoomFactor(value: double);
begin
    GetZoomFactor; // Updates FZoomMultiplier in case handle already created
    if (FZoomMultiplier <> value) then
    begin
        if ((value <= 0.015625) or (value >= 64)) then
        begin
            ...
            //throw new ArgumentOutOfRangeException("ZoomFactor", SR.GetString("InvalidExBoundArgument", args));
        end
        else
...
procedure TwwCustomRichEdit.SendZoomFactor(zoom: double);
...
begin
    ...
    if (HandleAllocated) then
    begin
        SendMessage(Handle, $4e1, num, num2);
        if num<>0 then
           FZoomMultiplier:= num/num2
        else
           FZoomMultiplier:= 1.0;
    end
    else if (num <> 0) then
        FZoomMultiplier := num/num2
    else FZoomMultiplier := 1;

end;


function TwwCustomRichEdit.StoreZoomFactor: boolean;
begin
  result := FZoomMultiplier<>1.0;
end;

function TwwCustomRichEdit.ZoomPixelsPerInch: integer;
begin
   result:= Trunc(ScreenPixelsPerInch * ZoomFactor);
end;

function TwwCustomRichEdit.GetSelectionHangingIndent : integer;
var v: integer;
    LeftIndent, RightIndent, FirstIndent: integer;
begin
    //v:=0;
    HandleNeeded;
    GetParaIndent(LeftIndent, RightIndent, FirstIndent);
    v:= LeftIndent;
    result:= TwipsToPixels(v);
end;

procedure TwwCustomRichEdit.SetSelectionHangingIndent(value: integer);
var v: integer;
    //LeftIndent, RightIndent, FirstIndent: integer;
begin
    //GetParaIndent(LeftIndent, RightIndent, FirstIndent);
    v:= PixelsToTwips(value);
    SetParaFormat([rpoLeftIndent],
           '', False, v, 0, 0, 0, nil, 0, 0, 0, 0);
end;

function TwwCustomRichEdit.GetSelectionRightIndent : integer;
var v: integer;
    LeftIndent, RightIndent, FirstIndent: integer;
begin
    //v:=0;
...
procedure TwwCustomRichEdit.SetSelectionRightIndent(value: integer);
var v: integer;
    //LeftIndent, RightIndent, FirstIndent: integer;
begin
    //GetParaIndent(LeftIndent, RightIndent, FirstIndent);
    v:= PixelsToTwips(value);
    SetParaFormat([rpoRightIndent],
           '', False, 0, v, 0, 0, nil, 0, 0, 0, 0);
end;

function TwwCustomRichEdit.GetRightMargin: integer;
begin
   result:= FRightMargin;
end;

procedure TwwCustomRichEdit.SetRightMargin(value: Integer);
var dc: HDC;
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            begin
                //dc:= GetDC(Handle);
                dc := CreateIC('DISPLAY', nil, nil, nil);//@deviceMode); //new HandleRef(null, IntPtr.Zero));
                try
                  SendMessage(Handle, $448, dc, PixelsToTwips(value));
                finally
                   DeleteDC(DC);
                end;
            end;
...
function TwwCustomRichEdit.GetSelectionIndent : integer;
var v: integer;
    LeftIndent, RightIndent, FirstIndent: integer;
begin
    //v:=0;
...
procedure TwwCustomRichEdit.SetSelectionIndent(value: integer);
var v: integer;
    //LeftIndent, RightIndent, FirstIndent: integer;
begin
    //GetParaIndent(LeftIndent, RightIndent, FirstIndent);
    v:= PixelsToTwips(value);
    SetParaFormat([rpoFirstLineIndent],
           '', False, 0, 0, v, 0, nil, 0, 0, 0, 0);
end;
