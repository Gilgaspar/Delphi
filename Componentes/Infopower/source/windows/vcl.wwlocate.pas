...
{$Warnings Off}
{$ifdef wwDelphi2008Up}
Function ValueAsString(field : TField; buffer: PAnsiChar) : string;
{$else}
...
//                  Field.DataSet.Translate(PByte(Buffer),PByte(Buffer),False);
                  {$else}
                  Field.DataSet.Translate(PAnsiChar(Buffer),PAnsiChar(Buffer),False);
                  {$endif}
               end;
...
var FindText, TableFieldValue: string;
  ...
  begin
     result:= wwDataSetIsCaseSensitiveIndex(tbl);
(*
...
*)
   ...
   begin
      ...
      with dataset do for i:= 0 to IndexDefs.count-1 do begin
	 with IndexDefs do begin
	     if (AnsiUppercase(wwGetIndexName(dataset))=AnsiUppercase(Items[i].name)) then {Changed to AnsiUpperCase}
	     begin
   ...
   begin
      ...
      if True or wwisNonBDEField(curField) then begin // Bypass bde specific optimizations to remove bde dependency
...
{
         FieldValue:= '';
         res:= wwGetVariableWithParams(DataSet, 'GetNextFieldValue', searchField);
         result:= (res<>null);
         if result then FieldValue:= res;
}
(*
...
begin
   ...
   if wwIsClass(dataSet, 'TwwTable')
      and (curField.dataType<>ftMemo) and (not wwIsTableQuery(DataSet)) then
   begin
      IndexDefs.update;
      IndexFieldCount:= wwGetIndexFieldCount(dataSet);
      if IndexFieldCount>0 then FirstIndexField:= wwGetIndexFields(dataSet, 0)
	 ...
	 begin
            result:= wwGetVariableWithParams2(dataset, 'FindKey', [FindText], []);
            exit;
	    ...
	    begin
		 ...
		 if not caseSens then begin
		    ...
		    if (FindText<FirstIndexField.asString) then exit;  {Not found}
               ...
               wwGetVariableWithParams2(dataset, 'FindNearest', [FindText], []);
...
//                  (dataSet as TTAble).findNearest([FindText]);
   ...
   try
	 ...
	 DataSet.updateCursorPos;
      ...
      MatchFound:= False;
      if fldType <> ftMemo then begin
	       {if wwisNonBDEField(curField) then }
         Dataset.DisableControls;  // 4/24/10 - Always disablecontrols now
	 ...
	 Dataset.EnableControls;
      ...
      {if wwisNonBDEField(curField) then } Dataset.EnableControls; // 4/24/10 - Always disablecontrols now
...
begin
  ...
  begin
      DisplayProfessionalVersionOnlyMessage(self, 'TwwLocateDialog is only supported in the professional version');
  end;
...
//      end
   ...
   SearchValue.SetFocus; // 3/7/11 - Focus no longer defaults in Delphi XE
...
{$ifndef GRIDESSENTIALS}
      if ldoCloseOnMatch in (DlgComponent as TwwLocateDialog).Options then
        wwSetString(CancelBtn, 'Caption', BtnCancel)
//        CancelBtn.caption:= BtnCancel
      else
        wwSetString(CancelBtn, 'Caption', BtnClose);
