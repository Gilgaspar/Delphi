...
uses classes, messages, db, forms, wwdbedit, wwdatsrc, stdctrls, extctrls, controls,
      ...
      if curField<>Nil then begin
         with TwwMemoDialogPopup.create(self) do begin
            ...
            if curfield.readonly or
               (wwIsClass(curfield.dataset, 'TTable') and
                wwGetReadOnly(curField.dataset)) then
...
//                              nbCancel,nbRefresh];
  ...
  begin
      DisplayProfessionalVersionOnlyMessage(self, 'TwwRecordViewDialog is only supported in the professional version');
  end;
...
begin
   ...
   if Assigned(wwInternational.OnInitInfoPowerDialog) then
      wwInternational.OnInitInfoPowerDialog(self, Sender as TwwRecordViewForm);
...
var tempMask: string;
    tempCaseSensitive, tempAutoFill, tempUsePictureMask: boolean;
...
begin
   ...
   begin
    ...
    // 11/30/10 - Update regular expression
     wwRegexByField(self, curField.FieldName, tempMask,
       tempCaseSensitive, tempErrorMessage);
     with TwwDBCustomEdit(EditControl) do
     begin
       RegexMask.Mask := tempMask;
       RegexMask.CaseSensitive := tempCaseSensitive;
       RegexMask.ErrorMessage := tempErrorMessage;
       RegexMask.AllowInvalidExit := False;
     end;
   end
end;
...
{               Height:= Canvas.TextHeight('0')*2+8;}
         ...
         else if (curField.dataType = ftMemo) or (curField.dataType = ftWideMemo) // 1/25/10 - Support widememo
         then begin
...
begin
   ...
   with RecordViewForm do begin
      ...
      for i:= 0 to numFields-1 do begin
         ...
         else if (curField.dataType = ftMemo) or (curField.dataType = ftWideMemo) then begin// 1/25/10 - Support widememo
            if EditFrame.IsFrameEffective then
               EditControl:= TwwDBEditMemo.create(ScrollBox)
            else
               EditControl:= TwwDBEditMemo.create(ScrollBox);
...
//   Value: {$ifdef wwDelphi2006Up} TNavButtonSet {$else} TButtonSet {$endif});
   Value: {$ifdef wwDelphi2006Up} TNavButtonSet {$else} TNavButtonSet {$endif});
var i: TNavigateBtn;
begin
  Exit;
  FNavigatorButtons := [];
...
procedure TwwRecordViewDialog.SetRegexMasks(val: TStrings);
begin
