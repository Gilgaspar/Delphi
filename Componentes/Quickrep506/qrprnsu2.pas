...
var
     ...
     qDeviceMode : THandle;
begin
       prSUdlg := TPrintDialog.Create(self);
       // set dialog params
       prSUdlg.Copies := FCopies;
       printer.PrinterIndex := FPrinterIndex;
       printer.Orientation := Orientation;
       prSUdlg.Collate := FCollate > 0;
       prSUdlg.FromPage := FFrompage;
       prSUdlg.ToPage := FTopage;
       prSUdlg.Options := Options;
       prSUdlg.MinPage := FMinPage;
       prSUdlg.MaxPage := FMaxpage;

       if not prSUdlg.Execute() then
       begin
            result := false;
            prSUdlg.Free;
            exit;
