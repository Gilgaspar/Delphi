{ :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  :: 2010/12/04 Add Ansi-encoding properties and code
  :: 7/6/2010 new code to use XLColumn property of controls  ::
  :: 01/08/2011 CSV & Ascii filter memory leak               ::
  ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: }
...
uses
  ...
  QuickRpt, Db, StdCtrls, QRCtrls, QR5Const, Printers, forms,system.UITypes;
...
{$ifndef QRSTANDARD}
  ...
  protected
    ...
    procedure setUseXLCols( value : boolean);
    function getUseXLCols : boolean;
  private
    FReportNum : integer;
    LinesCollection : TStringlist;
  public
    ...
    procedure Finish; override;
    property TextEncoding;
    property UseXLColumns : boolean read getUseXLCols write setUseXLCols;
  end;
...
procedure TQRAbstractExportFilter.TextOutRec( ExportInfo : TExportInfoRec);
var
...
begin
  ...
  with aTextEntry do
  begin
    XPos := ExportInfo.Xpos;
    YPos := ExportInfo.Ypos;
    FText := ExportInfo.Text;
    FAlignment := ExportInfo.Alignment;
    TextFont := TFont.Create;
    TextFont.Assign(ExportInfo.Font);
    Column := ExportInfo.Column;
    ExportAs := ExportInfo.ExportAs;
  end;
...
begin
   ...
   case TextEncoding of
     AnsiEncoding: finaldoc.SaveToFile(Filename, TMBCSEncoding.Create(1252, 0, 0));
...
begin
      ...
      LinesCollection := TStringlist.Create;
      LinesCollection.Sorted := true;
      LinesCollection.Duplicates := dupError;
      PositionLatitude := 1.0;
end;

destructor TQRXLSFilter.destroy;
begin
   LinesCollection.free;
   inherited;
end;
...
procedure TQRXLSFilter.setUseXLCols( value : boolean);
begin
    XLS_UseXLCols := value;
end;

function TQRXLSFilter.getUseXLCols : boolean;
begin
    result := XLS_UseXLCols ;
end;
...
var
  ...
  begin
    ...
    for i := 1 to length(inval) do begin
      if inval[i] <> FormatSettings.ThousandSeparator then
        result := result + inval[i];
    end;
  end;
  function FindCellByRowCol( CCol, CRow : integer ): string;
   var
      ir, ic : integer;
      elist : TStringlist;
   begin
       result := '';
       for ir := 0 to LinesCollection.Count-1 do
       begin
            lineObj := TLineObj(LinesCollection.Objects[ir]);
            if lineObj.YPos <> CRow then continue;
            elist := lineObj.Entries;
            for ic := 0 to elist.Count-1 do
            begin
                if TTextEntry(elist.Objects[ic]).Column = CCol then
                begin
                    result := TTextEntry(elist.Objects[ic]).FText;
                end;
            end;
       end;
   end;
begin
  ...
  if FReportNum > 0 then Sheetrow := FLastSheetrow;

    if XLS_UseXLCols then
   begin
     //trace := TStringlist.Create;
     // assemble the TextEntries into a grid. XPos has been replaced with a row number already.
     ColCount := 0;
     for I := 0 to Entries.count-1 do
     begin
        if not (TObject(Entries[I]) is TTextEntry) then  continue;
        aEntry := TTextentry(Entries[I]);
        if aEntry.Column > ColCount then ColCount := aEntry.Column;
        doneEntry := false;
        for j := 0 to LinesCollection.count-1 do
        begin
             lineObj :=  TLineobj(LinesCollection.objects[j]);
             if abs( lineObj.YPos - aEntry.YPos) = 0 then
             begin
                  lineObj.Entries.AddObject(format('%6.2f',[aEntry.YPos]),aEntry);
                  doneEntry := true;
                  //trace.Add('  Added ''' + aentry.FText + ''' ' + format('%6.2f',[aEntry.YPos])+' ' + format('%4d',[aEntry.Column]));
                  break;
             end;
        end;
        if not doneEntry then
        begin
             lineObj := TLineobj.Create;
             Lineobj.YPos := aEntry.YPos;
             LineObj.Entries.AddObject(format('%6.2f',[aEntry.YPos]), aEntry);
             LinesCollection.AddObject(format('%6.2f',[aEntry.YPos]), lineObj);
             //trace.Add('New ''' + aentry.FText + ''' ' + format('%6.2f',[aEntry.YPos])+' ' + format('%4d',[aEntry.Column]));
        end;
     end;
     LineCount := LinesCollection.Count;
  end;
  ...
  begin
    ...
    begin
        if XLS_UseXLCols then
        begin
           Cell := FindCellByRowCol( J+1, I+1 );
           //trace.Add('Searching : L: ' + inttostr(I+1)+'  C: ' + inttostr(J+1)+ '  res '''+Cell+'''' );
        end
        else
  ...
  // clear up memory
  for I := 0 to LinesCollection.Count-1 do
     TLineObj(LinesCollection.Objects[I]).Free;
...
procedure TQRExcelFilter.setUseXLCols( value : boolean);
begin
    XLS_UseXLCols := value;
end;

function TQRExcelFilter.getUseXLCols : boolean;
begin
    result := XLS_UseXLCols ;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      EnumFontFamiliesEx(DC, LFont, @QRRTFEnumFontsProc, NativeInt(RTFFontList), 0);
    end
    else
