{ :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  :: Unicode version                                         ::
  :: 27/11/2008 Adding Export code                           ::
  :: 22/03/2010 Added ForceLeftJustify for csv export bug    ::
  :: 23/05/2011 added exporting to TQRPCheckbox              ::
  :: 22/07/2011 QRPLabel with null rotated text bug fixed    ::
  :: 19/08/2011 Fixed TQRPGrid export code for PDF           ::
  :: 11/06/2012 fixed rotated fonts                          ::
  :: 07/07/2014 added 'OnCellPaint' event to TQRPGrid,       ::
  ::            and Picture property to TGridCell
  :: 06/08/2014 Added XLNumFormat and exporting to xlsx code ::
  ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: }
...
uses
  {$ifndef VER220}Winapi.Windows{$else}Windows{$endif}, ComCtrls,  Controls, SysUtils, Forms, Grids,  DBGrids,  Classes,
  QuickRpt, QRCtrls, CSProp, Graphics, ExtCtrls, QRExpr, grImgCtrl, {$ifndef DXE}system.UITypes,{$endif}
...
type
  TCPEventHandler  = procedure( sender : TObject; Xp,Yp : integer; cellRect : TRect; pCanvas : TCanvas) of object;
  TDLEventHandler  = procedure( sender : TObject; X1,Y1,X2,Y2 : double; pCanvas : TCanvas;
                               var lbmp : TBitmap; var Done : boolean; var useBmp : boolean) of object;
  ...
  private
    ...
    FCellColumns       : TList;
    FOnPaintCell : TCPEventHandler;
    FOnDrawLine  : TDLEventHandler;
    FOnPrintImage  : TCPEventHandler;
    FLinebmp     : TBitmap;
    FXLColumn : integer;
  ...
  published
    ...
    property OnPaintCell : TCPEventHandler Read FOnPaintCell Write FOnPaintCell;
    property OnDrawLine : TDLEventHandler Read FOnDrawLine Write FOnDrawLine;
    property OnPrintImage  : TCPEventHandler Read FOnPrintImage Write FOnPrintImage;
...
uses
  Math, Dialogs, DB, QRExpbld, qrnewxlsxfilt;
...
begin
  ...
  case aValue.Kind of
    ...
    resString : Caption:=aValue.StringVal;
...
begin
  ...
  If aValue.Kind=resString Then
    EvalStringExpression:=aValue.StringVal
  Else
    EvalStringExpression:='';
  Evaluator.Unprepare;
  Evaluator.Free;
...
var
  ...
  procedure ExportLine( VorH : string; Xpos, Ypos, len : integer; lcolor : TColor);
  begin
       expShape := TQRShape.Create(self);
       if VorH = 'H' then
       begin
         expShape.Shape := qrsHorline;
         // bodge.
         expShape.Width := len + 1*round((screen.PixelsPerInch/254)/qrprinter.XFactor);
         expShape.Height := 1;
       end
       else
       begin
         expShape.Shape := qrsVertLine;
         expShape.Width := 1;
         expShape.Height := len;
       end;
       expShape.Pen.Style := psSolid;
       expShape.Pen.Width := 1;
       expShape.Brush.Style := bsClear;
       expShape.Pen.Color := lcolor;
       TQuickrep(parentreport).ExportFilter.AcceptGraphic( XPos, YPos, expShape);
       expShape.free;
  end;

  procedure ExportLineImage( VorH : string; Xpos, Ypos, len : integer; lcolor : TColor);
  begin
        expimage := TQRImage.Create(self);
        if VorH = 'H' then
              expimage.Name := 'LineImage_H'
        else
              expimage.Name := 'LineImage_V';

        expimage.Width := FLinebmp.Width;
        expimage.Height := FLinebmp.Height;
        expimage.Picture.Assign(FLinebmp);
        TQuickrep(parentreport).ExportFilter.AcceptGraphic( xpos, ypos, expimage);
        expimage.free;
...
begin
        ...
        begin
          ...
          If not Transparent Then
            FillRect(rect(XPos(OfsX + Size.Left),
                          ...
                          YPos(OfsY + Size.Top + Size.Height)));
          {---- export grid background ---}
          If not Transparent and TQuickrep(parentreport).exporting and (brush.Color<>clWhite) Then
          begin
            expShape := TQRShape.Create(self);
            expShape.Shape := qrsRectangle;
            expShape.Pen.Style := psClear;
            expShape.Brush.Style := bsSolid;
            expShape.Brush.Color := Brush.Color;
            expShape.Width := self.Width;
            expShape.Height := self.Height;
            TQuickrep(parentreport).ExportFilter.AcceptGraphic( XPos(OfsX + Size.Left),YPos(OfsY + Size.Top), expShape);
            expShape.free;
          end;
          AllWidth:=Size.Width;
          If Frame.DrawLeft Then
              ...
              For X:=0 to Columns-1 Do
                begin
                  ...
                  begin
                      ...
                      {----------- export cell background -----------}
                      If TQuickrep(parentreport).exporting  and (brush.Color<>clWhite) Then
                      begin
                         expShape := TQRShape.Create(self);
                         expShape.Shape := qrsRectangle;
                         expShape.Pen.Style := psClear;
                         expShape.Brush.Style := bsSolid;
                         expShape.Brush.Color := Cells[X,Y].Brush.Color;
                         expShape.Width := round(GetColWidth(X)*QRPrinter.XFactor);
                         expShape.Height := round(GetRowHeight(Y)*QRPrinter.YFactor);
                         TQuickrep(parentreport).ExportFilter.AcceptGraphic( XPos(OfsX + Size.Left + AX + DX),
                                                                             YPos(OfsY + Size.Top + AY + DY),
                                                                             expShape);
                         expShape.free;
                      end;
                      pictrect := Rect(XPos(OfsX + Size.Left + AX + DX),  YPos(OfsY + Size.Top + AY + DY),
                                    XPos(OfsX + Size.Left + AX + GetColWidth(X) + DX),
                                    YPos(OfsY + Size.Top + AY + GetRowHeight(Y) + DY));
                      if assigned( FonPrintImage) then
                      begin
                          FOnPrintImage( self.Cells[X,Y], X,Y,pictrect, canvas);
                      end;
                      {-------------------- print cell picture ------------}
                      if Cells[X,Y].Picture <> nil then
                      begin
                           pictrect := Rect(XPos(OfsX + Size.Left + AX + DX + (Cells[X,Y].PicXoff * QRPrinter.XFactor)),
                                            YPos(OfsY + Size.Top + AY + DY + (Cells[X,Y].PicYoff * QRPrinter.YFactor)),
                              XPos(OfsX + Size.Left + AX + (cells[X,Y].Picture.Width * QRPrinter.XFactor) + (Cells[X,Y].PicXoff * QRPrinter.XFactor) + DX),
                              YPos(OfsY + Size.Top + AY + (cells[X,Y].Picture.Height * QRPrinter.YFactor)  + DY + (Cells[X,Y].PicYoff * QRPrinter.YFactor)));
                           if qrprinter.Destination=qrdPrinter then
                              canvas.StretchDraw(pictrect, Cells[X,Y].Picture.Graphic)
                           else
                              canvas.Draw(pictrect.Left + Cells[X,Y].PicXoff, pictrect.Top + Cells[X,Y].PicYoff, Cells[X,Y].Picture.Graphic);
                           If TQuickrep(parentreport).exporting Then
                           begin
                                expimage := TQRImage.Create(self);
                                expimage.Name := self.Name+trim(inttostr(X))+trim(inttostr(Y));
                                expimage.Width := Cells[X,Y].Picture.Graphic.Width;
                                expimage.Height := Cells[X,Y].Picture.Graphic.Height;
                                expimage.Picture.Assign(Cells[X,Y].Picture.Graphic);
                                TQuickrep(parentreport).ExportFilter.AcceptGraphic( XPos(OfsX + Size.Left + AX + DX+ (Cells[X,Y].PicXoff / QRPrinter.XFactor)),
                                                                        YPos(OfsY + Size.Top + AY + DY + (Cells[X,Y].PicYoff / QRPrinter.YFactor)), expimage);
                                expimage.free;
                           end;
                      end;
            ...
            begin
              ...
              begin
                  ...
                  begin
                      If FirstColumnJoined Then
                      begin
                        MoveTo(XPos(OfsX + Size.Left + GetColWidth(0) + DX), YPos(OfsY + Size.Top + P+DY));
                        lineX := trunc(XPos(OfsX + Size.Left + GetColWidth(0) + DX));
                        lineY := trunc(YPos(OfsY + Size.Top + P+DY));
                      end
                      Else
                      begin
                        MoveTo(XPos(OfsX + Size.Left), YPos(OfsY + Size.Top + P+DY));
                        lineX := trunc(XPos(OfsX + Size.Left));
                        lineY := trunc(YPos(OfsY + Size.Top + P+DY));
                      end;
                      cancelLine := false;
                      useLinebmp := false;
                      Flinebmp.Width := self.Width;
                      Flinebmp.Height := 1;
                      if assigned(FOnDrawline) then
                                 FOnDrawline(self,XPos(OfsX + Size.Left), YPos(OfsY + Size.Top + P+DY),linex,liney,canvas,FLinebmp, cancelLine,useLinebmp);
                      if useLinebmp then
                      begin
                          canvas.Draw(XPos(OfsX + Size.Left), YPos(OfsY + Size.Top + P+DY), Flinebmp);
                          // export the grid line
                          if TQuickrep(parentreport).exporting then
                            ExportLineImage( 'H', lineX, lineY, round(allWidth*QRPrinter.XFactor), clBlack);
                      end
                      else if not cancelLine then
                      begin
                          LineTo(XPos(OfsX + Size.Left + Size.Width), YPos(OfsY + Size.Top + P+DY));
                          // export the grid line
                          if TQuickrep(parentreport).exporting then
                            ExportLine( 'H', lineX, lineY, round(allWidth*QRPrinter.XFactor), clBlack);
                      end;
                  end;
              end;
          end;
            ...
            begin
              ...
              For X:=1 to Columns-1 Do
                begin
                  ...
                  If FirstRowJoined Then
                  begin
                    MoveTo(XPos(OfsX + Size.Left + P+DX), YPos(OfsY + Size.Top + GetRowHeight(0)+DY));
                    lineX := trunc(XPos(OfsX + Size.Left + P+DX));
                    lineY := trunc(YPos(OfsY + Size.Top + GetRowHeight(0)+DY));
                  end
                  Else
                  begin
                    MoveTo(XPos(OfsX + Size.Left + P+DX), YPos(OfsY + Size.Top));
                    lineX := trunc(XPos(OfsX + Size.Left + P+DX));
                    lineY := trunc(YPos(OfsY + Size.Top));
                  end;
                      cancelLine := false;
                      useLinebmp := false;
                      Flinebmp.Width := self.Width;
                      ...
                      begin
                          ...
                          // export the grid line
                          if TQuickrep(parentreport).exporting then
                            ExportLineImage( 'V', lineX, lineY, round(allWidth*QRPrinter.XFactor), clBlack);
                      end
                      else if not cancelLine then
                      begin
                         LineTo(XPos(OfsX + Size.Left + P+DX), YPos(OfsY + Size.Top + Size.Height));
                         // export the grid line
                         if TQuickrep(parentreport).exporting then
                            ExportLine( 'V', lineX, lineY, self.Height, clBlack);
                      end;
                end;
            end;
                ...
                begin
                  If Cells[X,Y].Caption<>'' Then
                    begin
                      ...
                      {------------ call cellPaint event ----------------}
                      if assigned(FOnPaintCell) then
                            FOnPaintCell( self, X, Y, arect, Canvas );
                      ...
                      If TQuickrep(parentreport).exporting Then
                      begin
                            if ( TQuickrep(parentreport).ExportFilter is TQRXLSXDocumentfilter ) then
                            begin
                               ginforec.Font := canvas.Font;
                               ginforec.Xpos := Xpos(AX);
                               ginforec.Ypos := Ypos(AY) + self.top;
                               if (X=0) and (Y=0) and (self.FirstCellColor <> clWhite) then
                                  ginforec.BGColor := FirstCellColor
                               else if (Y=0) and (self.FirstRowColor <> clWhite) then
                                  ginforec.BGColor := FirstRowColor
                               else if (X=0) and (self.FirstColumnColor <> clWhite) then
                                  ginforec.BGColor := FirstColumnColor
                               else
                                   ginforec.BGColor := cells[X,Y].Brush.Color;
                               ginforec.Text := cells[X,Y].Caption;
                               ginforec.Column := FXLColumn + X;
                               ginforec.ExportAs := cells[X,Y].ExportType;
                               ginforec.XLNumFormat := cells[X,Y].XLNumFormat;
                               if cells[X,Y].HorizAlignment = haCenter then
                                    ginforec.Alignment := TAlignment.taCenter
                               else if cells[X,Y].HorizAlignment = haRight then
                                    ginforec.Alignment := TAlignment.taRightJustify
                               else
                                    ginforec.Alignment := TAlignment.taLeftJustify;
                               TQuickrep(parentreport).ExportFilter.TextOutRec(gInfoRec);
                            end
                            else
                               TQuickrep(parentreport).ExportFilter.TextOut(AX, AY, canvas.Font, canvas.Brush.color, taLeftJustify, cells[X,Y].Caption);
                      end;
                    end;
  ...
  transparent := true;
  if Frame.AnyFrame and enabled then
    TQRPFrame(Frame).PaintIt(ParentReport.QRPrinter.Canvas,
      Rect(ParentReport.QRPrinter.XPos(OfsX + Size.Left),
      ParentReport.QRPrinter.YPos(OfsY + Size.Top),
      ParentReport.QRPrinter.XPos(OfsX + Size.Left + Size.Width),
...
begin
  ...
  if (ParentReport.FinalPass or ParentReport.Exporting )and Enabled then
    begin
      QRPrinter.Canvas.Brush := Brush;
      QRPrinter.Canvas.Pen := Pen;
      with QRPrinter do
                    ...
                    begin
                      ...
                      if GlyphChecked.Graphic is TBitmap then
                        ...
                        with QRPrinter.Canvas do
                          StretchDraw(Dest, GlyphChecked.Graphic);
                      // export it
                      if TQuickrep(qrprinter.ParentReport).Exporting then
                      begin
                                expImage := TQRImage.Create(self);
                                expImage.Width := self.Width;
                                expImage.Height := self.Height;
                                //expImage.Picture := TPicture.Create;
                                expImage.Picture.Assign(GlyphChecked);
                                TQuickrep(qrprinter.ParentReport).ExportFilter.AcceptGraphic(
                                      XPos(OfsX + Size.Left), QRPrinter.YPos(OfsY + Size.Top),expImage);
                                expImage.Free;
                      end;
                    end;
                end
                  ...
                  If CheckmarkType=cmGlyph then
                    begin
                      ...
                      begin
                                ...
                                expImage.Width := self.Width;
                                expImage.Height := self.Height;
                                expImage.Picture.Assign(GlyphUnchecked);
                                TQuickrep(qrprinter.ParentReport).ExportFilter.AcceptGraphic(
                                      XPos(OfsX + Size.Left), QRPrinter.YPos(OfsY + Size.Top),expImage);
                                expImage.Free;
                      end;
...
{CH function Max(I1, I2: Integer): Integer;
begin
  If I1>I2 Then
    Result:=I1
  else
