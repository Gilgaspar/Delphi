{===============================================================
 ...
 8/03/2010 Fixed Row select bug in selectpage() 'if ... and showthumbs'
 QR6
 04/09/2013 added setup from preview controller
================================================================}
...
type
  ...
  { TQRXMLPreviewInterface }
  TQRXMLPreviewInterface = class(TComponent)
   public
    procedure Show ; virtual;
    procedure ShowModal; virtual;
    function GetImageControl : TImage; virtual;
    procedure SetDocument( aDoc : TQRXDocument );virtual;
  end;

  TQRXMLPreviewInterfaceClass = class of TQRXMLPreviewInterface;

  TQRXMLStandardPreviewInterface = class(TQRXMLPreviewInterface)
  private
  ...
  public
    ...
    function GetImageControl : TImage; override;
    procedure SetDocument( aDoc : TQRXDocument ); override;
  end;
  ...
  var
    GlobalXMLPreviewInterface : TQRXMLPreviewInterfaceClass;

implementation
uses QRXSearchfrm, clipbrd, qrxmlsfilt;
   var
...
{ TQRXMLPreviewInterface }
procedure TQRXMLPreviewInterface.Show ;
begin
     //
end;

procedure TQRXMLPreviewInterface.Showmodal;
begin
     //
end;

function TQRXMLPreviewInterface.GetImageControl : TImage;
begin
     result := nil;
end;

procedure TQRXMLPreviewInterface.SetDocument( aDoc : TQRXDocument );
begin

end;

  { TQRXMLStandardPreviewInterface }
constructor TQRXMLStandardPreviewInterface.Create(Owner : TComponent);
begin
    inherited create(Owner);
    FStandPrev := TQRXStandardViewer.Create(application);
end;

procedure TQRXMLStandardPreviewInterface.Show ;
begin
     FStandPrev.Show;
end;

procedure TQRXMLStandardPreviewInterface.Showmodal;
begin
     FStandPrev.ShowModal;
end;

function TQRXMLStandardPreviewInterface.GetImageControl : TImage;
begin
     result := FStandPrev.GetImage;
end;

{}
procedure TQRXMLStandardPreviewInterface.SetDocument( aDoc : TQRXDocument );
begin
     FStandPrev.SetDocument( aDoc);
end;
...
function TQRXStandardViewer.GetImage:TImage;
begin
     result := Image1;
end;

procedure TQRXStandardViewer.SetupFromController(prevcon : TQRPreviewController);
begin
     FormStyle := prevcon.PrevWindowStyle;
     ShowThumbs := prevcon.ShowThumbs;
     ShowSearch := prevcon.ShowFindButton;
     FZoomState := prevcon.PrevInitZoom;
     //reportTitle := TCustomquickrep(qrprinter.ParentReport).ReportTitle;
     //QRPrinter.Title := reportTitle;
     FReportOrientation := FQRXDocument.Quickrep.Page.Orientation;
     ...
     //ZoomPickButton.Visible := prevcon.ShowZoomPickButton;
     ...
     FInitLeft := prevcon.PrevInitLeft;
     FInitTop := prevcon.PrevInitTop;
     // system menu
     ...
     if prevcon.PrevInitZoom = qrZoomToFit then
                 ZoomToFitClick( self)
     else if prevcon.PrevInitZoom = qrZoomToWidth then
                 ZoomToWidthClick( self)
     else if prevcon.PrevInitZoom = qrZoom100 then
                 ZoomTo100Click( self);
end;
...
begin
     ...
     Caption := FQRXDocument.ReportTitle;
     if FQRXDocument.Quickrep.PreviewController<>nil then
     begin
          SetupFromController( FQRXDocument.Quickrep.PreviewController);
     end
     else
     begin
       FReportOrientation := FQRXDocument.Quickrep.Page.Orientation;
       ...
       FInitTop := FQRXDocument.InitViewerTop;
       FInitLeft := FQRXDocument.InitViewerLeft;
...
begin
     ...
     UpdateInfo;
     // set thumb row
     if (aPagenumber > 0) and ( aPagenumber <= FQRXDocument.PageCount) and Showthumbs then
     begin
       ThumbUpdating := true;
...
procedure TQRXStandardViewer.PositionImage;
begin
     if image1.width >= self.ScrollBox1.Width then exit;
...
{$ifdef SAMPLE1}
     //  report recreation : this code is for use in custom previewers
     //  to allow users to filter or amend the report, then recreate it in
     //  the browser.
     xmlfilt := TQRXDocumentFilter.Create('');
     // make changes to the report before ...
     FQRXDocument.Quickrep.ExportToFilter(xmlfilt);
     xmlfilt.Stream.Position := 0;
     FQRXDocument.LoadFromStream( xmlfilt.stream);
     self.SetDocument(FQRXDocument);
{$endif}
end;
