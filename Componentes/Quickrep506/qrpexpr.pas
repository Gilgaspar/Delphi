...
uses
  Windows, Dialogs,
  QRCtrls, CSProp, Forms, Classes, SysUtils, QRExpBld, QRPrntr,
  QuickRpt, Registry, QRExpr,  IniFiles, DB, qrpBaseCtrls, qrpCtrls;
...
begin
  ...
  try
    Result.stringVal:=chr(Argument(0).IntResult);
...
begin
  ...
  try
    CompName:=Argument(0).stringVal;
    FieldName:=Argument(1).stringVal;
    C:=nil;
    For I:=1 to Screen.FormCount Do
      begin
...
begin
  Result.Kind:=resString;
  Result.stringVal:=FormatSettings.CurrencyString;
end;
...
begin
  ...
  try
    DecodeTime(StrToTime(Argument(0).stringVal),_H,_M,_S,_MS);
    H:=_H; S:=_S; M:=_M;
    ...
    Result.stringVal:=TimeToStr(EncodeTime(H,M,S,_MS));
    Result.Kind:=resString;
  except
  end;
end;
...
begin
  ...
  If (ArgList.Count<>4) OR
     ...
     (Argument(3).Kind<>resInt) Then Exit;
  try
    DecodeDate(StrToDateTime(Argument(0).stringVal),Y,M,D);
    ...
    Result.stringVal:=DateToStr(EncodeDate(Y,M,D));
    Result.Kind:=resString;
  except
  end;
end;
...
begin
  ...
  If (ArgList.Count<>1) OR (Argument(0).Kind<>resString) Then Exit;
  try
    Result.IntResult:=DayOfWeek(StrToDateTime(Argument(0).stringVal));
    Result.Kind:=resInt;
  except
  end;
end;
...
begin
  ...
  If (ArgList.Count<>1) OR (Argument(0).Kind<>resInt) Then Exit;
  try
    Result.stringVal:=FormatSettings.LongDayNames[Argument(0).IntResult];
    Result.Kind:=resString;
  except
  end;
end;
...
begin
  ...
  If (ArgList.Count<>1) OR (Argument(0).Kind<>resInt) Then Exit;
  try
    Result.stringVal:=FormatSettings.LongMonthNames[Argument(0).IntResult];
    Result.Kind:=resString;
  except
  end;
end;
...
{$ifdef BDETABLES}
  ...
  try
    CompName:=Argument(0).stringVal;
    ParamName:=Argument(1).stringVal;
    C:=NIL;
    For I:=1 to Screen.FormCount Do
      begin
        C:=Screen.Forms[I-1].FindComponent(CompName);
        If (C<>NIL) AND (C is TxQuery) Then Break;
      end;

    If (C<>NIL) AND (C is TxQuery) Then
      Case TxQuery(C).ParamByName(ParamName).DataType of
        ftDate       : Result.stringVal:=DateToStr(TxQuery(C).ParamByName(ParamName).AsDateTime);
        ftTime       : Result.stringVal:=TimeToStr(xTQuery(C).ParamByName(ParamName).AsDateTime);
        ftDateTime   : Result.stringVal:=DateTimeToStr(TxQuery(C).ParamByName(ParamName).AsDateTime);
      else
        Result.stringVal:=TxQuery(C).ParamByName(ParamName).AsString;
      end;
...
begin
  ...
  try
    TI:=TIniFile.Create(ExpandFileName(Argument(0).stringVal));
    Result.stringVal:=TI.ReadString(Argument(1).stringVal,Argument(2).stringVal,'');
    TI.Free;
    Result.Kind:=resString;
  except
  end;
...
begin
  ...
  try
    {$IFDEF WIN32}
    TI:=TRegIniFile.Create(Argument(0).stringVal);
    Result.stringVal:=TI.ReadString(Argument(1).stringVal,Argument(2).stringVal,'');
    TI.Free;
    {$ELSE}
    Result.stringVal:='';
...
begin
  ...
  If (ArgList.Count<>2) OR
     (Argument(0).Kind<>resString) OR (Argument(1).Kind<>resString) Then Exit;
  try
    DT:=StrToDateTime(Argument(0).stringVal);
    Result.stringVal:=FormatDateTime(Argument(1).stringVal, DT);
    Result.Kind:=resString;
  except
  end;
end;
...
begin
  ...
  If (ArgList.Count<>1) OR (Argument(0).Kind<>resString) Then Exit;
  try
    Result.IntResult:=StrToInt(Argument(0).stringVal);
    Result.Kind:=resInt;
  except
  end;
end;
...
begin
  ...
  If (ArgList.Count<>1) OR (Argument(0).Kind<>resString) Then Exit;
  try
    Result.DblResult:=StrToFloat(Argument(0).stringVal);
    Result.Kind:=resDouble;
  except
  end;
end;
...
begin
  ...
  try
    ...
    If C<>NIL Then
      begin
        If C is TQRExpr Then Result.stringVal:=TQRExpr(C).Caption
        Else
          Result.stringVal:=GetStringProperty(C,'Caption');
        Result.Kind:=resString;
      end;
  except
  end;
...
begin
  ...
  try
    ...
    Result.IntResult:=TDataset(C).RecordCount;
{$ifdef BDETABLES}
    If C<>NIL Then
      begin
        If C is TxTable Then
          begin
            Result.IntResult:=TxTable(C).RecordCount;
            Result.Kind:=resInt;
          end
        Else
        If C is TxQuery Then
          begin
            Result.IntResult:=TxQuery(C).RecordCount;
            Result.Kind:=resInt;
          end;
      end;
{$endif}
