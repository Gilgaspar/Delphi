...
//      16/03/2010 Multipage mode fixed by Henk van Bokhorst
//      16/03/2010 Constants for 'Next Page' etc added
//      14/04/2010 fixed export to stream bug. See 'Finish'.
////////////////////////////////////////////////////////////////////////////
{$define VER36}
interface
uses windows, classes, controls, stdctrls,sysutils, graphics, buttons,
     forms, extctrls, dialogs, printers, db, ComCtrls, qrexport,
     QRPrntr, Quickrpt, QR5Const, qrctrls, grimgctrl, jpeg, system.UITypes;
...
var
  enc : TQRTextEncoding;
  ansienc : TMBCSEncoding;
begin
  if not HTMMultiPage and (filename <> '') then
  begin
    enc := TQRTextEncoding.Create;
    ansienc := TMBCSEncoding.Create(1252, 0, 0);
    case TextEncoding of
      AnsiEncoding: finaldoc.SaveToFile(Filename, ansienc);
  ...
  if filename = ''  then
  begin
      finaldoc.SaveToStream(stream);
  end;
  if not HTMMultiPage and (filename <> '' ) then CloseStream;
  FreeAndNil(FFont);
  FreeAndNil(FActiveFont);
...
begin
    try
        ...
        if parentrep.DataSet <> nil 
        then HTMLastPage := ( parentrep.DataSet.Eof and ( HTMConcatcount = FReportNumber ))
        else HTMLastPage := ( (parentrep.QRPrinter.Progress = 100) and ( HTMConcatcount = FReportNumber ));
    except
    end;
    if not MultiPage or HTMFirstPage then
    begin
        if not HTMFirstPage then inc(FPageNumber);
        HTMFirstPage := false;
        exit;
    end;
    ...
    begin
      ...
      begin
          ...
          nextstr := '<a href="'+ LinkPrefix + hfilename +'">' + sqrHTMLNextPage + '</a>';
      end;
      if fpagenumber > 1 then
      begin // put in a back link
         hfilename := basename(filename) + format( '%-d.html', [FPageNumber-1] );
         prstr := '<a href="'+ LinkPrefix + hfilename +'">' + sqrHTMLPrevPage + '</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
      end;
      ...
      begin
          ...
          begin
             ...
             laststr := '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="'+ hfilename +'">' + sqrHTMLLastPage + '</a>';
          end;
          if HTMLastPage then
          begin
             hfilename := basename(filename) + format( '%-d.html', [1] );
             firststr := '<a href="'+ LinkPrefix + hfilename +'">' + sqrHTMLFirstPage + '</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
          end;
      end;
    ...
    enc := TQRTextEncoding.Create;
    case TextEncoding of
      ASCIIEncoding: finaldoc.SaveToFile(hfilename, enc.ASCII);
      BigEndianUnicodeEncoding:  finaldoc.SaveToFile(hfilename, enc.BigEndianUnicode);
      DefaultEncoding: finaldoc.SaveToFile(hfilename, enc.ASCII);
      UnicodeEncoding: finaldoc.SaveToFile(hfilename, enc.Unicode);
      UTF7Encoding: finaldoc.SaveToFile(hfilename, enc.UTF7);
      UTF8Encoding: finaldoc.SaveToFile(hfilename, enc.UTF8);
    end;
    finaldoc.clear;
    enc.Free;
    CloseStream;
...
begin
    ...
    begin
        ...
        yp := round((FPagenumber-1) * (mmToPix*FPaperHeight/10.0));
        writetostream( '<a name="page'+inttostr(Fpagenumber)+'" ID="page'+inttostr(Fpagenumber)+'" class="pagemarker" style="position:absolute;left:0px;top:'
                    + format( '%-d', [yp]) +'px;"></a>' );
        exit;
    end;
    HTMFirstPage := false;
...
begin
      ...
      try
        ...
        // ********* SINGLE FORM
        yp := round(((FPagenumber-1) * (mmToPix*FPaperHeight/10.0)) + yp);
        FixDivStr;
...
begin
  ...
  try
    ...
    // ********* SINGLE FORM
    gdest.Top := round(((FPagenumber-1) * (mmToPix*FPaperHeight/10.0)) + gdest.Top);
