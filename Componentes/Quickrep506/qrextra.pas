{ :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  :: 06/01/2009 fixed metafile search to ansistring          ::
  :: 26/04/2009 amended listbuilder to use qrdbtext
  ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: }
...
begin
  ...
  if not MFMatchCase then aStr := ansistring(AnsiUppercase(string(AStr)));
  if ansiPos(MFSearchStr, string(aStr)) > 0 then
  begin
    Result := 0;
    MFFound := true;
  end else
...
{$endif}
    ...
    // fixes 'originalQRPrinter' bug
    FQuickrep.QRPrinter := FQRPrinter;
    FFilename := Value;
...
{$else}
      // replace the qrexpr with QRDBText
      aDBText := TQRDBText(Report.Bands.DetailBand.AddPrintable(TQRDBText));
      aDBText.AutoSize := true;
      aDBText.AutoStretch := false;
      aDBText.DataField :=AField.FieldName;
      aDBText.Left := aLabel.Left;
      aDBText.Width := aLabel.Width;
      aDBText.Alignment := aField.Alignment;
      aDBText.DataSet := Dataset;
      if (aDBText.Left + aDBText.Width > Report.Bands.DetailBand.Width) and
        (Orientation = poPortrait) then
        Orientation := poLandscape;
      if aDBText.Left + aDBText.Width > Report.Bands.DetailBand.Width then
      begin
        aLabel.Free;
        aDBText.Free;
      end;
...
{$ifdef DO_BLOBS}
    ...
    begin
      ...
      aLabel.Caption := aField.DisplayName;
      aLabel.Frame.DrawBottom := true;
      // replace the qrexpr with QRDBText for memo fields
      aDBText := TQRDBText(Report.Bands.DetailBand.AddPrintable(TQRDBText));
      aDBText.AutoSize := false;
      aDBText.AutoStretch := true;
      aDBText.DataField :=AField.FieldName;
      aDBText.Left := aLabel.Left;
      aDBText.Width := aLabel.Width;
      aDBText.Alignment := aField.Alignment;
