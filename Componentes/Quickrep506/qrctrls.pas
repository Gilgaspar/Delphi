{ :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  :: 1/6/2010 XL filter Added XLColumn property+new code in
  ::            PrintToCanvas
  :: 11/06/2010 fixed QRExpr to work with unicode
  :: 28/06/2010 QRDBRichText : changed Unicode to ansi conv.
  :: 12/06/2012 QRDBRichText : fixed blank field bug
  :: 09/12/2013 add HiresExport boolean property to customrichtext
  :: 14/08/2014 add Transparent boolean property to customrichtext
  :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: }
...
type
  TWrapStyle = (BreakOnSpaces, BreakAnywhere);
  ...
  private
    ...
    FWordWrap : boolean;
    FFullJustify : boolean;
    FMaxBreakChars : integer;
  ...
  public
    ...
    property Alignment;
    property VerticalAlignment : TTextLayout read FVerticalAlignment write FVerticalAlignment;
    ...
    property WrapStyle : TWrapStyle read FWrapStyle write FWrapStyle stored true;
    ...
    property ExportAs;
    property FullJustify : boolean read FFullJustify write FFullJustify;
    property MaxBreakChars : integer read FMaxBreakChars write FMaxBreakChars default 5;
    property XLColumn;
    property XLNumFormat;
    property ParagraphChar;
  ...
  published
    ...
    property WordWrap;
    property OnPrint;
    property FullJustify;
    property MaxBreakChars;
    property XLColumn;
    property XLNumFormat;
    property ParagraphChar;
    property ParagraphIndent;
  ...
  private
    ...
    FieldNo : integer;
    FieldOK : boolean;
    DataSourceName : string{$IFNDEF UNICODE}[30]{$ENDIF};
    FDataSet : TDataSet;
    FDataField : string;
    FMask : string;
    IsMemo : boolean;
...
{$endif}
  ...
  private
    ...
    procedure SetRTFTransparent( value : boolean);
    function  getRTFTransparent:boolean;
  protected
    procedure CreateWnd;  override;
    procedure SetAlignment(Value : TAlignment); override;
  ...
  published
    ...
    property HiresExport : boolean read FHiresExport write FHiresExport;
    property XLColumn;
    property Transparent : boolean read getRTFTransparent write setRTFTransparent;
...
{ TQRDBCalc - included for backwards compatibility }
  ...
  var
    ...
    LogX, LogY : integer;
     ...
     globalTransparentRTF : boolean;

implementation

uses
  QRLablEd, QRExprEd, qrexport, qrxlsxfilt, qrnewxlsxfilt;
...
{TQRRichEdit}
procedure TQRRichEdit.CreateParams(var params: TCreateParams);
begin
  inherited CreateParams(Params);
end;

procedure TQRRichEdit.CreateWnd;
begin
  inherited;
  if globalTransparentRTF then
...
var
  ...
  begin
    ...
    begin // no #13 (LF) in line
      J := 0;
      NewLine := '';
      LineFinished := false;
      if AutoSize then  // stretch horizontally
      begin
        NewLine := Line;
        FlushLine;
        LineFinished := True;
      ...
      begin
        // stretching : break text into words and build a line
        if Wrapstyle = BreakOnSpaces then
        begin
          while (J < Length(Line)) and (Length(Line) > 0) do
          begin
            repeat // look for a breakchar
              inc(J)
            until CharInSet(Line[J],[' ', #13, '-']) or (J >= Length(Line));
            AddWord(copy(Line, 1, J));// copy out the word
            Delete(Line, 1, J);
            J := 0;
          end;
        ...
        else // wrapstyle = breakanywhere
        begin
          J := 1;
          while (J <= Length(Line)) and (Length(Line) > 0) do
          begin
               tstr := copy(Line, J, 1);
               if aLineWidth(NewLine + tstr) <= Width then
                   Newline := Newline + tstr
               else
               begin
                   FlushLine;
                   Newline := Newline + tstr;
  ...
  begin
    ...
    begin
      UpdatingBounds := true;
      if Height < (longint(ParentReport.TextHeight(Font, 'Wp') * Zoom div 100) + 1) then
         Height := (longint(ParentReport.TextHeight(Font, 'Wp')) * Zoom div 100) + 1;
      UpdatingBounds := false;
    end
  end;
...
{  if AutoSize then Flags := 0 else Flags := ETO_CLIPPED;}
  ...
  if FFormattedLines.Count = 1 then
  begin
    tmpx := aCanvas.TextHeight('Wq');
    //tmpx := (height - tmpx)/QRPrinter.YFactor - tmpx;
    tmpx := (height - tmpx);
    vpos := Starty;
    case self.VerticalAlignment of
       TTextlayout.tlCenter  :  VPos := round(StartY + tmpx/2);
       TTextlayout.tlBottom  :  VPos := round(StartY + tmpx);
    end;
    Cap := FFormattedLines[0];
...
var
  ...
  procedure JustifyLine( aCanvas : TCanvas; x : extended; var y: extended; flags : integer;
                             rect : TRect; text : string; textlen : integer);
  var
     words : TStringlist;
     wordslen, linelen, gap, xfact, xpos, spacewidth : extended;
     k, ypos : integer;
     wordstr : string;
  begin
    ...
    try
       ...
       wordslen := 0.0;
       xfact := (1.0*QRPrinter.YPos(100000)/100000.0)/0.37795;
       //trace.Add('> ' + text );
       for k := 0 to words.Count-1 do
       begin
              if (k=0) and (words[k][1]=ParagraphChar) and (ParagraphChar <> '') then
                 words[0] := ParagraphIndent + copy(words[0],2,255);
           wordslen := wordslen + acanvas.TextWidth(words[k]);
           //trace.Add( + words[k] + '  ' + inttostr(acanvas.TextWidth(words[k])))
       end;
       linelen := self.Width*xfact;
       gap := acanvas.TextWidth(' ');
       spacewidth := gap;
       if (words.Count > 1) and (linelen > wordslen) then
           gap := (linelen - wordslen)/(words.Count-1);
       if (gap/spacewidth) > MaxBreakChars then gap := spacewidth;
       xpos := QRPrinter.YPos(X);
       ypos := QRPrinter.YPos(Y);
       for k := 0 to words.Count-1 do
       begin
            wordstr := words[k];
            if not parentreport.Exporting then
               ExtTextOut(aCanvas.Handle, round(xpos), ypos,  Flags, @Rect, @wordstr[1], length(wordstr), nil)
            else
               ParentReport.ExportFilter.TextOut(round(xpos/0.37795), Y, Font, Color, AAlignment, wordstr);
            xpos := xpos + acanvas.TextWidth(words[k]) + gap;
       end;
       //trace.Add('  >  words ' + inttostr(words.Count) + ' wlen ' + floattostr(wordslen)+ ' gap ' + floattostr(gap));
     if not parentreport.Exporting then Y := Y + LineHeight;
    finally
     words.free;
    end;
  end;
  ...
  begin
    ...
    begin
        if self.FFullJustify then
        begin
               JustifyLine( aCanvas, X, Y,
                   Flags, aRect, FFormattedLines[LineNumber], length(FFormattedLines[LineNumber]));
               exit;
        end;
    ...
    begin
         if self.FFullJustify then
               JustifyLine( aCanvas, X, Y,
                   Flags, aRect, FFormattedLines[LineNumber], length(FFormattedLines[LineNumber]))
         else if (ParentReport.ExportFilter is TQRXMLSSDocumentFilter) or (ParentReport.ExportFilter is TQRXLSXDocumentFilter) then
         begin
              exportInfo.Xpos := X;
              exportInfo.Ypos := Y;
              exportInfo.Font := Font;
              exportInfo.BGColor := Color;
              exportInfo.Alignment := AAlignment;
              exportInfo.Text := FFormattedLines[LineNumber];
              exportInfo.Column := self.XLColumn;
              exportInfo.XLNumFormat := self.XLNumFormat;
              exportInfo.ExportAs := self.ExportAs;
              exportInfo.ControlName := self.Name;
              TQRXMLSSDocumentFilter(ParentReport.ExportFilter).TextOutRec(exportInfo);
         end
         else if ParentReport.ExportFilter is TQRXLSFilter then
         begin
              exportInfo.Xpos := X;
              exportInfo.Ypos := Y;
              exportInfo.Font := Font;
              ...
              exportInfo.Column := self.XLColumn;
              exportInfo.ExportAs := self.ExportAs;
              TQRXLSFilter(ParentReport.ExportFilter).TextOutRec(exportInfo);
         end
         else
...
// used in QR3.5
  ...
  begin
    ...
    tmpx := (height - tmpx)/QRPrinter.YFactor - tmpx;
    case self.VerticalAlignment of
       TTextlayout.tlCenter  :  Y :=  Y + tmpx/2;
       TTextlayout.tlBottom  :  Y :=  Y + tmpx;
    end;
  end;
...
begin
  ...
  begin
    ...
    case aValue.Kind of
      ...
      // unicode value assigned
      resString : FPrintCaption := aValue.StringVal;
      resDouble :
      begin
      ...
      resError : FPrintCaption := aValue.StringVal;
    end;
    inherited Print(OfsX, OfsY);
...
procedure TQRCustomRichText.CreateWnd;
begin
  inherited;
  if globalTransparentRTF then
     SetWindowLong(Handle, GWL_EXSTYLE, GetWindowLong(Handle, GWL_EXSTYLE) or WS_EX_TRANSPARENT);
...
procedure TQRCustomRichText.SetRTFTransparent( value : boolean);
begin
    globalTransparentRTF := value;
end;

function TQRCustomRichText.getRTFTransparent:boolean;
begin
    result := globalTransparentRTF;
end;
...
var
  ...
  begin
    if RenderIt then
      Result := SendMessage(ARichEdit.Handle, EM_FORMATRANGE, 1, LPARAM(@Range))
    else
      Result := SendMessage(ARichEdit.Handle, EM_FORMATRANGE, 0, LPARAM(@Range));
  end;
...
begin
  ...
  begin
    ...
    TextLength := SendMessage(ARichedit.Handle, EM_GETTEXTLENGTHEX, WPARAM(@TextLen), 0);
    chrg.cpMax := -1;
    chrg.cpMin := LastChar;
  ...
  begin
       ...
       SendMessage(ARichEdit.Handle, EM_FORMATRANGE, 1, LPARAM(@Range));
       ...
       if FHiresExport then printer.Abort;
...
var
    ...
    Begin
      ...
      AnsiEnc := TMBCSEncoding.Create(1252, 0, 0);
      result := ansistring(AnsiEnc.GetString(uniEnc.convert(uniEnc,AnsiEnc,uniEnc.GetBytes(value))));
      AnsiEnc.Free;
    End;
begin
  if assigned(Field) then
  begin
    if ((Field is TMemoField) or (Field is TBlobField)) and
       (Length(Field.AsString) > 0) then
       begin
    ...
    begin
      ...
      begin
        ...
        case aResult.Kind of
          ...
          resString : Replacement := aResult.StringVal;
