{ :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  :: QuickReport 506.0 for Delphi and C++Builder               ::
  ::                                                         ::
  ...
  :: 11/03/2009 TCustomQuickRep.Print ( free Progress)
  :: 8th June 2009 custom paper orientation bugfix
  :: 25th June 2009 - add wrapstyle property to printables   ::
  :: 4th January 2010  Add continuous paper                  ::
  :: TQRCompositereport.QRPrinter new read only property
  :: TQRCompositereport.prepare changed
  :: 16/01/2010 PreviewLeft, PreviewTop added
  :: 22/01/10 CustomBand.print summary band in multicolumn   ::
  :: 02/02/2010 Customband.print : bug in bandwidths in
  ::            multicolumn fixed.
  :: 23/02/2010 StartNewpageInPreview
  :: 23/02/2010 CreateInBGThread
  :: 07/03/2010 Add TQRCompositeReport.PreviewXML
  :: 12/04/2010 Fix of group ForceNewPage and ReprintOnNewpage bug
  :: 01/08/2011 TQRcr.Preview, stop preview if showing
  :: QR6
  :: 04/12/2013 add OnExport event
  :: 20/05/2014 multi-col composite reps col-header fixed
  :: 12/09/2014 fixed parentcomposite bug in qrcompositereport.exporttofilter
  :: 06/11/2014 fix zoom before exporting in exporttofilter
  :: 19/11/2014 PreviewDefaultSaveType defaults to TPrevSaveType.stPDF
  :: 22/04/2015 Change CustomBand.Print to allow childband to do newpage
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: }
...
type
  ...
  TCompositeEvent = procedure( Sender : TObject; Report : TObject; reportNumber : integer ) of object;
   // QR6
  TQRExportEvent = procedure( Sender : TCustomQuickRep;  var ExportHandled : boolean ) of object;

  // QR6
  TQRPreviewController = class(TComponent)
  private
       // buttons
       FshowMinimise : boolean;
       FshowMaximise : boolean;
       FshowSysmenu : boolean;
       FshowSaveReport : boolean;
       FshowLoadReport : boolean;
       FShowPrint : boolean;
       FShowPrintSetup : boolean;
       FShowFindButton : boolean;
       FShowCopyButton : boolean;
       FShowGotoButton : boolean;
       FShowZoomFitButton : boolean;
       FShowZoom100Button : boolean;
       FShowZoomToWidthButton : boolean;
       FShowCancelButton : boolean;
       FShowZoomPickButton : boolean;
       FShowFirstButton : boolean;
       FShowNextButton : boolean;
       FShowPrevButton : boolean;
       FShowLastButton : boolean;
       // properties
       FPagesInPreview : integer;
       FprevInitWidth : integer;
       FprevInitHeight : integer;
       FPrevInitTop : integer;
       FPrevInitLeft : integer;
       FPrevWinStyle : TFormstyle;
       FPrevDefaultSave : TPrevSaveType;
       FPrevInitState : TWindowstate;
       FPrevInitZoom : TQRZoomstate;
       FShowThumbs : boolean;
    public
       constructor Create(aOwner : TComponent); override;
    published
       property ShowMinimise : boolean read FshowMinimise  write  FshowMinimise default true;
       property ShowMaximise : boolean read FshowMaximise  write  FshowMaximise default true;
       property ShowSaveReportButton : boolean read FshowSaveReport  write  FshowSaveReport default true;
       property ShowLoadReportButton : boolean read FshowLoadReport  write  FshowLoadReport default true;
       property ShowPrintButton : boolean read FShowPrint  write  FShowPrint default true;
       property ShowPrintSetupButton : boolean read FShowPrintSetup  write  FShowPrintSetup default true;
       property ShowFindButton : boolean read FShowFindButton  write  FShowFindButton default true;
       property ShowCopyButton : boolean read FShowCopyButton  write  FShowCopyButton default true;
       property ShowSysMenu : boolean read FshowSysmenu  write  FshowSysmenu default true;
       property ShowGotoButton : boolean read FShowGotoButton  write  FShowGotoButton default true;
       property ShowZoomFitButton : boolean read FShowZoomFitButton  write  FShowZoomFitButton default true;
       property ShowZoom100Button : boolean read FShowZoom100Button  write  FShowZoom100Button default true;
       property ShowZoomToWidthButton : boolean read FShowZoomToWidthButton  write  FShowZoomToWidthButton default true;
       property ShowCancelButton : boolean read FShowCancelButton  write  FShowCancelButton default true;
       property ShowZoomPickButton : boolean read FShowZoomPickButton  write  FShowZoomPickButton default true;
       property ShowFirstButton : boolean read FShowFirstButton  write  FShowFirstButton default true;
       property ShowNextButton : boolean read FShowNextButton  write  FShowNextButton default true;
       property ShowPrevButton : boolean read FShowPrevButton  write  FShowPrevButton default true;
       property ShowLastButton : boolean read FShowLastButton  write  FShowLastButton default true;
       // properties
       property PagesInPreview : integer read FPagesInPreview  write  FPagesInPreview;
       property prevInitWidth : integer read FprevInitWidth  write  FprevInitWidth default 800;
       property prevInitHeight : integer read FprevInitHeight  write  FprevInitHeight default 600;
       property PrevInitLeft : integer read FPrevInitLeft  write  FPrevInitLeft;
       property PrevInitTop : integer read FPrevInitTop  write  FPrevInitTop;
       property PrevDefaultSaveType : TPrevSaveType read FPrevDefaultSave  write  FPrevDefaultSave;
       property PrevWindowStyle : TFormstyle  read FPrevWinStyle write  FPrevWinStyle;
       property PrevInitState : TWindowstate  read FPrevInitState write  FPrevInitState;
       property PrevInitZoom : TQRZoomstate  read FPrevInitZoom write  FPrevInitZoom;
       property ShowThumbs : boolean  read FShowThumbs write  FShowThumbs default true;
  end;
  ...
  published
    ...
    property DrawTop : boolean index 0 read FTop write SetValue default False;
    property DrawBottom : boolean index 1 read FBottom write SetValue default False;
    property DrawLeft : boolean index 2 read FLeft write SetValue default False;
    property DrawRight : boolean index 3 read FRight write SetValue default False;
  ...
  private
    ...
    // QR506
    FOnPreviewShowing : TQRAfterPreviewEvent;
    ...
    // jan 2010
    FStartNewPageInComposite : boolean;
    FCreateInBGThread : boolean;
    FPreviewLeft : integer;
    FPreviewTop  : integer;
    // QR6
    FOnExport : TQRExportEvent;

    FPreviewSetup : TQRPreviewController;
...
{$endif}
    ...
    property ShowingPreview : boolean read FShowingPreview write FShowingPreview;
    ...
    property OnPreviewShowing : TQRAfterPreviewEvent read FOnPreviewShowing write FOnPreviewShowing;
    ...
    // jan 2010
    Property PreviewLeft : integer read FPreviewLeft write FPreviewLeft default 100;
    Property PreviewTop : integer read FPreviewTop write FPreviewTop default 100;
    Property StartNewPageInComposite : boolean read FStartNewPageInComposite write FStartNewPageInComposite default false;
    Property CreateInBGThread : boolean read FCreateInBGThread write FCreateInBGThread default false;     
    Property PreviewController : TQRPreviewController read  FPreviewSetup write FPreviewSetup;
    Property OnExport : TQRExportEvent read FOnExport write  FOnExport;
  end;
  ...
  published
    ...
    // jan 2010
    property PreviewLeft;
    property PreviewTop;
    Property StartNewPageInComposite;
    property CreateInBGThread;
    // qr6
    Property PreviewController;
    Property OnExport;
    property OnPreviewShowing;// TQuickrep and TAbstractQuick ...
  end;
...
{$endif}
  ...
  private
    ...
    FXLColumn : integer;
    FXLNumFormat : TXLNumFormat;
    // QR506
    FParagraphChar : string;
    FParagraphIndent : string;
  ...
  public
    ...
    property ParagraphChar : string read FParagraphChar write FParagraphChar;
    property ParagraphIndent : string read FParagraphIndent write FParagraphIndent;

  published
    property Enabled;
    property Frame : TQRFrame read FFrame write SetFrame;
    property Size : TQRPrintableSize read FSize write FSize;
    property XLColumn : integer read FXLColumn write FXLColumn;
    property XLNumFormat : TXLNumFormat read FXLNumFormat write FXLNumFormat;
  end;
...
{$endif}
    ...
    procedure PrinterSetup;
    procedure Preview;
    procedure PreviewXML;
    procedure Print;
    property Index : integer read FIndex;
    property Reports : TList read FReports write FReports;
    property QRPrinter : TQRPrinter read FQRPrinter;
  published
...
{TQRPreviewSetup}
 constructor TQRPreviewController.Create(aOwner : TComponent);
 begin
     inherited create(aOwner);
     self.fshowMinimise := true;
     self.fshowMaximise := true;
     self.ShowSysMenu := true;
     self.ShowGotoButton := true;
     self.ShowCancelButton := true;
     self.fshowSaveReport := true;
     self.fshowLoadReport := true;
     self.FShowPrint := true;
     self.FShowPrintSetup := true;
     self.FShowFindButton := true;
     self.FShowCopyButton := true;
     self.FShowThumbs := true;
     self.ShowZoomFitButton := true;
     self.ShowZoom100Button := true;
     self.ShowZoomPickButton := true;
     self.ShowZoomToWidthButton := true;
     self.ShowFirstButton := true;
     self.ShowNextButton := true;
     self.ShowPrevButton := true;
     self.ShowLastButton := true;

     self.FprevInitWidth := 800;
     self.FprevInitHeight := 640;
 end;
...
{$endif}
    ...
    begin
      ...
      // does not work with multi-col reports
...
{$ifdef COMP_COL_HEADER_BUG}
      ...
      begin
         if (ParentReport.Bands.ColumnHeaderBand <> nil) and
            (ParentReport.Bands.ColumnHeaderBand.Enabled) then
         begin
           if GlobalCompositeFlag then
           begin
             if TQRCompositeReport(ParentReport.ParentComposite).FCurrReport = 0 then
             begin
                  ParentReport.PrintBand(ParentReport.Bands.ColumnHeaderBand);
                  ParentReport.NoForceNewPage := true;
             end;
...
begin
  ...
  begin
    ...
    begin
      case aValue.Kind of
        resString : Reprint := aValue.StringVal <> GroupValue.StringVal;
  ...
  begin
       ...
       begin
           ...
           begin
               ...
               aCanvas.Pen.Width := TQRFrameline(TQRCustomband(fparent).Controls[i]).FFrame.Width;
               aCanvas.Pen.Color := TQRFrameline(TQRCustomband(fparent).Controls[i]).FFrame.FColor;
               tempx := TQRFrameline(TQRCustomband(fparent).Controls[i]).Left + ( TQRFrameline(TQRCustomband(fparent).Controls[i]).width div 2);
               tempx := arect.Left + round( 1.0*(arect.Right-arect.Left)* tempx/TQRCustomband(fparent).Width);
...
var
   tmp : extended;
begin
  if (papersize=Custom) and (value<>FOrientation) then
  begin
      // fixes the custom size landscape bug see also 'SetPrinterValues'
      tmp := Width;
      Width := Length;
      Length := tmp;
      SetParentSizes;
  ...
  FOrientation := Value;
  parentreport.PrinterSettings.Orientation := value;
  PaperSize := PaperSize;
end;
...
begin
  ...
  begin
    ...
    Font.Name := TFontname(cqrRulerFontName);
...
begin
  ...
  begin
    ...
    begin
      ...
      if ParentReport.Page.Continuous and not (ParentReport.PageNumber = 0) then exit;
      ParentReport.NewColumn;
    end
    else
...
var
  I : integer;
  OrgLength : extended;
  newHeight, awidth : extended;
  saveExpanded : extended;
  PrintBand : boolean;
  MyChild : TQRChildBand;
  AllFinished : boolean;
  FirstPass : boolean;
  OKforGroupNP : boolean;
...
begin
  ...
  if (BandType in [rbSummary]) and (parentreport.Page.Columns>1) then
  begin
       // summary band multicolumn fix Feb 2010 : now seems OK
       if not aligntobottom then
       begin
           aWidth :=  parentreport.page.Width - (parentreport.Page.LeftMargin + parentreport.Page.RightMargin);
           aWidth := ((aWidth - parentreport.Page.ColumnSpace * (parentreport.Page.Columns - 1)) / parentreport.Page.Columns);
           parentreport.currentx := round(parentreport.Page.LeftMargin + (parentreport.CurrentColumn-1)*awidth);
           parentreport.currentx := parentreport.CurrentX + round(parentreport.Page.ColumnSpace * (parentreport.CurrentColumn - 1));
           self.Size.FWidth := awidth;
       end
       else
       begin
           // the summary band has to go on a new page : we cannot reserve space at the
           // bottom of every page for the band.
           if parentreport.Page.Columns>1 then
               parentreport.NewPage
       end;
  end;
  ...
  begin
    ...
    begin
        OKforGroupNP := true;
        if ( bandtype = rbGroupHeader ) then
                    OKforGroupNP := not TQRGroup(self).InGroup;
        if OKforGroupNP then
        begin
...
//------------------------------------------------}
  ...
  begin
    ...
    begin
      if BandType = rbPageFooter then
         MyChild.BandType := rbPageFooter;
       pageBroken := false;// this enables a child band to start a new page
       ParentReport.PrintBand(MyChild);
       MyChild.BandType := rbChild;
      end;
  end;
...
{      if aWidth <> Size.Width then Size.Width := aWidth;}
      ...
      begin
        ...
        // this can start a recursive lock because SetBandValues call this (SetBounds)
        //ParentReport.SetBandValues;
      end;
      Size.Units := aUnit;
...
begin
  inherited Create(AOwner);
  BandType := rbChild;
  self.FPrintOrder := cboAfterParent;
end;
...
begin
  ...
  FPreviewDefaultSaveType := TPrevSaveType.stPDF;
end;
...
begin
   ...
   // this together with changes in TQRPage.setorientation fixes the custom size landscape bug
   QRPrinter.Orientation := Page.Orientation;
   if QRPrinter.PaperSize = Custom then
   begin
        if Page.Orientation = poPortrait then
        begin
          QRPrinter.PaperWidth := round(Page.FWidth);
          QRPrinter.PaperLength := round(Page.FLength);
        end
        else
        begin
          QRPrinter.PaperWidth := round(Page.FLength);
          QRPrinter.PaperLength := round(Page.FWidth);
        end;
   end;
...
begin
  ...
  try
    ...
    QRPrinter := OldQRPrinter;
    // 11/03/2009
    //if showprogress and Assigned(AProgress) then
    FreeAndNil(AProgress);
  end else
  begin
    if QRPrinter <> nil then
...
begin
  if qrprinter<>nil then
     if qrprinter.ShowingPreview  then  exit;

  if assigned(FBeforePreview) then
        FBeforePreview( self );
  Application.ProcessMessages;
  if CreateInBGThread then
   PreviewIt( false)
  else
  begin
    if PrepareQRPrinter and not QRPrinter.ShowingPreview then
    begin
      ...
      try
        CreateReport(false);
         ...
         TForm( QRPrinter.Client.parent).Close;
         QRPrinter.Free;
         Exit;
      end;
      QRPrinter.ReportLoaded := true;
      if assigned(FOnPreviewshowing) then
        FOnPreviewshowing(Self);
      if Cancelled then
      begin
        QRPrinter.Free;
        QRPrinter := nil;
...
begin
  ...
  aDialog.printer.PrinterIndex := Printer.PrinterIndex;
...
begin
  ...
  try
    ...
    AProgress := TQRProgressForm.Create(nil);
    AProgress.QRPrinter := QRPrinter;
    AProgress.Caption := sqrExportProgress;
...
begin
  ...
  for I := 0 to reports.Count-1 do
               TQuickRep(Reports[I]).ParentComposite := self;
  ...
  begin
    ...
    begin
      ...
      if aReport.StartNewPageInComposite then
          aReport.CurrentY := round(aReport.QRPrinter.PaperLength);
...
begin
  ...
  for k := 0 to reports.Count-1 do
               TQuickRep(Reports[k]).ParentComposite := self;
...
begin
  ...
  temprep.PrevShowThumbs := firstrep.PrevShowThumbs;
   // jan 2010
  temprep.PreviewLeft := firstrep.PreviewLeft;
  temprep.PreviewTop := firstrep.PreviewTop;
  temprep.previewdefaultsavetype :=  firstrep.previewdefaultsavetype;
...
procedure TQRCompositeReport.PreviewXML;
var
    qrxDoc : TQRXDocument;
    firstreport : TQuickrep;
begin
    qrxDoc := TQRXDocument.Create(self);
    if assigned(FOnAddReports) then
      FOnAddReports(self);

    firstreport := TQuickrep(reports[0]);
    qrxDoc.Showthumbs := firstreport.PrevShowThumbs;
    qrxDoc.ShowSearch := firstreport.PrevShowSearch;
    qrxDoc.ShowToolbar := true;
    qrxDoc.ShowPrintButtons := true;
    qrxDoc.ShowSaveButton := true;
    qrxDoc.ShowLoadButton := true;
    qrxDoc.InitViewerWidth := firstreport.PreviewWidth;
    qrxDoc.InitViewerHeight := firstreport.PreviewHeight;
    qrxDoc.InitViewerTop := firstreport.PreviewTop;
    qrxDoc.InitViewerLeft := firstreport.PreviewLeft;
    qrxDoc.InitViewerZoom := firstreport.Zoom;
    qrxDoc.InitViewerZoomState := firstreport.PrevInitialZoom;
    qrxDoc.InitViewerState := firstreport.PreviewInitialState;

    qrxDoc.ViewCompositeReport(self);
end;
...
begin
    FQRPrinter := TQRPrinter.Create( self );
    CreateComposite;
    // allows FQRPrinter.print to work
    FQRPrinter.ParentReport := Reports[0];
end;
    ...
    if Environment.IndexOf(Query.Params.Items[I].Name) >= 0 then
      ...
      begin
        ...
        case DataType of
          ...
          ftDate : if AResult.Kind = resString then AsDate := StrToDate(AResult.StringVal);
          ftTime : if AResult.Kind = resString then AsTime := StrToTime(AResult.StringVal);
          ftDateTime : if AResult.Kind = resString then AsDateTime := StrToDateTime(AResult.StringVal);
        end;
      end;
  Query.Active := true;
  //aresult.Free;
