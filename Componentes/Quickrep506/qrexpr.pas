{ :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  ::
  :: 11/06/2010 fixed Expressions to work with unicode       ::
  ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: }
...
uses
  Windows, Sysutils, Classes, DB, Forms, QR5Const;
...
begin
  case AValue.Kind of
    resString : Result := AValue.StringVal;
    ...
    Raise Exception.Create(Format(SqrExpError, [AValue.StringVal]));
  end;
end;
...
begin
  case AValue.Kind of
    ...
    resString : if AnsiUppercase(AValue.StringVal) = 'TRUE' then // Do not translate
                  Result := true
                else
                  if AnsiUppercase(AValue.StringVal) = 'FALSE' then // Do not translate
                    Result := false
                  else
  ...
  if (Res1.Kind <> resError) and (Res2.Kind <> resError) then
    ...
    begin
      ...
      begin
        ...
        begin
          Res1.StringVal := QREvResultToString(Res1);
          Res1.Kind := resString;
          Res2.StringVal := QREvResultToString(Res2);
          Res2.Kind := resString;
        end;
    end
end;
  ...
  if result.Kind <> resError then
    case FOpCode of
      opPlus: case result.Kind of
                ...
                resString: result.StringVal := Res1.StringVal + Res2.StringVal;
      ...
      opGreater: begin
                   ...
                   case Res1.Kind of
                     ...
                     resString: result.booResult := Res1.StringVal > Res2.StringVal;
...
type
  ...
  private
    FValue : string;
  public
    constructor CreateString(Value : TQREvResult);
    function Value(FiFo : TQRFiFo) : TQREvResult; override;
  end;

constructor TQREvElementString.CreateString(Value : TQREvResult);
begin
  inherited Create;
  FValue := Value.StringVal;
end;

function TQREvElementString.Value(FiFo : TQRFiFo) : TQREvResult;
begin
  result.Kind := resString;
  result.StringVal := FValue;
  result.StringVal := FValue;
end;
...
begin
  ...
  if FField is TWideStringField then
  begin
    result.Kind := resString;
    result.StringVal := TWideStringField(FField).Value;
  end
  else if FField is TStringField then
  begin
    result.Kind := resString;
    result.StringVal := string(TStringField(FField).Value);
  end
  ...
  begin
       result.Kind := resString;
       result.StringVal := TDateField(FField).AsString;
  end
...
begin
  Result.Kind := resError;
  Result.StringVal := FErrorMessage;
end;
...
begin
  result.Kind := resString;
  // line below loses unicode
  result.CodePage := StringCodePage( strString);
  result.StringVal := strString;
  FiFo.Put(TQREvElementString.CreateString(Result));
end;
...
begin
  N := 1;
  aString := strConstant;
  while (N <= Length(aString)) and  charinset(aString[N], ['0'..'9']) do
    inc(N);
  result.Kind := resInt;
  while ((N <= Length(aString)) and charinset(aString[N],['0'..'9', '.', 'e', 'E', '+', '-'])) do
  begin
    inc(N);
    result.Kind := resDouble;
  end;
    ...
    begin
      ...
      begin
        while pos('.', aString) > 0 do
          aString[pos('.', aString)] := FormatSettings.DecimalSeparator;
      end;
      try
  ...
  if result.Kind = resError then
    FiFo.Put(TQREvElementError.Create(Result.StringVal))
  else
...
begin
  ...
  begin
    case Argument(0).Kind of
      resInt : Result.StringVal := 'INTEGER';    // Do not translate
      resDouble : Result.StringVal := 'FLOAT';   // Do not translate
      resString : Result.StringVal := 'STRING';  // Do not translate
      resBool : Result.StringVal := 'BOOLEAN';   // Do not translate
      resError : Result.StringVal := 'ERROR';    // Do not translate
    else
...
begin
  ...
  begin
    ...
    Result.Kind := resError;
  if Result.Kind = resError then
    Result.StringVal := SqrSQRTInvalid;
end;
...
begin
  ...
  begin
    ...
    begin
      Result.Kind := resError;
      Result.StringVal := SqrDivisionByZero;
    end
  end else
  begin
    Result.Kind := resError;
    Result.StringVal := Format(SqrExpWrongArguments, ['DIV']);
  end;
end;
...
begin
  ...
  begin
    ...
    Result.Kind := resString;
    case ArgRes.Kind of
      resInt : Result.StringVal := IntToStr(ArgRes.IntResult);
      resDouble : Result.StringVal := FloatToStr(ArgRes.DblResult);
      resBool : if ArgRes.booResult then
                result.StringVal := 'True' // Do not translate
              else
                result.StringVal := 'False'; // Do not translate
    else
      result.Kind := resError;
    end
  end else
    Result.Kind := resError;
  if Result.Kind = resError then
    Result.StringVal := SqrConversionError;
end;
...
begin
  ...
  begin
    Result.Kind := resString;
    Result.StringVal := AnsiUpperCase(Argument(0).StringVal);
  end else
  begin
    Result.Kind := resError;
    if ArgList.Count = 1 then
      Result.StringVal :=Argument(0).StringVal
    else
      Result.StringVal := Format(SqrMissingArgument, ['UPPER']);
  end;
end;
...
begin
  ...
  begin
    Result.Kind := resString;
    Result.StringVal := AnsiLowerCase(Argument(0).StringVal);
  end else
  begin
    Result.Kind := resError;
    if ArgList.Count = 1 then
      ...
      Result.StringVal := Format(SqrMissingArgument, ['LOWER']);
  end;
end;
...
begin
  ...
  begin
    ...
    if (Length(Argument(0).StringVal) > 0) and CharInSet(Argument(0).StringVal[1],LeadBytes) then
      Result.StringVal := AnsiUpperCase(Copy(Argument(0).StringVal, 1, 2)) +
                          AnsiLowerCase(Copy(Argument(0).StringVal, 3, length(Argument(0).StringVal)))
    else
      Result.StringVal := AnsiUpperCase(Copy(Argument(0).StringVal, 1, 1)) +
                          AnsiLowerCase(Copy(Argument(0).StringVal, 2, length(Argument(0).StringVal)));
  end else
  begin
    Result.Kind := resError;
    if ArgList.Count = 1 then
      ...
      Result.StringVal := Format(SqrMissingArgument, ['PRETTY']);
  end;
end;
...
begin
  ...
  begin
    Result.Kind := resString;
    Result.StringVal := TimeToStr(Now);
  end else
    Result.Kind := resError;
end;
...
begin
  ...
  begin
    Result.Kind := resString;
    Result.StringVal := DateToStr(Date);
  end else
    Result.Kind := resError;
end;
...
begin
  ...
  begin
    ...
    if (ByteType(Argument(0).StringVal, Start) = mbTrailByte) and (Len > 0) then
    begin
      Inc(Start);
      Dec(Len);
    end;
    if (ByteType(Argument(0).StringVal, Start + Len - 1) = mbLeadByte) and (Len > 0) then Dec(Len);
    Result.StringVal := copy(Argument(0).StringVal, Start, Len);
    Result.Kind := resString;
  end else
    Result.Kind := resError;
end;
...
begin
  ...
  begin
    ...
    try
      if Argument(1).Kind = resInt then
        Result.StringVal := FormatFloat(Argument(0).StringVal, Argument(1).IntResult * 1.0)
      else
        Result.StringVal := FormatFloat(Argument(0).StringVal, Argument(1).DblResult);
    except
      Result.Kind := resError;
      Result.StringVal := SqrInvalidFormatnumeric;
    end;
  end else
    Result.Kind := resError;
end;
  ...
  begin
    ...
    begin
      case MaxResult.Kind of
        ...
        resString : if aValue.StringVal > MaxResult.StringVal then
                      MaxResult.StringVal := aValue.StringVal;
  ...
  begin
    ...
    begin
      case MinResult.Kind of
        ...
        resString : if aValue.StringVal < MinResult.StringVal then
                      MinResult.StringVal := aValue.StringVal;
...
begin
  if Constant is TQREvElementConstant then
    ...
    if (Constant is TQREvElementString) and (Value.Kind = resString) then
      TQREvElementString(Constant).FValue := Value.StringVal;
end;
...
begin
  if AConstant is TQREvElementConstant then
    ...
    if (AConstant is TQREvElementString) and (Value.Kind = resString) then
      TQREvElementString(AConstant).FValue := Value.StringVal;
end;
