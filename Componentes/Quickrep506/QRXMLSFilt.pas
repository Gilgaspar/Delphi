...
//  10/03/2010 added rotated font support
//  19/04/2010 added underline and strikeout font
//  24/04/2013 DC updates
//  24/04/2013 fixed default colours in DTD to valid webcolors
////////////////////////////////////////////////////////////////////////////
{$include QRDefs.inc}
interface
...
type
 ...
 public
    ...
    procedure NewPage; override;
    procedure EndPage; override;
    procedure AcceptVector( comms : TStringlist);
    procedure AcceptBand( aBand : TControl; Xoff, Yoff, Expanded : extended); override;
    procedure AcceptGraphic( Xoff, Yoff : extended; GControl : TControl); override;
    procedure SetDocumentProperties( author, title, copyright : string );
    procedure AddToStream( xmlInlist : TStringlist);
    property Stream;
...
implementation
uses lzw, grimgctrl
  , GraphUtil // added by DCA 21/04/2013 for ColorToWebColorStr
  ;
...
// added by DCA on 09/04/2013
destructor TQRXAbstractExportFilter.Destroy;
begin
  FreeAndNil(FStream); // fixes FStream memory leak
  inherited;
end;
...
procedure TQRXDocumentFilter.AddToStream( xmlInlist : TStringlist);
var
  k : integer;
begin
     for k  := 0 to xmlInlist.Count-1 do
             writeToStream(xmlInlist[k]);
end;
...
begin
     ...
     dlist.add('<!ATTLIST Item Color CDATA "#000000">');
     dlist.add('<!ATTLIST Item BC CDATA "#FFFFFF">');
     dlist.add('<!ATTLIST Item FW CDATA "Normal">');
     dlist.add('<!ATTLIST Item FR CDATA "0">');
...
// DCA 21/04/2013: replaced function ColTrans by function ColorToWebColorStr
...
begin
    ...
    writetostream( '</Header>' + CRLF );
       ...
       dlist.Free; // added by DCA on 09/04/2013
    end;
end;
...
begin
    ...
    writetostream( '</Page>'); // DCA: wrong if no <Page> created
    writetostream( '</QXDocument>' );
    case TextEncoding of
      AnsiEncoding: penc := TMBCSEncoding.Create(1252, 0, 0);
...
var
  I  : integer;
  parentrep : TQuickrep;
  ctext, fbold, fitalic, funderline, fstrike, decstr : string;
...
begin
    ...
    if fsStrikeout in Font.Style then fstrike := 'Strike';
    decstr := fitalic + ' ' + funderline + ' ' + fstrike;
    // rotation stuff
    GetObject(Font.Handle, sizeof(lf), @lf);
    fontrot := lf.lfOrientation;
    i := font.size;
    ctext := format('<Item Type="Text" LY="0" Font="%-s" X="%-4.2f" Y="%-4.2f" Height="%-d" Color="%-s" FR="%3d" FW="%s" Dec="%-s" CID="%-s" DBR="%-d">',
                    [ font.name, Xmm, Ymm, i, ColorToWebColorStr(font.color),fontrot, fbold, decstr, ExportingControlname, ExportingRecordnumber ]);
    writetostream( ctext);
    ctext := text;
...
procedure TQRXDocumentFilter.AcceptVector( comms : TStringlist);
var
  ctext : string;
  k : integer;
begin
         ctext := ' ';
         for k := 0 to comms.Count-1 do
         begin
            ctext:= ctext + comms[k] + '!';
         end;
         ctext := copy(ctext,1,length(ctext)-1);
         writetostream( '<Item Type="Vector">');
         writetostream( ctext);
         writetostream( '</Item>' );
end;
         ...
         ctext := format('<Item Type="Graphic" LY="'+layer+'" X="%-4.2f" Y="%-4.2f" LW="%-4.2f" '+
                  'SP="%-d" Width="%-4.2f" Height="%-4.2f" Color="%s" BC="%s" FT="%s" CID="%-s" DBR="%-d">',
                    [  Xmm, Ymm, 25.4*sscont.pen.width / Screen.PixelsPerInch, ishape, rectwidth, rectheight,
                      ColorToWebColorStr(sscont.pen.color), ColorToWebColorStr(sscont.Brush.color), fill, GControl.name, exportingRecordNumber ]);
         writetostream( ctext);
         writetostream( '</Item>' );
         exit;
    end;
