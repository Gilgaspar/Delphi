...
begin
  ...
  try
    ...
    if {$IFNDEF DELPHI104}not {$ENDIF}EnumFontFamiliesEx(ADC, ALogFont, @CheckFontStyles, LPARAM(@Result), 0){$IFDEF DELPHI104} = 0 {$ENDIF} then
...
begin
  ...
  if {$IFNDEF DELPHI104}not {$ENDIF}EnumFontFamiliesEx(ADC, ALogFont, @IsFontInstalled, LPARAM(@ALogFont), 0){$IFDEF DELPHI104} = 0{$ENDIF} then  
  begin
